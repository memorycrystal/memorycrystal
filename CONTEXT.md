...
An agent-runtime integration role that makes memory *capable* of automatic injection: relevant context *can* appear before the agent responds when inject recall is enabled, and completed turns can be captured without the user calling a tool. Default posture is tool-mode (no automatic injection) on every host. A Native Memory Provider is the background memory substrate, not the broad manual toolbox.
...
**Reflection Cycle**:
The once-per-day, per-user maintenance pass over a user's memory. It has exactly two phases in order: Distillation, then Forgetting. A Reflection Cycle is one job with one cost line and one run record, not a family of independent memory crons.
_Avoid_: Reflection (bare), decay run, cleanup, consolidation

**Distillation**:
The first phase of a Reflection Cycle: turning raw short-term messages into durable long-term memories that keep the specifics — who, what, when, and the relationship of every named person to the user — while dropping transcript fluff. Distillation reads messages, never other memories.
_Avoid_: Reflection, summarization, extraction, consolidation

**Forgetting**:
The second phase of a Reflection Cycle: archiving an account's lowest-value memories, and only ever the number by which that account exceeds its Memory Allowance. Forgetting never runs pre-emptively below the allowance, and it archives rather than deletes, so anything it takes stays recoverable until the archived purge window closes.
_Avoid_: Decay, purge, cleanup, pruning

**Memory Value**:
The ranking a memory carries into Forgetting, combining its strength, how recently it was genuinely recalled, and how often it has been recalled. Memory Value measures proven use, so a memory that has never had the chance to be recalled is exempt rather than ranked lowest.
_Avoid_: Salience, strength, score, decay score

**Memory Allowance**:
The number of active conversational long-term memories an account may retain for its plan. Knowledge-base chunks are outside the Memory Allowance — they are user-supplied reference material governed by the knowledge-base lifecycle, so imported corpora can never crowd out an account's own memory. Exceeding the Memory Allowance is what triggers Forgetting; it is not a write-time rejection.
_Avoid_: Memory limit, quota, cap, storage limit, active memories

**Verbatim Window**:
How long an account's raw messages stay readable after capture. The Verbatim Window governs only the searchable transcript archive; it does not govern when Distillation runs, and a message is never dropped at the end of its Verbatim Window unless it has already been distilled.
_Avoid_: STM TTL, message retention, expiry

**Session Digest** _(retired)_:
The former memory-to-memory pass that re-read recent memories and wrote ungrounded `Decision:` / `Lesson:` / `Reflection summary:` / `Open loop:` memories. Superseded by Distillation, which emits the same categories directly from message evidence and can cite its source messages.
_Avoid_: Reflection, distillation
...
**Support Tier**:
The publicly declared portion of the Integration Contract that a platform actually satisfies. A Support Tier is bounded by what the harness can really do, never by what we intend it to do, so a platform that cannot recall automatically must not be advertised as if it can. The published values are Full, Capture, Tools, and Hosted.
_Avoid_: Supported, officially supported, full support

**Hosted Platform**:
A runtime that runs other agents rather than being installed into. Memory Crystal is installed in the CLIs the host runs. Buzz is a Hosted Platform. Channel memory is not available, and channel isolation is not enforced.
_Avoid_: install target, official CLI, isolated channel memory
...
Developer: "Reflection ran fine every night but the account still has 46,000 messages sitting in short-term."

Domain expert: "Then reflection is not what ran. Distillation reads messages; the job you're describing re-read memories. A Reflection Cycle isn't done until Distillation has run and Forgetting has been considered."

Developer: "The account is at 35,525 active memories against a 50,000 cap, so we're fine for now?"

Domain expert: "No — 15,488 of those are knowledge-base chunks, and knowledge-base chunks are outside the Memory Allowance. That account is at 20,037. Never quote active memories when you mean allowance."

Developer: "Their Verbatim Window closed but the messages were never distilled. Delete them?"

Domain expert: "Never. The Verbatim Window governs how long the transcript stays readable, not whether the memory was taken from it. An undistilled message is not eligible for deletion at any age."

Developer: "The account is at 92% of its allowance. Start trimming the weakest tail?"

Domain expert: "No. Forgetting runs only past the allowance, and only for the overage. At 92% you send the usage warning and do nothing else."

Developer: "This memory has never been recalled, so it's the lowest Memory Value on the account?"

Domain expert: "How old is it? If it hasn't lived through a Reflection Cycle yet it has never had the chance to be recalled. Exempt it — Memory Value measures proven use, not absence of evidence."

Developer: "The client's OpenRouter key is rate-limited, so we email them?"

Domain expert: "No. A rate limit isn't theirs to fix. Email immediately for out-of-credits, dead key, permission denied, or their own spend cap — the things a person can act on in five minutes."
