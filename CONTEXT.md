...
**Group Memory Scope**:
The shared memory boundary for a public or multi-person conversation. Group Memory Scope is opt-in for writes and must not include private direct-message or user-profile memory unless the user explicitly requests and authorizes that broader boundary.
