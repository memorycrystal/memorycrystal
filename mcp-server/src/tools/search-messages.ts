...
export const searchMessagesTool: Tool = {
  ...
  description:
    "Semantic search over retained short-term messages, scoped to a channel/peer and optionally to a time window. Use for questions like \"what did <person> say last month about <topic>\": pass the topic as `query`, the peer as `channel`, and the window via `fromMs`/`toMs` (epoch ms) or `startDate`/`endDate` (ISO). Page deeper with `offset` (response.pagination.hasMore/nextOffset) instead of assuming the first page is complete — never assert a date range is empty from one page.",
  inputSchema: {
    type: "object",
    properties: {
      query: {
      ...
      startDate: {
        type: "string",
        description: "Lower time bound as an ISO date/datetime (e.g. 2026-06-01).",
      },
      endDate: {
        type: "string",
        description: "Upper time bound as an ISO date/datetime (e.g. 2026-06-30).",
      },
      offset: {
        type: "integer",
        minimum: 0,
        description: "Skip this many ranked results for pagination; use response.pagination.nextOffset to walk the full set.",
      },
    },
    required: ["query"],
