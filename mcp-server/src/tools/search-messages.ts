...
const filterMessagesByChannel = (messages: STMMessage[], channel?: string): STMMessage[] => {
  if (!channel) return messages;
  return messages.filter((message) => message?.channel === channel);
};

const filterTurnsByChannel = (turns: MessageTurn[], channel?: string): MessageTurn[] => {
  if (!channel) return turns;
  const filtered: MessageTurn[] = [];
  for (const turn of turns) {
    const messages = Array.isArray(turn.messages)
      ? filterMessagesByChannel(turn.messages, channel)
      : [];
    if (messages.length > 0) {
      filtered.push({ ...turn, channel: turn.channel === channel ? turn.channel : messages[0]?.channel, messages });
    }
  }
  return filtered;
};
...
const handleSearchMessages = async (args: unknown): Promise<CallToolResult> => {
  ...
  const messages = Array.isArray(response)
    ...
    : Array.isArray(response?.messages)
      ? response.messages
      : [];
  const filteredMessages = filterMessagesByChannel(messages, parsed.channel);
  const turns = !Array.isArray(response) && Array.isArray(response.turns)
    ? filterTurnsByChannel(response.turns, parsed.channel)
    : [];
