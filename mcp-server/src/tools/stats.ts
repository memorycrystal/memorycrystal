...
// The account block answers "which account is this API key for?" — previously
// only an opaque userId came back. Read from the nested `account` object, with
// the flat aliases as a fallback for older deployments.
const buildAccountLines = (stats: Record<string, unknown>): string[] => {
  const account = (stats.account ?? {}) as Record<string, unknown>;
  const email = account.email ?? stats.email ?? null;
  const userId = account.userId ?? stats.userId ?? null;
  const name = account.name ?? null;
  if (!email && !userId) return [];

  return [
    "## Account",
    `- Email: ${email ?? "not set on this account"}`,
    ...(name ? [`- Name: ${String(name)}`] : []),
    ...(userId ? [`- User ID: ${String(userId)}`] : []),
    ...(stats.apiKeyLabel ? [`- API key: ${String(stats.apiKeyLabel)}`] : []),
    "",
  ];
};
