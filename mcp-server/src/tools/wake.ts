...
const redactContentField = (value: unknown): unknown => {
  if (typeof value !== "object" || value === null) return value;
  const row = value as Record<string, unknown>;
  return {
    ...row,
    ...(typeof row.content === "string" ? { content: redactSecrets(row.content) } : {}),
  };
};
...
export const handleWakeTool = async (args: unknown): Promise<CallToolResult> => {
  try {
    ...
    const payload = {
      briefing: redactSecrets(response.briefing),
      openGoals: Array.isArray(response.openGoals) ? response.openGoals.map(redactContentField) : [],
      recentDecisions: Array.isArray(response.recentDecisions) ? response.recentDecisions.map(redactContentField) : [],
    };
