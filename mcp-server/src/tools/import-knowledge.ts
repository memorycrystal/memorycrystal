...
function parseInput(args: unknown): ImportKnowledgeInput {
  ...
  const chunks = Array.isArray(input.chunks)
    ? input.chunks
        .filter((chunk) => chunk && typeof chunk === "object" && typeof (chunk as any).content === "string")
        .map((chunk) => ({
          content: String((chunk as any).content),
          metadata: (chunk as any).metadata && typeof (chunk as any).metadata === "object"
