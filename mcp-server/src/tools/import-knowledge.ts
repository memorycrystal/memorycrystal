...
export const importKnowledgeTool: Tool = {
  ...
  description:
    "Import text chunks into a Memory Crystal knowledge base. Give a chunk a stable `dedupeKey` (e.g. a song title) to UPSERT it: re-importing that key replaces the existing chunk in place (one key = one chunk) instead of appending a duplicate. Chunks without a dedupeKey append as before.",
  inputSchema: {
    type: "object",
    properties: {
      ...
      chunks: {
        ...
        items: {
          ...
          properties: {
            ...
            dedupeKey: {
              type: "string",
              description: "Stable upsert key within this KB; re-importing replaces the matching chunk in place.",
            },
            metadata: {
              type: "object",
              properties: {
  ...
  const chunks = Array.isArray(input.chunks)
    ? input.chunks
        ...
        .map((chunk) => {
          ...
          const dedupeKey = typeof (chunk as any).dedupeKey === "string" && (chunk as any).dedupeKey.trim()
            ? String((chunk as any).dedupeKey).trim().slice(0, 500)
            : undefined;
          return {
