...
type WriteToolResult = {
  ...
  // Advisory freshness warning (ILL-106) when a volatile "fast fact" is stored as
  // a timeless fact. Forwarded verbatim to the model via the `...result` spread.
...
export const rememberTool: Tool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      agentId: {
        type: "string",
        description: "Agent/profile identifier to store in metadata.",
      },
      projectId: {
        type: "string",
        description: "Project store ID for repo-scoped memory.",
      },
      repoSlug: {
        type: "string",
...
const ensureMemoryInput = (value: unknown): CrystalRememberInput => {
  ...
  const { store, category, title, content, tags, confidence, valence, arousal, channel, agentId, projectId, repoSlug } = input;
  ...
  if (valence !== undefined && (typeof valence !== "number" || !Number.isFinite(valence) || valence < -1 || valence > 1)) {
    throw new Error("Valence must be between -1 and 1");
  }
  if (arousal !== undefined && (typeof arousal !== "number" || !Number.isFinite(arousal) || arousal < 0 || arousal > 1)) {
...
export const handleRememberTool = async (args: unknown): Promise<CallToolResult> => {
  try {
    ...
    const result = await client.post<WriteToolResult & { ok: boolean; id: string }>("/api/mcp/capture", {
      ...
      ...(parsed.store === "sensory" && parsed.category === "conversation" ? { sensoryCaptureMode: "special_capture" } : {}),
    });
