...
export const toolHandlers: Record<string, ToolHandler> = {
  async crystal_capture(args) {
    const response = await client.post("/api/mcp/capture", {
      title: args?.title,
  ...
  async crystal_recall(args) {
    const response = await client.post("/api/mcp/recall", {
      query: args?.query,
      limit: args?.limit,
    });
    return textResult(response);
  ...
  async crystal_checkpoint(args) {
    const response = await client.post("/api/mcp/checkpoint", {
      label: args?.label,
  ...
  async crystal_wake(args) {
    const response = await client.post("/api/mcp/wake", {
  ...
  async crystal_stats() {
    const response = await client.get("/api/mcp/stats");
    return textResult(response);
  },
};
