  ...
  inputSchema: {
    ...
    properties: {
      ...
      includeGraphContext: {
        type: "boolean",
        default: true,
        description: "Include compact entity and relationship context. Set false for lower-latency recall."
      }
    },
    required: ["query"],
    additionalProperties: false
...
export const toolHandlers: Record<string, ToolHandler> = {
  async crystal_capture(args) {
    const response = await getClient().post("/api/mcp/capture", {
      ...
      tags: args?.tags,
      ...(args?.store === "sensory" && args?.category === "conversation" ? { sensoryCaptureMode: "special_capture" } : {}),
    });
