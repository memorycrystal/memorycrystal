...
const UPGRADE_LEGACY_API_KEY_MESSAGE =
  "CRYSTAL_API_KEY is no longer supported by Memory Crystal MCP. Upgrade Memory Crystal and set MEMORY_CRYSTAL_API_KEY.";
const UPGRADE_LEGACY_API_URL_MESSAGE =
  "MEMORY_CRYSTAL_BACKEND_URL and CRYSTAL_BASE_URL are no longer supported by Memory Crystal MCP. Upgrade Memory Crystal and set MEMORY_CRYSTAL_API_URL.";
...
export function hasApiKeyAuth(): boolean {
  if (!process.env.MEMORY_CRYSTAL_API_KEY && process.env.CRYSTAL_API_KEY) {
    throw new Error(UPGRADE_LEGACY_API_KEY_MESSAGE);
  }
  return Boolean(
...
export class ConvexClient {
  ...
  constructor(options: ConvexClientOptions = {}) {
    ...
    if (!baseUrl) {
      if (process.env.MEMORY_CRYSTAL_BACKEND_URL || process.env.CRYSTAL_BASE_URL) {
        throw new Error(UPGRADE_LEGACY_API_URL_MESSAGE);
      }
      throw new Error(
        "MEMORY_CRYSTAL_API_URL environment variable is required"
      );
    }
    ...
    if (!this.apiKey) {
      ...
      throw new Error(
        "MEMORY_CRYSTAL_API_KEY environment variable is required"
      );
    }
  }
