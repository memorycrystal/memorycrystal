...
const OPENROUTER_EMBEDDING_MODEL = "google/gemini-embedding-2-preview";
...
export function assertEmbeddingDimensions(vector: number[], label?: string): void {
  if (vector.length !== REQUIRED_EMBEDDING_DIMENSIONS) {
    throw new Error(
      ...
      "Ensure EMBEDDING_PROVIDER=openrouter with a 3072-dimensional embedding model."
    );
  }
}
...
export class OpenAIEmbedAdapter implements EmbedAdapter {
  constructor(_apiKey?: string) {
    throw new Error(
      ...
      "All vectors must be 3072 dimensions."
    );
  }
...
export class OpenRouterGeminiEmbedAdapter implements EmbedAdapter {
  ...
  constructor(config: { apiKey?: string; model?: string } = {}) {
    ...
    this.model = config.model ?? process.env.OPENROUTER_EMBEDDING_MODEL ?? OPENROUTER_EMBEDDING_MODEL;
  }
  ...
  async embed(text: string): Promise<number[] | null> {
    ...
    try {
      ...
      assertEmbeddingDimensions(values, `OpenRouter ${this.model}`);
      return values;
    } catch {
      return null;
    } finally {
  ...
  switch (provider) {
    ...
    case "openai":
      throw new Error(
        ...
        "All vectors must use OpenRouter and return 3072 dimensions."
      );
    default:
