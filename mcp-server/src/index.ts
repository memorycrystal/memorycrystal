...
import { handleHealthTool, healthTool } from "./tools/health.js";
...
import { handleSupersedeTool, supersedeTool } from "./tools/supersede.js";
...
import { handleEmptyKnowledgeBaseTool, emptyKnowledgeBaseTool } from "./tools/empty-knowledge-base.js";
import { handleListKnowledgeBaseMemoriesTool, listKnowledgeBaseMemoriesTool } from "./tools/list-knowledge-base-memories.js";
...
import { handleSetKnowledgeBaseAccessTool, setKnowledgeBaseAccessTool } from "./tools/set-knowledge-base-access.js";
import {
  handleResearchCreateCollection,
  handleResearchIngest,
  handleResearchPromote,
  handleResearchQuery,
...
export function consumeStreamableHttpQuota(rateLimitKey: string, now = Date.now()) {
  if (STREAMABLE_HTTP_RATE_LIMIT === 0) {
...
function createMcpServer() {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
      tools: [
        ...
        setKnowledgeBaseAccessTool,
        queryKnowledgeBaseTool,
        importKnowledgeTool,
        emptyKnowledgeBaseTool,
        listKnowledgeBaseMemoriesTool,
        ...researchTools,
      ],
    };
  });
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const result = await (async (): Promise<CallToolResult> => {
      ...
      // "crystal_supercede" is no longer advertised in tools/list — it was a
      // misspelled alias occupying a second slot. Still accepted here so a client
      // holding a cached tool list does not hard-fail; costs no slot.
      ...
      case "crystal_set_knowledge_base_access":
        return handleSetKnowledgeBaseAccessTool(args);
      ...
      case "crystal_empty_knowledge_base":
        return handleEmptyKnowledgeBaseTool(args);
      case "crystal_list_knowledge_base_memories":
        return handleListKnowledgeBaseMemoriesTool(args);
      case "crystal_research_create_collection":
        return handleResearchCreateCollection(args);
      case "crystal_research_ingest":
        return handleResearchIngest(args);
      case "crystal_research_status":
        return handleResearchStatus(args);
      case "crystal_research_query":
        return handleResearchQuery(args);
      case "crystal_research_trace":
        return handleResearchTrace(args);
      case "crystal_research_promote":
        return handleResearchPromote(args);
      default:
        return {
          isError: true,
    ...
    return redactToolErrorResult(result);
  });
