...
export function resolveBackendContextBearerToken(requestBearerToken?: string | null, hasTransportToken = Boolean(process.env.MC_MCP_TOKEN)) {
  return hasTransportToken ? process.env.MEMORY_CRYSTAL_API_KEY ?? null : requestBearerToken ?? null;
}
...
async function runHttp() {
  ...
  const legacyApiKeyConfigured = !process.env.MEMORY_CRYSTAL_API_KEY && Boolean(process.env.CRYSTAL_API_KEY);
  ...
  const mcpToken =
    ...
    (legacyApiKeyConfigured ? "__memory_crystal_upgrade_required__" : undefined);
    ...
    if (req.method === "POST" && reqUrl.pathname === "/mcp") {
      try {
        if (!isAuthorizedMcpRequest(req, mcpToken)) {
          ...
          res.end(JSON.stringify({ error: unauthorizedMessage(legacyApiKeyConfigured) }));
          return;
        }
