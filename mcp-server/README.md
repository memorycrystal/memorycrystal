...
Knowledge-base access uses an allowlist with three states: omitted `agentIds` is open to the account, `agentIds: []` is closed, and a non-empty list restricts search to those agents. `crystal_set_knowledge_base_access` supports `add`, `remove`, `set`, and `open`; removing the final agent closes the KB instead of reopening it.
