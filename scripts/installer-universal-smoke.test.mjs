...
assert.doesNotMatch(installer, /\bconfigure_(claude_code|codex|codex_desktop|hermes|openclaw|factory_droid)\b/, "no per-target configure_* remain");
...
// neither exists — a green run must never mean "we did not execute".
{
  const psTargets = [
    ...
    "claude-code", "claude-desktop", "factory-droid", "openclaw",
  ];
