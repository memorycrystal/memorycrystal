...
const reportPath = value("--report", join(process.env.MEMORY_CRYSTAL_HOME || join(homedir(), ".memorycrystal"), "reports", `research-scale-${Date.now()}.json`));
...
const artifactExpanded = new TextEncoder().encode("representative compressed artifact fixture\n".repeat(20));
const artifactPayload = gzipSync(artifactExpanded);
for (let index = 0; index < artifactCount; index += 1) {
  ...
  const upload = await post("/api/research/artifacts/upload-url", {
    collectionId, expectedBytes: artifactPayload.byteLength, stableKey, idempotencyKey: stableKey,
    artifactType: "compressed_report", checksum: hash(artifactPayload), mimeType: "application/gzip",
    uncompressedBytes: artifactExpanded.byteLength, compressionType: "gzip",
  });
  ...
  await post("/api/research/artifacts/register", {
    artifactId: upload.artifactId, storageId,
  });
}
...
try { dockerMemoryBytes = Number(execFileSync("docker", ["info", "--format", "{{.MemTotal}}"], { encoding: "utf8" }).trim()) || null; } catch {}
const report = {
  schemaVersion: 2,
  createdAt: new Date().toISOString(),
  environment: { node: process.version, hostMemoryBytes: totalmem(), dockerMemoryBytes, apiUrl },
  collectionId,
  ...
  durableIngestionProtocolVerifiedSeparately: true,
  gates: {
    experimentThroughput: { thresholdRowsPerSecond: 500, actualRowsPerSecond: primary.rowsPerSecond, passed: primary.rowsPerSecond >= 500 },
    indexedQuery: { thresholdP95Ms: 750, actualP95Ms: queryP95Ms, passed: queryP95Ms < 750 },
    semanticQuery: { thresholdP95Ms: 1500, actualP95Ms: semanticP95Ms, passed: semanticP95Ms < 1500 },
  },
};
report.passed = Object.values(report.gates).every((gate) => gate.passed);
