...
function convexRun(functionName: string, args: Record<string, unknown>): string {
  ...
  const result = spawnSync("npx", ["convex", "run", functionName, JSON.stringify(args), ...envFileArgs], {
    cwd: repoRoot,
    env: convexEnv,
    encoding: "utf8",
...
function main(): void {
  ...
  const result = convexRun("crystal/localAuth:upsertLocalInstallerApiKey", {
    userId: localAuth.userId,
    keyHash: localAuth.keyHash,
    label: "local installer",
    now: Date.now(),
  });
  if (!result.includes("self_hosted_unmetered")) {
    throw new Error("Local auth import did not confirm self_hosted_unmetered capacity policy");
  }
  console.log(`Imported local installer API key and synchronized self_hosted_unmetered entitlement for ${localAuth.userId}.`);
}
