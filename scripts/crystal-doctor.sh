...
if [ -f "$ENV_FILE" ]; then
  ...
  case "$EMBEDDING_PROVIDER" in
    gemini)
      EFFECTIVE_GEMINI_KEY="${CRYSTAL_API_KEY:-${GEMINI_API_KEY:-}}"
      if [ -z "$EFFECTIVE_GEMINI_KEY" ]; then
        fail "CRYSTAL_API_KEY (or GEMINI_API_KEY) is required when EMBEDDING_PROVIDER=gemini"
      elif [ -n "${CRYSTAL_API_KEY:-}" ]; then
        ok "CRYSTAL_API_KEY set (used for Gemini embeddings)"
      else
  ...
  if [ -z "${CRYSTAL_MCP_MODE:-}" ] || [ -z "${CRYSTAL_MCP_HOST:-}" ] || [ -z "${CRYSTAL_MCP_PORT:-}" ]; then
    warn "CRYSTAL_MCP_* vars not fully configured (MODE/HOST/PORT)."
  else
    ok "CRYSTAL_MCP_MODE=${CRYSTAL_MCP_MODE}, HOST=${CRYSTAL_MCP_HOST}, PORT=${CRYSTAL_MCP_PORT}"
  fi
...
if not isinstance(entry, dict):
    ...
    if isinstance(config, dict):
        if config.get("apiKey"):
            ok("crystal-memory config has apiKey")
        else:
            warn("crystal-memory config missing apiKey (remote API calls will fail)")
...
PY
    ...
    info "MCP mode is '${MCP_MODE}' (not SSE), skipping HTTP health probe."
  fi
