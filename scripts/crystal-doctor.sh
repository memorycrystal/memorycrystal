    ...
    gemini)
      if [ -z "${GEMINI_API_KEY:-}" ]; then
        if [ "$SMOKE" == "true" ] || [ "$DRY_RUN" == "true" ]; then
          warn "GEMINI_API_KEY is not set for local Gemini embeddings; doctor will continue because smoke/dry-run verification does not require a local embedding key."
        else
          fail "GEMINI_API_KEY is required when EMBEDDING_PROVIDER=gemini"
        fi
      else
...
if isinstance(slots, dict) and slots.get("contextEngine") == "crystal-memory":
    warn("plugins.slots.contextEngine is still set to 'crystal-memory'. Remove it to restore OpenClaw's default context engine.")
elif isinstance(slots, dict) and slots.get("contextEngine"):
    ok(f"plugins.slots.contextEngine preserved as {slots.get('contextEngine')!r}")
...
if isinstance(allow, list) and "crystal_status" in allow:
    warn("plugins.allow includes stale tool entry crystal_status; remove it because OpenClaw allowlists plugin IDs, not tool names")
if isinstance(allow, list) and "crystal_doctor" in allow:
    warn("plugins.allow includes stale tool entry crystal_doctor; remove it because OpenClaw allowlists plugin IDs, not tool names")
...
if [ "${LIVE:-false}" == "true" ]; then
  ...
  MCP_PROBE_TOKEN="${MC_MCP_TOKEN:-${MEMORY_CRYSTAL_API_KEY:-}}"
...
if [ "${SMOKE:-false}" == "true" ]; then
  ...
  if [ -f "$PLUGIN_PATH/index.js" ]; then
    if node -e "const m = require('$PLUGIN_PATH/index.js'); console.log('  [ok]   Plugin exports:', typeof m === 'function' ? 'function' : typeof m); const metrics = m.__runtimeMetrics; if (metrics && metrics.callbacks) { for (const [name, value] of Object.entries(metrics.callbacks)) console.log('  [info] callback ' + name + ' count=' + (value && typeof value.count === 'number' ? value.count : 0)); } else { console.log('  [warn] runtime metrics not registered'); }" 2>/dev/null; then
...
# ── 10. Transcript capture diagnostics ───────────────────────────────────────

echo "  == Transcript capture diagnostics =="
TRANSCRIPT_SCAN_ROOT="${CRYSTAL_TRANSCRIPT_ROOT:-$HOME/.claude/projects}"
if [ ! -d "$TRANSCRIPT_SCAN_ROOT" ]; then
  info "No Claude/Codex transcript root found at $TRANSCRIPT_SCAN_ROOT"
else
  TRANSCRIPT_SCAN_RESULT="$(node - "$TRANSCRIPT_SCAN_ROOT" "${MC_DOCTOR_TRANSCRIPT_LIMIT:-200}" <<'NODE'
const fs = require('node:fs');
const path = require('node:path');

const root = process.argv[2];
const limit = Math.max(1, Number(process.argv[3] || 200));
const files = [];
...
function inspect(file) {
  ...
  for (const line of fs.readFileSync(file, 'utf8').split(/\r?\n/)) {
    ...
    const hook = entry.hookEvent || entry.hook_event_name || entry.hook_event;
    if (hook === 'SessionStart') sessionStart += 1;
    if (hook === 'UserPromptSubmit') userPromptSubmit += 1;
    if (entry.type === 'queue-operation' && entry.operation === 'enqueue' && typeof entry.content === 'string' && entry.content.trim()) queueEnqueue += 1;
    if (entry.type === 'assistant' && entry.message && entry.message.role === 'assistant') assistant += 1;
  }
  return { sessionStart, userPromptSubmit, queueEnqueue, assistant };
}
...
try {
  ...
  let risky = 0;
  let abandoned = 0;
  let queueBackfillable = 0;
  for (const file of recent) {
    const result = inspect(file.full);
    if (result.sessionStart > 0 && result.userPromptSubmit === 0) risky += 1;
    if (result.queueEnqueue > 0 && result.assistant === 0) abandoned += 1;
    if (result.queueEnqueue > 0 && result.assistant > 0) queueBackfillable += 1;
  }
  console.log(JSON.stringify({ ok: true, scanned: recent.length, totalJsonl: files.length, risky, abandoned, queueBackfillable }));
} catch (error) {
  console.log(JSON.stringify({ ok: false, error: error && error.message ? error.message : String(error) }));
}
NODE
)"
  TRANSCRIPT_SCAN_OK="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(data.ok ? 'true' : 'false');")"
  if [ "$TRANSCRIPT_SCAN_OK" != "true" ]; then
    TRANSCRIPT_SCAN_ERROR="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(String(data.error || 'unknown error'));")"
    warn "Transcript scan failed: ${TRANSCRIPT_SCAN_ERROR}"
  else
    TRANSCRIPT_SCANNED="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(String(data.scanned));")"
    TRANSCRIPT_RISKY="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(String(data.risky));")"
    TRANSCRIPT_ABANDONED="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(String(data.abandoned));")"
    TRANSCRIPT_BACKFILLABLE="$(printf '%s' "$TRANSCRIPT_SCAN_RESULT" | node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(String(data.queueBackfillable));")"
    ok "Scanned ${TRANSCRIPT_SCANNED} recent transcript JSONL files"
    if [ "$TRANSCRIPT_RISKY" -gt 0 ]; then
      warn "${TRANSCRIPT_RISKY} recent transcript(s) have SessionStart but no UserPromptSubmit; run scripts/crystal-backfill-from-jsonl.mjs --dry-run to inspect recoverable first turns."
    fi
    if [ "$TRANSCRIPT_BACKFILLABLE" -gt 0 ]; then
      info "${TRANSCRIPT_BACKFILLABLE} queue-operation transcript(s) include assistant text and are backfill candidates if missing from the backend."
    fi
    if [ "$TRANSCRIPT_ABANDONED" -gt 0 ]; then
      info "${TRANSCRIPT_ABANDONED} queue-operation transcript(s) appear abandoned before assistant text."
    fi
  fi
fi
echo ""

# ── Host compatibility (advisory only) ───────────────────────────────────────
# ADVISORY: this section only ever emits [warn]/[info]. It must NEVER call fail()
# or otherwise block, because a newer/older host is not an install error. Tested
# host versions as of 2026-07-01; keep in lockstep with apps/web/public/install.sh
# and the root package.json "testedWith" block.

echo "  == Host compatibility (advisory) =="

TESTED_OPENCLAW="2026.6.1"
TESTED_CLAUDE_CODE="2.1.197"
TESTED_CODEX="0.142.5"
TESTED_FACTORY="0.162.0"
TESTED_HERMES="v0.14.0"

# Extract the first version-looking token from a command's --version output.
host_version() {
  local bin="$1"; shift
  command -v "$bin" >/dev/null 2>&1 || return 1
  "$bin" "$@" 2>/dev/null | grep -oE 'v?[0-9]+(\.[0-9]+)+' | head -1
}

# Advisory compare: warn only when detected differs from tested. We do not try to
# decide "older vs newer" (host schemes differ); a mismatch is a soft heads-up.
tested_with_check() {
  local label="$1" detected="$2" tested="$3"
  if [ -z "$detected" ]; then
    info "$label version not detected; skipping compatibility check (tested against $tested)."
    return 0
  fi
  if [ "$detected" = "$tested" ] || [ "$detected" = "v$tested" ] || [ "v$detected" = "$tested" ]; then
    ok "$label $detected matches tested version."
  else
    warn "$label $detected is outside the tested version ($tested). Memory Crystal is advisory-compatible; this does not block anything."
  fi
}

if command -v openclaw >/dev/null 2>&1; then
  tested_with_check "OpenClaw" "$(host_version openclaw --version || true)" "$TESTED_OPENCLAW"
fi
if command -v claude >/dev/null 2>&1; then
  tested_with_check "Claude Code" "$(host_version claude --version || true)" "$TESTED_CLAUDE_CODE"
fi
if command -v codex >/dev/null 2>&1; then
  tested_with_check "Codex" "$(host_version codex --version || true)" "$TESTED_CODEX"
fi
if command -v droid >/dev/null 2>&1; then
  tested_with_check "Factory (droid)" "$(host_version droid --version || true)" "$TESTED_FACTORY"
fi
if command -v hermes >/dev/null 2>&1; then
  tested_with_check "Hermes" "$(host_version hermes --version || true)" "$TESTED_HERMES"
fi
if ! command -v openclaw >/dev/null 2>&1 && ! command -v claude >/dev/null 2>&1 \
   && ! command -v codex >/dev/null 2>&1 && ! command -v droid >/dev/null 2>&1 \
   && ! command -v hermes >/dev/null 2>&1; then
  info "No supported host CLIs detected on PATH; skipped host compatibility checks."
fi
echo ""
