...
test("assembleContext injects relevant summaries before recency context", async () => {
  ...
  assert.ok(result[0].content.includes('id="sum_rel1"'), "should include source receipt id");
  assert.ok(result[0].content.includes('source_messages="0"'), "should include source receipt count");
});

test("assembleContext includes source receipts for context summaries", async () => {
  const result = await assembleContext(summaryAndTailStore(), "sess", 8000, 2);
  const summaryMsg = result.find((m) => String(m.content).includes("<crystal_summary"));
  assert.ok(summaryMsg, "summary should be included");
  assert.ok(summaryMsg.content.includes('id="sum_abc"'));
  assert.ok(summaryMsg.content.includes('kind="leaf"'));
  assert.ok(summaryMsg.content.includes('depth="0"'));
  assert.ok(summaryMsg.content.includes('source_messages="0"'));
});
