...
# Pulls latest plugin files from the `release` branch and installs them.
# Usage:
...
# Public mirror — no auth required. Falls back to private repo if GITHUB_TOKEN is set.
if [[ -z "${GITHUB_TOKEN:-}" && -z "${GH_TOKEN:-}" ]] && ! (command -v gh &>/dev/null && gh auth token &>/dev/null 2>&1); then
  REPO="memorycrystal/memorycrystal"
  BRANCH="main"
else
  REPO="illumin8ca/memorycrystal"
  BRANCH="release"
fi
...
# Fetch checksums.txt from the release branch (optional — backward compatible)
