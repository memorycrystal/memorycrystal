...
function getScopedChannelPolicy(ctx, event, fallbackScopeOrConfig) {
  ...
  // introduced by 3ad22a7 (Prevent peer-scoped memory bleed for Morrow).
...
function resolveReadChannelKey(ctx, event, fallbackScope, explicitChannelParam) {
  ...
  // Explicit peer policies (e.g. Morrow coach with mode:peer in
...
async function captureTurn(api, event, ctx, userMessage, assistantText) {
  ...
  const config = getPluginConfig(api, ctx);
  const channel = resolveAutomaticWriteChannel(ctx, event, config);
  if (!canUseWriteChannel(channel)) return;
  await request(config, "POST", "/api/mcp/capture", {
    title: `OpenClaw — ${new Date().toISOString().slice(0, 16).replace("T", " ")}`,
    content: [safeUserMessage ? `User: ${safeUserMessage}` : null, `Assistant: ${assistantText}`].filter(Boolean).join("\n\n"),
      ...
      async ingestBatch(payload, ctx) {
        try {
          ...
          const flushed = mode === "full"
            ? (canUseWriteChannel(channelKey) ? await flushContextEngineMessages(api, ctx, sessionKey, { sessionKey, channel: channelKey }) : { flushed: 0, skipped: true })
            : { flushed: 0 };
          if (store && sessionKey) {
            const localContextKey = resolveLocalContextKey(
              sessionKey,
