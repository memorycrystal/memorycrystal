...
async function buildCrystalStatusReport(api, ctx) {
  ...
  lines.push(
    `Callbacks: before_agent_start=${runtimeMetrics.callbacks.beforeAgentStart.count}, message_received=${runtimeMetrics.callbacks.messageReceived.count}, llm_output=${runtimeMetrics.callbacks.llmOutput.count}, assemble=${runtimeMetrics.callbacks.assemble.count}, afterTurn=${runtimeMetrics.callbacks.afterTurn.count}`,
  );
...
function getDispatchGapDiagnosis() {
  ...
  return {
    ...
    detail:
      `message_received fired ${messageReceivedCount}×, but before_agent_start, llm_output, message_sending, assemble, and afterTurn are all 0. ` +
...
async function logAssistantMessageOnce(api, ctx, sessionKey, assistantText, channel) {
  ...
  const result = await logMessage(api, ctx, {
    role: "assistant",
...
module.exports = (api) => {
  ...
  const hook = typeof api.registerHook === "function"
    ? api.registerHook.bind(api)
    : (typeof api.on === "function" ? api.on.bind(api) : null);
  ...
  hook("llm_output", async (event, ctx) => {
    try {
      ...
      if (canUseWriteChannel(channelKey)) await logAssistantMessageOnce(api, ctx, sessionKey, assistantText, channelKey);
      const store = await getLocalStore(config, api.logger);
  ...
  hook("message_sending", async (event, ctx) => {
    try {
      ...
      if (!canUseWriteChannel(channelKey)) return;
      await logAssistantMessageOnce(api, captureCtx, sessionKey, assistantText, channelKey);
  ...
  hook("message_sent", async (event, ctx) => {
    try {
      ...
      const userMessage = pendingUserMessages.get(sessionKey) || "";
      if (canUseWriteChannel(channelKey)) await logAssistantMessageOnce(api, captureCtx, sessionKey, assistantText, channelKey);
      const store = await getLocalStore(config, api.logger);
      if (store && localContextKey) { store.addMessage(localContextKey, "assistant", assistantText); _registerLocalTools(api); }
      appendConversationPulseMessage(sessionKey, "assistant", assistantText);
      pendingUserMessages.delete(sessionKey);
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        let data = filterChannelScopedPayload(
          await crystalRequest(cfg, "/api/mcp/search-messages", withAgentId({ query, limit, sinceMs, ...(resolvedChannel ? { channel: resolvedChannel } : {}) }, ctx, ctx)),
          resolvedChannel,
        );
        ...
        if (!messages.length && typeof params?.channel !== "string" && resolvedChannel && allowGlobalFallback) {
          data = await crystalRequest(cfg, "/api/mcp/search-messages", withAgentId({ query, limit, sinceMs }, ctx, ctx));
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const cfg = getPluginConfig(api, ctx);
        const resolvedChannel = resolveReadChannelKey(ctx, ctx, cfg);
        ...
        const data = await crystalRequest(cfg, "/api/mcp/memory", { memoryId, ...(resolvedChannel ? { channel: resolvedChannel } : {}) });
        const m = data?.memory;
  ...
  registerTool({
    ...
    description: "List Memory Crystal knowledge bases visible to this account and optional channel scope.",
    parameters: {
      type: "object",
      properties: {
  ...
  registerTool({
    ...
    description: "Import text chunks into a Memory Crystal knowledge base, creating it by name when needed.",
    parameters: {
      type: "object",
      properties: {
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/update", payload);
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  const registerSupersedeTool = (name) => registerTool({
    name, label: name === "crystal_supercede" ? "Crystal Supercede" : "Crystal Supersede",
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/supersede", payload);
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  registerSupersedeTool("crystal_supersede");
  registerSupersedeTool("crystal_supercede");
  registerTool({
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/forget", { memoryId, permanent, ...(resolvedChannel ? { channel: resolvedChannel } : {}) });
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/trace", { memoryId, ...(resolvedChannel ? { channel: resolvedChannel } : {}) });
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/recall", withAgentId({ query: `who owns ${topic}`, categories: ["person"], limit, mode: "people", ...(resolvedChannel ? { channel: resolvedChannel } : {}) }, ctx, ctx));
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/recall", withAgentId({ query: `connection between ${from} and ${to}`, limit, ...(resolvedChannel ? { channel: resolvedChannel } : {}) }, ctx, ctx));
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(cfg, "/api/mcp/recall", withAgentId({ query: `dependencies for ${topic}`, limit, mode: "project", ...(resolvedChannel ? { channel: resolvedChannel } : {}) }, ctx, ctx));
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
