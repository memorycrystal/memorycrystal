...
function computeAgentEndTurnId(sessionKey, userMessage, assistantText, event, ctx) {
  const explicit = firstString(event?.runId, ctx?.runId, event?.turnId, ctx?.turnId, event?.id, ctx?.id);
  if (explicit) return explicit;
...
async function captureAgentEndTurn(api, event, ctx) {
  ...
  if (hadPendingUserMessage) {
    ...
    const result = await logCompletedTurn(api, captureCtx, {
      sessionKey,
      channel: channelKey,
      turnId: computeAgentEndTurnId(sessionKey, userMessage, assistantText, event, ctx),
      userMessage,
