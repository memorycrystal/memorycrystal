...
function hasRecentlyWrittenAssist(sessionKey, text, now = Date.now()) {
  ...
  const key = getAssistDedupeKey(sessionKey, text);
  const existing = recentlyWrittenAssist.get(key);
  if (!existing) return false;
  recentlyWrittenAssist.delete(key);
  recentlyWrittenAssist.set(key, { hash: existing.hash, ts: now });
  return true;
}
...
function noteAssistWrite(sessionKey, text, now = Date.now()) {
  ...
  recentlyWrittenAssist.delete(key);
  recentlyWrittenAssist.set(key, { hash, ts: now });
  pruneAssistDedupe(now);
}
...
function getDispatchGapDiagnosis() {
  ...
  return {
    ...
    detail:
      `message_received fired ${messageReceivedCount}×, but before_agent_start, llm_output, assemble, and afterTurn are all 0. ` +
    ...
    fallback:
      messageSentCount > 0
        ? `message_sent fired ${messageSentCount}×, so assistant-capture fallback may still be available.`
        : "message_sent has not fired either; automatic assistant capture cannot be guaranteed from plugin runtime alone.",
  };
}
...
module.exports = (api) => {
  ...
  hook("message_received", async (event, ctx) => {
    try {
      ...
      if (text && sessionKey) await emitCaptureStalledMetric(api, ctx, sessionKey, channelKey);
  ...
  hook("llm_output", async (event, ctx) => {
    try {
      ...
      await logAssistantMessageOnce(api, ctx, sessionKey, assistantText, channelKey);
      const store = await getLocalStore(config, api.logger);
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        let data = await crystalRequest(getPluginConfig(api, ctx), "/api/mcp/search-messages", withAgentId({ query, limit, sinceMs, channel: resolvedChannel }, ctx, ctx));
        let messages = Array.isArray(data?.messages) ? data.messages : [];
        let scope = resolvedChannel ? "channel" : "global";
        if (!messages.length && typeof params?.channel !== "string" && resolvedChannel) {
          data = await crystalRequest(getPluginConfig(api, ctx), "/api/mcp/search-messages", withAgentId({ query, limit, sinceMs }, ctx, ctx));
  ...
  registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(getPluginConfig(api, ctx), "/api/mcp/update", payload);
  ...
  const registerSupersedeTool = (name) => registerTool({
    ...
    async execute(_id, params, _sig, _upd, ctx) {
      try {
        ...
        const data = await crystalRequest(getPluginConfig(api, ctx), "/api/mcp/supersede", payload);
        return toToolResult(data);
      } catch (err) { return toToolError(err); }
    },
  });
