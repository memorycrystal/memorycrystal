  ...
  return {
    ...
    registerContextEngine(nameOrEngine, factory) {
      if (typeof factory === "function") {
        contextEngineId = nameOrEngine;
        contextEngine = factory();
      } else {
        contextEngineId = nameOrEngine?.info?.id || null;
...
describe("crystal-memory plugin — Phase 2 integration", () => {
  test("1. Plugin loads and registers context engine in reduced mode by default", () => {
    ...
    assert.equal(api._getEngineId(), "crystal-memory");
    assert.equal(engine.info.id, "crystal-memory");
    ...
    assert.equal(typeof engine.ingestBatch, "function");
  ...
  test("2a. ingest hook accepts a single OpenClaw message without relying on this binding", async () => {
    ...
    try {
      const api = loadPlugin({ contextEngineMode: "full", localStoreEnabled: true, dbPath });
      const engine = api._getEngine();
      const ingest = engine.ingest;

      await ingest({
        sessionKey: "single-ingest-session",
        message: { role: "user", content: "Single-message ingest contract works" },
      }, makeCtx({ sessionKey: "single-ingest-session" }));
      ...
      try {
        const row = db.prepare(`
          ...
          WHERE c.session_key = ?
        `).get("single-ingest-session");
        assert.deepEqual(row, {
          role: "user",
          content: "Single-message ingest contract works",
        });
      } finally {
        db.close();
      }
  ...
  test("3. assemble hook: returns messages array (with or without system prepend)", async () => {
    ...
    assert.ok(typeof result.estimatedTokens === "number", "result.estimatedTokens is number");
  ...
  describe("channelScope for recall tools", () => {
    ...
    test("automatic log and capture writes derive only opted-in OpenClaw direct session peers", async () => {
      ...
      assert.equal(captureBodies.length, 0, "ordinary turns should not create sensory capture memories");
    ...
    test("message_sending captures assistant replies when llm_output and message_sent are not dispatched", async () => {
      ...
      assert.equal(captureBodies.length, 0, "assistant reply fallback should not create sensory capture memories");
