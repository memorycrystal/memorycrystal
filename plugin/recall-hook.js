...
const searchMemories = async ({ embedding, query, mode, channel, sessionKey, env }) => {
  ...
  // are available. Fall back to CONVEX_URL because many live installs already
  // provide that, and the HTTP MCP routes are served from the same .convex.site base.
  const crystalSite = (env.CRYSTAL_SITE || env.CONVEX_URL || "").replace(/\/+$/, "");
