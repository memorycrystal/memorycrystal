...
// block. Env-configurable via MEMORY_CRYSTAL_MAX_MEMORIES (default 10, hard-capped
...
// Env-configurable via MEMORY_CRYSTAL_PREVIEW_CHARS (default 750). A sane ceiling
// of 2000 chars prevents a misconfiguration from flooding every turn.
const DEFAULT_MEMORY_PREVIEW_CHARS = 750;
...
const buildRecallInjectionBlock = (recentBlock, memories, degradation, env = process.env, config = {}) => {
...
const main = async () => {
  ...
  const injectionBlock = buildRecallInjectionBlock(recentBlock, memories, recallResult.degradation, env, config);
