...
test("resolveMaxMemories defaults to 10 and honors env override capped at 20", () => {
  assert.equal(__test__.resolveMaxMemories({}), 10);
  ...
  assert.equal(__test__.resolveMaxMemories({ MEMORY_CRYSTAL_MAX_MEMORIES: "abc" }), 10);
  assert.equal(__test__.resolveMaxMemories({ MEMORY_CRYSTAL_MAX_MEMORIES: "0" }), 10);
  assert.equal(__test__.resolveMaxMemories({ MEMORY_CRYSTAL_MAX_MEMORIES: "-3" }), 10);
});

test("resolvePreviewChars defaults to 750 and honors env override capped at 2000", () => {
  assert.equal(__test__.resolvePreviewChars({}), 750);
  ...
  assert.equal(__test__.resolvePreviewChars({ MEMORY_CRYSTAL_PREVIEW_CHARS: "nope" }), 750);
  assert.equal(__test__.resolvePreviewChars({ MEMORY_CRYSTAL_PREVIEW_CHARS: "0" }), 750);
});
...
test("resolveMaxMemories honors a plugin-config value, capped, with env taking precedence", () => {
  ...
  assert.equal(__test__.resolveMaxMemories({}, 0), 10);
  assert.equal(__test__.resolveMaxMemories({}, undefined), 10);
});
...
test("resolvePreviewChars honors a plugin-config value, capped, with env taking precedence", () => {
  ...
  assert.equal(__test__.resolvePreviewChars({}, 0), 750);
  assert.equal(__test__.resolvePreviewChars({}, undefined), 750);
});
...
test("formatBlock surfaces up to maxMemories and honors previewChars cap", () => {
  ...
  // Default caps: 10 surfaced, 750-char preview.
  const defaultBlock = __test__.formatBlock(memories);
  assert.equal((defaultBlock.match(/### SEMANTIC:/g) || []).length, 10);
  assert.match(defaultBlock, /_\+4 additional memories/);
  const firstContentLine = defaultBlock.split("\n").find((l) => l.startsWith("x"));
  assert.equal(firstContentLine.length, 751); // 750 chars + ellipsis
