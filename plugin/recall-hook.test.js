...
test("recall hook preserves compact graph context in the injection block", () => {
  const block = __test__.buildRecallInjectionBlock(
    "",
    [],
    undefined,
    ...
    {},
    {
      byMemoryId: {
        memory1: {
          entities: [{ label: "Memory Crystal", nodeType: "project" }],
        },
      },
      relations: [
        {
          fromLabel: "Memory Crystal",
          toLabel: "Railway",
          relationType: "uses",
        },
      ],
  ...
  assert.match(block, /Connected Graph Context/);
  assert.match(block, /Memory Crystal --uses--> Railway/);
  assert.match(block, /Entities: Memory Crystal/);
});
