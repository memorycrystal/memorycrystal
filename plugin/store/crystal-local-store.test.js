...
describe('CrystalLocalStore', () => {
  ...
  test('init() creates DB file and all schema tables', () => {
    ...
    try {
      ...
      for (const tbl of ['conversations', 'messages', 'summaries', 'summary_parents', 'summary_messages', 'context_items', 'lesson_counter', 'context_engine_ops', 'context_engine_debt']) {
  ...
  test('getHealthSnapshot() reports healthy opened schema', () => {
    const dbPath = makeTmpDb();
    try {
      const store = openStore(dbPath);
      const health = store.getHealthSnapshot();
      assert.equal(health.healthy, true);
      assert.equal(health.dbOpen, true);
      assert.equal(health.schemaReady, true);
      assert.equal(health.writable, true);
      assert.equal(typeof health.fts5Available, 'boolean');
  ...
  test('getHealthSnapshot() reports unopened store without throwing', () => {
    const store = new CrystalLocalStore();
    const health = store.getHealthSnapshot();
    assert.equal(health.healthy, false);
    assert.equal(health.dbOpen, false);
    assert.equal(health.schemaReady, false);
    assert.equal(health.writable, false);
    assert.match(health.lastError, /not open/);
  });

  test('context engine operation queue retries and dedupes by metadata', () => {
    const dbPath = makeTmpDb();
    try {
      const store = openStore(dbPath);
      const id1 = store.enqueueContextEngineOp({
        sessionKey: 'sess:queue',
        role: 'user',
        content: 'queue me once',
        sourceCallback: 'ingestBatch',
        turnId: 'turn-1',
        messageIndex: 0,
      });
      const id2 = store.enqueueContextEngineOp({
        sessionKey: 'sess:queue',
        role: 'user',
        content: 'queue me once',
        sourceCallback: 'ingestBatch',
      ...
      assert.equal(store.getPendingContextEngineOps('sess:queue').length, 1);
      store.markContextEngineOpFailed(id1, 'HTTP 500');
      assert.equal(store.getContextEngineQueueStats().failed, 1);
      assert.equal(store.getPendingContextEngineOps('sess:queue')[0].attempts, 1);
      store.markContextEngineOpFlushed(id1);
      assert.equal(store.getPendingContextEngineOps('sess:queue').length, 0);
      store.close();
    } finally {
      rmSync(dbPath, { force: true });
    }
  ...
  test('compaction debt stats track open and cleared debt', () => {
    const dbPath = makeTmpDb();
    try {
      const store = openStore(dbPath);
      const id = store.recordCompactionDebt('sess:debt', 'assemble_trim', {
        estimatedChars: 400,
        estimatedTokens: 100,
      });
      let stats = store.getCompactionDebtStats();
      assert.equal(stats.open, 1);
      assert.equal(stats.estimatedTokens, 100);
      store.clearCompactionDebt(id);
      stats = store.getCompactionDebtStats();
  ...
  test('searchMessages() treats LIKE wildcards as literal text', () => {
    const dbPath = makeTmpDb();
    try {
      const store = openStore(dbPath);
      const key = 'sess:search-literal';
      store.addMessage(key, 'user', 'ordinary memory without wildcard symbols');
      store.addMessage(key, 'assistant', 'literal 100% complete milestone');
      store.addMessage(key, 'user', 'literal use_of_underscores marker');

      const percentResults = store.searchMessages(key, '%');
      assert.equal(percentResults.length, 1, 'percent should not match every message');
      assert.equal(percentResults[0].content, 'literal 100% complete milestone');

      const underscoreResults = store.searchMessages(key, '_');
      assert.equal(underscoreResults.length, 1, 'underscore should not match every message');
      assert.equal(underscoreResults[0].content, 'literal use_of_underscores marker');
      store.close();
    } finally {
      rmSync(dbPath, { force: true });
    }
  ...
  test('insertSummary() inserts a summary row verifiable via getSummary()', () => {
    ...
    try {
      ...
      const matches = store.searchSummaries(key, 'inserted');
      assert.equal(matches.length, 1, 'plain substring search should still find summaries');
      assert.equal(matches[0].content, 'inserted summary');
      store.close();
    } finally { rmSync(dbPath, { force: true }); }
  });

  test('searchSummaries() treats LIKE wildcards as literal text', () => {
    const dbPath = makeTmpDb();
    try {
      const store = openStore(dbPath);
      const key = 'sess:summary-literal';
      store.insertSummary({ summaryId: `sum_plain_${Date.now()}`, sessionKey: key, kind: 'leaf', depth: 0, content: 'plain summary without wildcard symbols', tokenCount: 5 });
      store.insertSummary({ summaryId: `sum_percent_${Date.now()}`, sessionKey: key, kind: 'leaf', depth: 0, content: 'summary is 100% verified', tokenCount: 4 });
      store.insertSummary({ summaryId: `sum_under_${Date.now()}`, sessionKey: key, kind: 'leaf', depth: 0, content: 'summary_has_literal_underscores', tokenCount: 3 });

      const percentResults = store.searchSummaries(key, '%');
      assert.equal(percentResults.length, 1, 'percent should not match every summary');
      assert.equal(percentResults[0].content, 'summary is 100% verified');

      const underscoreResults = store.searchSummaries(key, '_');
      assert.equal(underscoreResults.length, 1, 'underscore should not match every summary');
      assert.equal(underscoreResults[0].content, 'summary_has_literal_underscores');
      store.close();
    } finally { rmSync(dbPath, { force: true }); }
  });
