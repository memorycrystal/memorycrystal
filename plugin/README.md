...
- knowledge-base lookup stays separate from broad recall: use `crystal_query_knowledge_base` for named manuals, voice guides, transcripts, and curated reference corpora
- graph-backed recall uses source roles such as `voice_style`, `persona_guardrail`, `canonical_reference`, and `client_context` to keep direct reference material ahead of unrelated artifacts
- coding sessions should pass `projectId` or `repoSlug` through recall-style tools when available so repo-specific memories stay isolated from unrelated project stores
...
- Knowledge bases can expose `sourceRole` metadata such as `voice_style`, `persona_guardrail`, `canonical_reference`, or `client_context`; recall uses this as a ranking signal, and direct KB query returns it as provenance.
...
Use direct KB query when a user names a specific KB/source. Use broad recall when the user needs blended memory context.
...
- `crystal_set_knowledge_base_access` — open, close, or assign agents to a knowledge base
