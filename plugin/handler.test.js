...
// ILL-281: handler tests need inject recall enabled (default is false)
...
const withStubbedSpawnSync = async (fn) => {
  ...
  const savedEnv = {};
  process.env.MEMORY_CRYSTAL_INJECT_RECALL = "true";
...
test("OpenClaw recall spawnSync waits the 16s auto-recall budget (ILL-272)", async () => {
  await withStubbedSpawnSync(async (calls) => {
    const result = await handler.before_model_resolve({ query: "what did we decide?" });

    assert.equal(result.ok, true);
    const call = recallCallOf(calls);
    const timeoutMs = call.options.timeout;

    assert.equal(
      handler.RECALL_HOOK_TIMEOUT_MS,
      16_000,
      "exported OpenClaw recall spawn timeout must stay 16000ms",
    );
    assert.equal(
      timeoutMs,
      handler.RECALL_HOOK_TIMEOUT_MS,
      "runRecallHook must pass RECALL_HOOK_TIMEOUT_MS to spawnSync",
    );
    assert.ok(
      timeoutMs >= 16_000,
      `OpenClaw recall spawnSync timeout ${timeoutMs}ms aborts before the 16s auto-recall default`,
    );
    assert.ok(
      timeoutMs > 10_000,
      "OpenClaw recall must not still SIGKILL at the pre-ILL-272 10s spawnSync default",
    );
  });
});
