...
// ILL-272: same 16s auto-recall budget as Hermes AUTO_RECALL and shared/cursor hooks.
// spawnSync must not SIGKILL the child before MEMORY_CRYSTAL_AUTO_RECALL_TIMEOUT.
const RECALL_HOOK_TIMEOUT_MS = 16_000;

/**
 * Check whether automatic recall injection is enabled for this OpenClaw host.
 * Default is false. Reads MEMORY_CRYSTAL_INJECT_RECALL env var.
 */
const injectRecallEnabled = () => {
...
// Pull the recall-shaping plugin-config values off the payload. OpenClaw passes
// plugin config either on `payload.config` or `payload.pluginConfig` depending on
// host version; we accept both. Only the numeric recall knobs are bridged.
const resolveRecallConfig = (payload = {}) => {
  const config =
    (payload.config && typeof payload.config === "object" && payload.config) ||
    (payload.pluginConfig && typeof payload.pluginConfig === "object" && payload.pluginConfig) ||
    {};
  const out = {};
  if (config.maxMemories !== undefined) out.maxMemories = config.maxMemories;
  if (config.previewChars !== undefined) out.previewChars = config.previewChars;
  return out;
};
    ...
    typeof payload.query === "string"
      ...
      : typeof payload.userMessage === "string"
        ...
        : typeof payload.message === "string"
          ? payload.message
          : "";
  // Bridge OpenClaw plugin config (maxMemories / previewChars) to the recall
  // child. recall-hook.js only ever read MEMORY_CRYSTAL_* env vars, so configuring
  // these the documented way (plugin config) previously had no effect. We forward
  // them both on the stdin payload (honored directly by the resolvers) and as env
  // vars (for any code path that still reads process.env), without clobbering an
  // explicit MEMORY_CRYSTAL_* env override the operator may have set.
  const recallConfig = resolveRecallConfig(payload);
  const childEnv = { ...process.env };
  if (recallConfig.maxMemories !== undefined && childEnv.MEMORY_CRYSTAL_MAX_MEMORIES === undefined) {
    childEnv.MEMORY_CRYSTAL_MAX_MEMORIES = String(recallConfig.maxMemories);
  }
  if (recallConfig.previewChars !== undefined && childEnv.MEMORY_CRYSTAL_PREVIEW_CHARS === undefined) {
    childEnv.MEMORY_CRYSTAL_PREVIEW_CHARS = String(recallConfig.previewChars);
  }
  // 16s timeout — matches Hermes AUTO_RECALL / shared hook clients. A hung
  // recall child still SIGKILLs so the gateway cannot freeze, but we no longer
  // abort before the 16s auto-recall default and drop a late 200 as empty.
...
const runHook = async (hookName, payload = {}) => {
  ...
  if (recallHooks.has(hookName)) {
    if (!injectRecallEnabled()) {
      return { ok: true, message: "Recall injection disabled (MEMORY_CRYSTAL_INJECT_RECALL not true)." };
    }
