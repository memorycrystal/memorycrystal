...
> **Production infrastructure notice — 2026-07-14:** Memory Crystal's hosted
> Convex runtime now uses the stable `https://convex.memorycrystal.ai` endpoint
> on Railway. Current plugin builds automatically heal the retired generated
> Convex Cloud production hostname; standalone clients pinned to that generated
> hostname must update their endpoint.

## [Unreleased]

### Changed — Recall SLO safety net and compositor overlap

- Automatic recall / hook clients now wait **16 seconds** (`MEMORY_CRYSTAL_AUTO_RECALL_TIMEOUT=16`, hook `16000ms`) so a slow backend answer beats a silent 499 skip. Do not raise past 16s. The installer still strips stale 2.5s/8s pins and does not rewrite a lower value. The legacy OpenClaw `plugin/handler.js` recall `spawnSync` uses the same 16s budget (was 10s). Claude/Codex/Factory `UserPromptSubmit` and Cursor `postToolUse` host hook timeouts are also 16s so the host does not kill a recall-calling hook before the fetch budget. Capture-only Cursor events stay at 10s.
- `/api/mcp/recall` and `crystal_recall` add `diagnostics.timings` (ms): `embed`, `vectorSearch`, `lexical`, `kb`, `messages`, `graph`, `compose`, `total`, plus `parallel` when stages overlap. Hosted MCP trim passes the object through.
- Independent recall stages now overlap (lexical/KB list with embed+vector; messages with lexical hydrate; assets with KB search; graph/provenance/bookkeeping together). `includeGraphContext` stays the production default.

## [0.8.22] — 2026-08-04

### Added — Recall quality pipeline, hosted health, and key lifecycle

- **Recall evaluation harness and release gate.** `bench:recall` scores recall@k and MRR over the real recall action against an authored gold set, so ranking changes are measured rather than asserted.
- **Reinforcement on recall.** Surfacing a memory now strengthens it, and decay reconciliation keeps strength and decay from drifting apart.
- **Provenance and staleness inside the recall payload.** Recalled memories carry where they came from and how stale they are, so an agent can weigh a fact instead of trusting it flatly.
- **Contradiction reverse-fade and query-intent recency in ranking.** A contradicted memory fades in favour of its successor, and queries that read as time-sensitive bias toward recent material.
- **Write-time fast-fact vs slow-fact classification.** Volatile facts and durable ones are separated as they are written, which is what lets staleness mean something later.
- **Hygiene worklists.** Stale-memory, graph-lint, and concept-gap worklists turn "memory feels noisy" into a concrete list of ids to act on.
- **Daily-driver UX.** An L0 core tier, future-recall questions, and an overnight brief.
- **Hosted health and recall provenance.** `crystal_health` reports a hygiene worklist grouped by severity with triage context.
- **CLI-mintable Convex API keys with safe rotation.** Keys can be minted and rotated from the CLI without a dashboard round trip.

### Changed — Installer architecture, account identity, and API-key responses

- **The installer is table-driven.** `install.sh`, `install.ps1`, and `uninstall.sh` now read `install-assets/platforms.json` — a capability table describing each platform's config path and format, MCP registration method, hook support, and skills directory — instead of per-target shell functions. Adding a platform is a table row. Behaviour for all nine existing targets is unchanged, verified by golden-output equivalence against the previous installer plus an executed PowerShell pass across every Windows target.
- **One account, multiple sign-in methods.** Password, Google, and GitHub sign-ins unify onto a single identity instead of silently minting a second user row.
- **API-key callers can identify their own account.** `/api/mcp/auth`, the new `/api/mcp/whoami`, `/api/mcp/stats`, and recall diagnostics now return the account email and name alongside the opaque `userId`, so "which account is this key for?" no longer requires a dashboard login.

### Removed

- **`crystal_supercede`.** The misspelled alias of `crystal_supersede` is no longer advertised by the MCP server or the OpenClaw plugin; it occupied a second agent-facing tool slot for nothing. The handler and the `POST /api/mcp/supercede` route still accept the old spelling, so a client holding a cached tool list keeps working.

### Fixed

- **Plugin update integrity.** `plugin/checksums.txt` covers only the files `update.sh` actually downloads; a manifest listing client-mutated files made every client abort with a corruption warning.
- **Convex deploys unblocked.** The bundled eval harness no longer imports `node:fs`, which was failing the deploy bundle.
...
> **Production backend:** Memory Crystal production Convex is self-hosted on Railway at `https://convex.memorycrystal.ai`, not Convex Cloud. Use the production deployment wrappers.
...
- `CONVEX_URL` — Convex deployment URL (production: `https://convex.memorycrystal.ai`, set in mcp-server `.env`)
