...
For coding agents, pass either `projectId` or `repoSlug` whenever the working repository is known. That keeps `cassai-v2` recall from being muddied by unrelated repositories such as `solarwyse`, while still allowing account-wide recall when no project scope is appropriate.
