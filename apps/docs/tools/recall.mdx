...
When `channel` is omitted on a peer-facing call, recall now fails-closed on any knowledge base with a concrete peer scope instead of silently upgrading to management-level visibility. If you're integrating recall from a peer session and expect KB results, always pass the peer channel (e.g. `morrow-coach:511172388`). Omit `channel` only when you intend a deliberately unscoped (management) read.
