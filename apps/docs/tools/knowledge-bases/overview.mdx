...
The universal installer now declares a platform-slug `agentId` at install time and prints reachable-versus-total knowledge bases for that identity. To grant a declared identity access to a restricted KB, add the slug to the KB's agent allow-list in the dashboard (Knowledge → Agents) or run `crystal_set_knowledge_base_access` with `action=add` and that `agentId`. Strict and peer-isolated KBs stay fail-closed; this does not change server-side gate logic.
