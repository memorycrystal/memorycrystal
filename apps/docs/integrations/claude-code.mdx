...
> **Default change (ILL-281):** Automatic recall injection is now off by default on all Full-tier hosts including Claude Code. Set `MEMORY_CRYSTAL_INJECT_RECALL=true` in your environment to restore automatic pre-response recall. Capture still writes. Call `crystal_recall` when you need past context.
