...
Official platforms are Claude Code, Codex, Grok Build, OpenClaw, Hermes Agent, and Buzz. Each publishes a Support Tier from the capability table. See [Support Tiers](/concepts/support-tiers).

The installer can also configure Claude Desktop, Codex Desktop, OpenCode, Factory Droid, Cursor, and generic MCP clients. Those hosts stay installable through `--targets generic-mcp` or their existing ids.

Grok Build is Capture tier: automatic capture and tools, no automatic recall. Buzz is Hosted: install into the CLI Buzz is running. Channel memory is not available.
