...
```text
Install Memory Crystal for the agent I am using. Run the official Memory Crystal installer for this client and operating system, follow the auth/backend prompts, do not manually edit config files unless the installer fails, then verify that Memory Crystal tools are available.
```
...
<Warning>Hermes Agent is not supported by the native PowerShell installer yet. OpenClaw PowerShell writes generic MCP guidance; use Bash, WSL, or Git Bash for the native OpenClaw plugin install. Codex Desktop requires generic MCP/manual setup until it exposes a stable config location.</Warning>
