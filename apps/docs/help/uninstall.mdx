...
Supported universal targets are `openclaw`, `hermes`, `claude-code`, `claude-desktop`, `codex-cli`, `factory-droid`, `cursor`, `grok`, `generic-mcp`, and `local-backend`. Grok Build is Capture tier: uninstall removes `~/.grok/hooks/memory-crystal.json`, the MCP entry, and the Managed Block in `~/.grok/AGENTS.md`.
