...
For local-first installs, uninstalling a client integration and stopping the local backend are separate actions.

1. Run the client-specific uninstall command above for each configured client.
2. If you only want to switch back to cloud mode, restore the installer-created config backup or re-run the installer and choose cloud.
3. Stop the local Docker backend with the installed artifact's down or rollback command:

   ```bash
   ~/.memorycrystal/local-backend/<version>/bin/rollback
   ```
4. Keep the local backend directory until you are sure you do not need its data or rollback metadata.
