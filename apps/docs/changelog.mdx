...
## 2026-08-25 — ILL-281: Tool-mode default recall (product-wide)

**Every inject-capable host** now defaults to tool-mode: `MEMORY_CRYSTAL_INJECT_RECALL` defaults `false`. Cursor, Claude Code, Codex CLI, Codex Desktop, OpenClaw, and Hermes Agent all start with automatic recall injection off. Operators may set the flag `true` to restore automatic wake/recall. Capture is independent and stays on. The asymmetric prompt contract replaces the old "always recall before each response" instructions with deliberate recall when needed and mandatory `crystal_remember` for durable facts.

- **Default change:** `_recall_enabled()` returns `false` unless `MEMORY_CRYSTAL_INJECT_RECALL=true`
- **Shared hooks:** `injectRecallEnabled()` replaces `inject = !grokInvocation` in crystal-hooks.mjs and cursor-hooks.mjs
- **Cursor:** sessionStart and postToolUse gated by the flag
- **OpenClaw:** handler.js skips recall spawn when flag is not true
- **Hermes:** `plugin.yaml` default `"false"`, `_recall_enabled()` default `False`
- **Installer:** `write_hermes_env` strips `MEMORY_CRYSTAL_INJECT_RECALL` without re-pinning it
- **Prompts:** All prompt copies (DISCIPLINE_FOOTER, rules, skills, instructions, managed block) use the asymmetric contract
- **Docs:** ADR 0010, support-tiers include Cursor, Full tier is capability not behavior
## Unreleased
