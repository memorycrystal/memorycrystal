  ...
  for (const id of ids) {
    ...
    if (memory.knowledgeBaseId) {
      ...
      if (
        ...
        // Per-chunk peer isolation (2026-07-02 leak): a channel-bearing KB chunk
        // (per-client content) is only visible to its exact peer, so graph
        // evidence never reveals another peer's private chunk id/provenance.
...
export const getGraphContextForMemoriesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // Only hydrate evidence for the relations that can actually appear in the
    // response (top maxRelations*2 by confidence). The caps below are a runaway
    // guard against the pathological ~680-doc case (16 MB/call in the
    // 2026-07-07 cost audit), lifted high enough that they don't bind on
    // realistic graphs — so a relation is never dropped merely because its
    // visible evidence sat past a tight per-relation limit. M9 (inline-embedding
    // drop) makes evidence-doc hydration cheap and retires this concern.
    const EVIDENCE_IDS_PER_RELATION = 12;
    const MAX_EVIDENCE_IDS_TOTAL = 150;
    const candidateRelations = Array.from(relationMap.values())
      .sort((a, b) => b.confidence - a.confidence)
      .slice(0, maxRelations * 2);
    const evidenceIdSet = new Set<string>();
    for (const relation of candidateRelations) {
      for (const id of relation.evidenceMemoryIds.slice(0, EVIDENCE_IDS_PER_RELATION)) {
        if (evidenceIdSet.size >= MAX_EVIDENCE_IDS_TOTAL) break;
        evidenceIdSet.add(String(id));
      }
      if (evidenceIdSet.size >= MAX_EVIDENCE_IDS_TOTAL) break;
    }
