...
export const whoOwns = action({
  ...
  handler: async (ctx, args) => {
    ...
    const targetNodes = await ctx.runQuery(internal.crystal.graphQuery.findNodesByLabel, {
      userId,
      labelQuery: entity,
...
export const explainConnection = action({
  args: {
    entityA: v.string(),
    entityB: v.string(),
  },
...
export const dependencyChain = action({
  args: {
    entity: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const startNodes = await ctx.runQuery(internal.crystal.graphQuery.findNodesByLabel, {
      userId,
      labelQuery: entity,
      limit: LABEL_SEARCH_LIMIT,
    });
    ...
    const sortedChain = chain
      ...
      .filter((entry, index, values) =>
        values.findIndex((value) => value.depth === entry.depth && value.label === entry.label && value.nodeType === entry.nodeType) ===
        index
      );
