...
export const getGraphContextForMemoriesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // response (top maxRelations*2 by confidence), and cap evidence ids per
    // relation. Unbounded hydration read hundreds of full memory docs (~24 KB
    // inline embedding each) per call — 16 MB/call in the 2026-07-07 cost audit.
    const EVIDENCE_IDS_PER_RELATION = 3;
    const MAX_EVIDENCE_IDS_TOTAL = 60;
