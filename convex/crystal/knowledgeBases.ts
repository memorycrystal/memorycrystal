...
export const purgeKnowledgeBaseBatchInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const memory of memories) {
      ...
      await cascadeDeleteMemory(ctx, memory, { applyDashboardTotals: false });
    }
    if (memories.length < batch) {
      // No more chunks remain — remove the knowledge-base row itself.
      await ctx.db.delete(args.knowledgeBaseId);
      return { deleted: memories.length, activeDeleted, done: true, missing: false, dashboardDelta };
    }
    return { deleted: memories.length, activeDeleted, done: false, missing: false, dashboardDelta };
  },
});
