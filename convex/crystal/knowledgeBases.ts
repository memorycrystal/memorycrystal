...
): Promise<{ emergency: boolean } | null> {
  try {
    ...
    return globalBudget.emergency ? globalBudget : userBudget;
  } catch (error) {
...
const runKnowledgeBaseQuery: any = async (
  ...
  const queryEmbedding = await embedText(args.query.trim(), ctx, {
    userId,
    source: "knowledgeBases.runKnowledgeBaseQuery",
  });

  const kbBudget = args.skipCostBreaker
    ? null
    : await debitKnowledgeBaseQueryCost(ctx, {
        userId,
        ...
        reason: "knowledgeBases.runKnowledgeBaseQuery",
      });
...
export const getKBMemoriesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const peerIdFromChannel = queryChannel?.includes(":")
      ? queryChannel.split(":").pop()
      : undefined;
