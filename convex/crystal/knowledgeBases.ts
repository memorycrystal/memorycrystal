...
const dashboardTotalsDeltaValidator = v.object({
  ...
  knowledgeBaseMemoriesDelta: v.optional(v.number()),
...
const DASHBOARD_DELTA_NUMBER_FIELDS = [
  ...
  // ILL-179 — must be summed with the other counters, otherwise a batched KB
  // import applies activeMemoriesDelta without its knowledgeBaseMemoriesDelta.
...
// Bare forms of these multi-tenant/group-capable channel families do not name
// one safe conversation scope. Proactive extraction uses this registry to fail
// closed before creating a memory; exact scoped forms remain eligible.
export const AMBIGUOUS_BARE_CHANNEL_SCOPES: ReadonlySet<string> = new Set([
  ...PEER_CAPABLE_SCOPES,
  "discord",
  "slack",
]);
...
const normalizeAgentIds = (agentIds?: string[]) => {
  ...
  // Access has three intentionally distinct states:
  //   undefined -> open to every agent on the account (legacy/default)
  //   []        -> closed to every agent
  //   [ids...]  -> restricted to the listed agents
  // Do not collapse [] into undefined: removing the final assigned agent must
  // close the KB, not accidentally make it account-wide.
  return normalized;
};
...
// ── ILL-79: per-agent recall priority multipliers ────────────────────────────
// agentPriorities scales a KB's contribution to AMBIENT recall ranking for the
// requesting agent. It NEVER affects visibility (that stays agentIds-gated),
// and deliberate single-KB queries (queryKnowledgeBase) are exempt by design.
export const KB_AGENT_PRIORITY_MIN = 0.05;
export const KB_AGENT_PRIORITY_MAX = 1;
export const KB_AGENT_PRIORITY_WILDCARD = "*";

export const clampKnowledgeBaseAgentPriority = (value: number): number =>
  Math.min(Math.max(value, KB_AGENT_PRIORITY_MIN), KB_AGENT_PRIORITY_MAX);

// Pure resolver (unit-tested). Exact agentId key wins, then the "*" wildcard,
// then 1.0. An undefined agentId (unauthenticated/underived context — same
// shape the visibility gate handles) can only match the wildcard. Stored
// values are clamped defensively on read as well as validated on write.
export const resolveKnowledgeBaseAgentPriority = (
  knowledgeBase: { agentPriorities?: Record<string, number> } | null | undefined,
  agentId: string | undefined,
): number => {
  const priorities = knowledgeBase?.agentPriorities;
  if (!priorities) return 1;
  const exact = agentId !== undefined ? priorities[agentId] : undefined;
  const resolved = exact ?? priorities[KB_AGENT_PRIORITY_WILDCARD];
  if (typeof resolved !== "number" || !Number.isFinite(resolved)) return 1;
  return clampKnowledgeBaseAgentPriority(resolved);
};

// Write-time validation: reject out-of-range / non-finite values instead of
// silently clamping, so a typo like 35 (meant 0.35) fails loudly.
export const validateKnowledgeBaseAgentPriorities = (
  priorities: Record<string, number>,
): void => {
  for (const [agentId, value] of Object.entries(priorities)) {
    if (!agentId.trim()) {
      throw new Error("agentPriorities keys must be non-empty agentIds (or \"*\")");
    }
    if (
      typeof value !== "number" ||
      !Number.isFinite(value) ||
      value < KB_AGENT_PRIORITY_MIN ||
      value > KB_AGENT_PRIORITY_MAX
    ) {
      throw new Error(
        `agentPriorities["${agentId}"] must be a number in [${KB_AGENT_PRIORITY_MIN}, ${KB_AGENT_PRIORITY_MAX}]; got ${String(value)}`,
      );
    }
  }
};
...
export const isKnowledgeBaseVisibleToAgent = (
  ...
  const allowedAgentIds = normalizeAgentIds(knowledgeBase.agentIds);
  if (!agentId) {
    return allowedAgentIds === undefined || allowedAgentIds.includes("*");
  }
...
const runBatchImportChunks: any = async (
  ...
  options?: { frontendJobId?: Id<"crystalFrontendJobs"> },
) : Promise<{
  knowledgeBaseId: Id<"knowledgeBases">;
  ...
  let updatedCount = 0;
  const progressUpdateStep = Math.max(1, Math.ceil(args.chunks.length / 50));
  ...
  for (let chunkIdx = 0; chunkIdx < args.chunks.length; chunkIdx++) {
    ...
    const processedChunks = chunkIdx + 1;
    const shouldPublishProgress =
      processedChunks === 1 ||
      processedChunks === args.chunks.length ||
      processedChunks % progressUpdateStep === 0;
    if (options?.frontendJobId && shouldPublishProgress) {
      await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
        jobId: options.frontendJobId,
        userId,
        status: "running",
        progressCurrent: processedChunks,
        progressTotal: args.chunks.length,
        progressLabel: `${processedChunks} of ${args.chunks.length} chunks imported`,
      });
    }
  ...
  if (parentMemoryId !== null) {
    if (options?.frontendJobId) {
      await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
        ...
        userId,
        status: "running",
        progressCurrent: args.chunks.length,
        progressTotal: args.chunks.length,
        progressLabel: "Finalizing knowledge graph",
      });
    }
      ...
      if (existing.length > 0) {
        ...
        await applyDashboardTotalsDelta(
          ...
          buildMemoryTransitionDelta({
            ...
            oldKnowledgeBaseId: primary.knowledgeBaseId,
            newArchived: false,
        ...
        for (let i = 1; i < existing.length; i += 1) {
          ...
          await archiveMemoryAndSyncCleanupProjection(ctx, dup, now);
          await applyDashboardTotalsDelta(
            ctx,
            args.userId,
            buildMemoryTransitionDelta({
              ...
              oldKnowledgeBaseId: dup.knowledgeBaseId,
              newArchived: true,
              newStore: dup.store,
  ...
  handler: async (ctx, args) => {
    ...
    if (insertedIds.length > 0) {
      addDashboardTotalsDelta(dashboardDelta, {
        ...
        // ILL-179 — every row inserted here carries args.knowledgeBaseId, so the
        // KB counter moves 1:1 with activeMemories for this batch.
        knowledgeBaseMemoriesDelta: insertedIds.length,
...
export const updateKnowledgeBase = mutation({
  args: {
    ...
    peerScopePolicy: peerScopePolicyValidator,
    openToAllAgents: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Unauthenticated");
    ...
    if (args.openToAllAgents === true) patch.agentIds = undefined;
    else if (args.agentIds !== undefined) patch.agentIds = normalizeAgentIds(args.agentIds);
...
export const patchKnowledgeBaseInternal = internalMutation({
  args: {
    ...
    clearAgentIds: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    const knowledgeBase = await ctx.db.get(args.knowledgeBaseId);
    if (!knowledgeBase || knowledgeBase.userId !== args.userId) {
    ...
    if (args.clearAgentIds === true) patch.agentIds = undefined;
    else if (args.patch.agentIds !== undefined) patch.agentIds = normalizeAgentIds(args.patch.agentIds);
    if (args.patch.agentPriorities !== undefined) {
      validateKnowledgeBaseAgentPriorities(args.patch.agentPriorities);
      // Empty object clears the field entirely (back to default-1.0 behavior).
      patch.agentPriorities = Object.keys(args.patch.agentPriorities).length > 0
        ? args.patch.agentPriorities
        : undefined;
    }
...
export const listKnowledgeBasesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const summaries = await (typeof args.channel === "string"
    ...
    // ILL-79 — additive: expose the caller's resolved per-agent priority per KB
    // (agentPriorities itself rides the summary spread). Resolution uses the
    // SAME effective-agentId context as visibility.
    return summaries.map((summary: any) => {
      const { effectiveAgentId } = resolveDirectKnowledgeBaseQueryContext(summary, {
        agentId: args.agentId,
        channel: args.channel,
      });
      return {
        ...summary,
        resolvedAgentPriority: resolveKnowledgeBaseAgentPriority(summary, effectiveAgentId),
      };
    });
  },
});
...
export const startPermanentKnowledgeBaseDelete = action({
  args: { knowledgeBaseId: v.id("knowledgeBases") },
  handler: async (ctx, args): Promise<{
    jobId: Id<"crystalKnowledgeBaseLifecycleJobs">;
    ...
    const frontendJob = await ctx.runMutation(
      (internal as any).crystal.frontendJobs.createInternal,
      {
        ...
        resourceId: String(args.knowledgeBaseId),
        title: `Deleting ${knowledgeBase.name}`,
        route: "/knowledge",
        progressTotal: knowledgeBase.memoryCount,
        progressLabel: "Queued",
      },
    );
    const lifecycleJob = await ctx.runMutation((internal as any).crystal.knowledgeBaseLifecycle.startInternal, {
      userId,
      knowledgeBaseId: args.knowledgeBaseId,
      operation: "delete",
      frontendJobId: frontendJob.jobId,
    });
    return { ...lifecycleJob, frontendJobId: frontendJob.jobId };
  },
});

export const startKnowledgeBaseArchive = action({
  args: { knowledgeBaseId: v.id("knowledgeBases") },
  handler: async (ctx, args): Promise<{
    jobId: Id<"crystalKnowledgeBaseLifecycleJobs">;
    frontendJobId: Id<"crystalFrontendJobs">;
    ...
    const frontendJob = await ctx.runMutation(
      (internal as any).crystal.frontendJobs.createInternal,
      {
        ...
        resourceId: String(args.knowledgeBaseId),
        title: `Archiving ${knowledgeBase.name}`,
        route: "/knowledge",
        progressTotal: knowledgeBase.memoryCount,
        progressLabel: "Queued",
      },
...
export const startKnowledgeBaseImport = action({
  args: {
    knowledgeBaseId: v.id("knowledgeBases"),
    chunks: v.array(batchImportChunkValidator),
  },
  handler: async (ctx, args): Promise<{ frontendJobId: Id<"crystalFrontendJobs">; created: boolean }> => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Unauthenticated");
    const userId = stableUserId(identity.subject);
    const knowledgeBase = await ctx.runQuery(
    ...
    if (!knowledgeBase.isActive) throw new Error("Knowledge base is archived");
    if (args.chunks.length === 0) throw new Error("No chunks to import");
    ...
    const frontendJob = await ctx.runMutation(
      (internal as any).crystal.frontendJobs.createInternal,
      {
        ...
        resourceId: String(args.knowledgeBaseId),
        title: `Importing into ${knowledgeBase.name}`,
        route: "/knowledge",
        progressTotal: args.chunks.length,
        progressLabel: "Queued",
      },
    ...
    if (frontendJob.created) {
      await ctx.scheduler.runAfter(
        0,
        (internal as any).crystal.knowledgeBases.processFrontendKnowledgeBaseImport,
        {
          userId,
          knowledgeBaseId: args.knowledgeBaseId,
          chunks: args.chunks,
...
export const processFrontendKnowledgeBaseImport = internalAction({
  args: {
    userId: v.string(),
    knowledgeBaseId: v.id("knowledgeBases"),
    chunks: v.array(batchImportChunkValidator),
  ...
  handler: async (ctx, args) => {
    ...
    try {
      const result = await runBatchImportChunks(
        ctx as any,
        args.userId,
        { knowledgeBaseId: args.knowledgeBaseId, chunks: args.chunks },
        { frontendJobId: args.frontendJobId },
      );
      await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
        jobId: args.frontendJobId,
        userId: args.userId,
...
const batchEmbedTexts = async (
  ...
  try {
    ...
    if (error instanceof ConvexError && (error.data as any)?.code === "missing_openrouter_key") {
      // ILL-184: no credential is an explicit failure — abort the batch rather
      // than silently importing an un-embeddable knowledge base.
      throw error;
    }
...
const runKnowledgeBaseQuery: any = async (
  ...
  args: {
    ...
    allowIndexedFill?: boolean;
    skipAccessBookkeeping?: boolean;
  }
  ...
  const indexedKbFillAllowed =
    args.allowIndexedFill !== false &&
    (retrievalMode.indexedFallbackAllowed || !retrievalMode.degraded);
  ...
  if (ranked.length > 0 && args.skipAccessBookkeeping !== true) {
...
export const queryKnowledgeBaseInternal = internalAction({
  args: {
    ...
    includeGraphContext: v.optional(v.boolean()),
    allowIndexedFill: v.optional(v.boolean()),
    skipAccessBookkeeping: v.optional(v.boolean()),
  },
  handler: async (ctx, args): Promise<any> => {
