...
export const patchKnowledgeBaseInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.knowledgeBaseId, {
      ...
      sourceType: args.patch.sourceType?.trim() || undefined,
      description: args.patch.description?.trim() || undefined,
...
const runKnowledgeBaseQuery: any = async (
  ...
  // Hard scope filter: if the channel has a peer ID (e.g. 'myapp:123'),
