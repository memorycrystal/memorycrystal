...
async function resolveKbRetrievalMode(
  ...
  args: { userId: string; skipCostBreaker?: boolean; precomputedCostBudget?: CostBudgetResult | null },
): Promise<KbRetrievalMode> {
  if (args.precomputedCostBudget) {
    const budget = args.precomputedCostBudget;
    const unknownDimensionEmergency = budget.emergency &&
      budget.vectorEmergency !== true &&
      budget.textEmergency !== true;
    const vectorAllowed = !budget.vectorEmergency && !unknownDimensionEmergency;
    const textAllowed = !budget.textEmergency && !unknownDimensionEmergency;
    const reasons = [
      ...(vectorAllowed ? [] : ["vector_budget_exceeded"]),
      ...(textAllowed ? [] : ["text_budget_exceeded"]),
    ];
    const tier = reasons.length > 0 ? await getUserTierForKbFallback(ctx, args.userId) : null;
    const policy = tier ? getKbRetrievalPolicyForTier(tier) : null;

    return {
      vectorAllowed,
      textAllowed,
      indexedFallbackAllowed: reasons.length > 0 && !!policy?.indexedFallbackAllowedOnDegradation,
  ...
  const vectorAllowed = vectorBudget?.vectorEmergency !== true;
  const textAllowed = textBudget?.textEmergency !== true;
  const reasons = [
    ...(vectorAllowed ? [] : ["vector_budget_exceeded"]),
    ...(textAllowed ? [] : ["text_budget_exceeded"]),
  ];
  const needsFallback = !vectorAllowed || !textAllowed;
  const tier = needsFallback ? await getUserTierForKbFallback(ctx, args.userId) : null;
  const policy = tier ? getKbRetrievalPolicyForTier(tier) : null;

  return {
    vectorAllowed,
    textAllowed,
    indexedFallbackAllowed: needsFallback && !!policy?.indexedFallbackAllowedOnDegradation,
    degraded: reasons.length > 0,
    reasons,
    tier: tier ?? undefined,
    vectorBudget,
...
//   - a real channel string (e.g. "peer-coach:511172388", "general"), or
...
export const isKnowledgeBaseVisibleToAgent = (
  ...
  if (normalizedScope && normalizedScope !== normalizedChannel) {
    ...
    // channel="peer-coach:511172388"). Handled below.
  ...
  if (isPeerCapablePrefix) {
    ...
    if (colonIdx < 0) {
      ...
      // "peer-coach:team") → DENY unless permissive.
      if (policy !== "permissive") {
...
export const insertKnowledgeBaseChunkInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // — if the parent carries an explicit scope (e.g. "peer-coach:12345"),
...
const normalizeSharedMainQueryChannel = (
  knowledgeBase: Pick<KnowledgeBaseDoc, "scope" | "peerScopePolicy">,
  channel?: string,
) => {
...
const peerIdForKnownPeerScopedChannel = (channel?: string) => {
  if (typeof channel !== "string") {
    return undefined;
  }
  const parts = channel.split(":");
  ...
  const suffix = parts.length > 1 ? parts[parts.length - 1] : "";
  if (!PEER_CAPABLE_SCOPES.has(prefix)) {
    return undefined;
  }
  if (suffix === "") {
    return "";
...
const runKnowledgeBaseQuery: any = async (
  ...
  const queryChannel = normalizeSharedMainQueryChannel(knowledgeBase, args.channel);
  ...
  const guardChannel: string | ManagementChannel =
    typeof queryChannel === "string" ? queryChannel : "";
  ...
  let queryEmbedding: number[] | null = Array.isArray(args.queryEmbedding)
    ? args.queryEmbedding
    : null;
  if (retrievalMode.vectorAllowed && !queryEmbedding) {
    try {
  ...
  const [vectorResults, textResults] = await Promise.all([
    retrievalMode.vectorAllowed && Array.isArray(queryEmbedding)
  ...
  if (retrievalMode.indexedFallbackAllowed && candidateDocs.length < normalizedLimit) {
  ...
  // Hard scope filter: if the channel has a peer ID (e.g. 'peer-coach:123'),
  ...
  // was the primary read-side carrier of the support assistant/peer-specific PII incident.
  ...
  const peerIdFromChannel = peerIdForKnownPeerScopedChannel(queryChannel);
  // Trailing-colon guard: a channel like "peer-coach:" would produce an empty
  ...
  const parentScopeMatchesPeer =
    !!peerIdFromChannel &&
    (knowledgeBase.scope === peerIdFromChannel || knowledgeBase.scope === queryChannel);
  const allowSharedMainChunksForPeer = isPermissiveSharedMainKnowledgeBase(knowledgeBase);
  const scopedDocs = peerIdFromChannel
    ? candidateDocs.filter((m: any) => {
        const keep =
          m.scope === peerIdFromChannel ||
          m.scope === queryChannel ||
...
export const getKBMemoriesInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const peerIdFromChannel = peerIdForKnownPeerScopedChannel(queryChannel);
    // Trailing-colon guard: "peer-coach:" produces empty peerId which
