...
export const getActiveMemories = query({
  args: { channel: v.optional(v.string()), agentId: v.optional(v.string()), limit: v.number() },
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) throw new Error("Unauthenticated");
    const userId = stableUserId(identity.subject);
    ...
    // Prefer an explicit agentId (forwarded from the wake tool for scoped KB
    // visibility); fall back to deriving it from the channel prefix.
    const effectiveAgentId = resolveKnowledgeBaseAgentId(args.agentId, channel);
    ...
    return memories
      .filter((memory) => {
        if (memory.knowledgeBaseId) {
          ...
          return Boolean(
            knowledgeBase &&
              isKnowledgeBaseVisibleToAgent(knowledgeBase, effectiveAgentId, guardChannel) &&
              // Per-chunk peer isolation (2026-07-02 cross-tenant KB leak).
              isKnowledgeBaseChunkVisibleInChannel(memory.channel, channel)
          );
        }
