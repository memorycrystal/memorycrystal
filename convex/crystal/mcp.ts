...
import { CORE_MEMORY_CAP } from "./dailyDriver";
import {
...
import {
  createRecallStageTimer,
  finalizeRecallTimings,
} from "./recallTimings";
...
function composeFinalRecallMemories(
  ...
  return rankRecallCandidates(
    ...
    .map((candidate) => {
      const { scoreValue, memoryId, rankingSignals, ...rest } = candidate as any;
      // ILL-245 R10: add relevance to each memory
      const relevance = computeRelevance(
        rankingSignals?.vectorScore ?? 0,
        rankingSignals?.textMatchScore ?? 0
      );
      return {
        ...rest,
...
async function searchMessageMatches(
  ...
  // INTENT: Parallelize embed, textBudget, and recent query startup.
  const embedPromise = precomputedEmbedding
    ? Promise.resolve(precomputedEmbedding as number[] | null)
    : vectorDepth !== 0
      ? embedText(query, ctx, { userId, source: "mcp.searchMessageMatches" }).catch(() => null as number[] | null)
      : Promise.resolve(null as number[] | null);
  const textBudgetPromise = costBreakerChecked
    ? Promise.resolve(null)
    : debitRecallCost(ctx, { userId, surface: "messages", estimatedTextQueryBytes: ESTIMATED_TEXT_INDEX_BYTES, reason: "mcp.searchMessageMatches.text" }).catch(() => null);
  // INTENT: Cap compositor recent fallback to <= 50; keep search_messages at 400.
  const recentLimit = options?.compositorRecentCap
    ? Math.min(Math.max(candidateDepth * 8, 50), 50)
    : Math.min(Math.max(candidateDepth * 8, 50), 400);
  const recentPromise = ctx.runQuery(internal.crystal.messages.getRecentMessagesForUser, { userId, limit: recentLimit, channel: normalizedChannel, sessionKey: normalizedSessionKey, sinceMs: recentSinceMs, beforeMs }).catch(() => []) as Promise<Array<{ _id: string; role: "user" | "assistant" | "system"; content: string; channel?: string; sessionKey?: string; turnId?: string; turnMessageIndex?: number; timestamp: number }>>;

  const embedding = await embedPromise;
  const textBudget = await textBudgetPromise;

  // INTENT: Vector and text search run in parallel after embed resolves.
  const [semanticResults, textResults] = await Promise.all([
    vectorDepth !== 0 && Array.isArray(embedding)
      ? ctx.runAction(internal.crystal.messages.searchMessagesForUser, { userId, embedding, limit: candidateDepth, channel: normalizedChannel, sessionKey: normalizedSessionKey, sinceMs, skipCostBreaker: costBreakerChecked, vectorDepth, textAllowed: options?.textAllowed }).catch(() => [] as MessageMatch[])
      : Promise.resolve([] as MessageMatch[]),
    options?.textAllowed !== false && !textBudget?.emergency
      ? ctx.runQuery(internal.crystal.messages.searchMessagesByTextForUser, { userId, query, limit: candidateDepth, channel: normalizedChannel, sessionKey: normalizedSessionKey, sinceMs }).catch(() => [] as MessageMatch[])
      : Promise.resolve([] as MessageMatch[]),
  ]);
  semanticMatches = semanticResults;
  indexedLexicalMatches = textResults;
  ...
  return enriched.slice(offset, offset + requestedLimit);
}
...
export const captureMemory = internalMutation({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    if (freshness.volatile && freshness.coercion) {
      const coerceVolatile = await resolveFeatureFlag(
        ctx,
        "freshnessCoercion",
        "MC_FRESHNESS_COERCION",
        false,
      );
      if (coerceVolatile) {
      ...
      results
        .map((r) => {
          ...
          return {
            ...
            // ILL-104 — carry provenance from the hydrated doc through ranking.
...
export const mcpRecall = httpAction(async (ctx, request) => {
  ...
  const stageTimer = createRecallStageTimer();

  let memories: any[] = [];
  const recallDiagnostics = {
    // email is filled in below from the account lookup that runs alongside the
    // tier fetch, so diagnostics identify the account, not just an opaque id.
    account: { userId: auth.userId, email: null as string | null, name: null as string | null },
    scope: {
  ...
  // Policy, identity, and cost debits are independent of each other.
  const [activePolicyWeights, userTier, accountIdentity, recallBudget, textBudget] =
    await Promise.all([
      ctx
        .runQuery(
          (internal as any).crystal.organic.policyTuner.getActivePolicyWeights,
      ...
      ctx
        .runQuery(internal.crystal.mcp.getAccountIdentity, {
          userId: auth.userId,
        })
        .catch((err: unknown) => {
          console.error("[recall] account identity fallback:", err);
          return { userId: auth.userId, email: null, name: null };
        }),
      debitRecallCost(ctx, {
        userId: auth.userId,
        surface: "recall",
  ...
  recallDiagnostics.account.email = accountIdentity.email;
  recallDiagnostics.account.name = accountIdentity.name;
  ...
  // Lexical IO and KB visibility do not need the query embedding, so they
  // overlap embed + crystalMemories vectorSearch.
  const lexicalIoPromise = stageTimer.measure("lexical", () =>
    Promise.all([
  ...
  const kbListPromise = stageTimer.measure("kb", () =>
    ctx
      .runQuery(
        hasKnowledgeBaseScope
  ...
  // INTENT: assign the outer binding only. Start after embed so STM overlaps
  // vectorSearch; start again after the semantic try so STM still runs when
  // LTM vector is skipped or embed throws (queryEmbedding may be null).
  const startRecallMessagesSearch = () => {
    if (messagesPromise) return;
    messagesPromise = stageTimer.measure("messages", async () => {
      if (hasKnowledgeBaseScope) return [] as MessageMatch[];
      ...
      return await searchMessageMatches(
        ctx,
        auth.userId,
        query,
        Math.min(limit, 10),
  ...
  try {
    if (!recallReachPolicy.vectorAllowed) {
      ...
      queryEmbedding = await stageTimer.measure("embed", () =>
        embedText(query, ctx, {
          userId: auth.userId,
          source: "mcp.recall",
        }),
      );
      startRecallMessagesSearch();

      if (Array.isArray(queryEmbedding) && !hasKnowledgeBaseScope) {
        memories = await stageTimer.measure("vectorSearch", () =>
  ...
  const [textSearchResults, recentMemories] = await lexicalIoPromise;
  ...
  for (const source of [
    ...
    lexicalCandidatesById.set(id, {
      ...
      // ILL-104 — carry provenance so mcpRecall can flag age/source/supersession.
  ...
  const lexicalMemories = rankRecallCandidates(
    ...
    .map(
      (
        ...
        // ILL-104 — carry provenance for the mcpRecall staleness/source flags.
        source: m.source,
        supersededByMemoryId: m.supersededByMemoryId,
        score: m.scoreValue,
  ...
  stageTimer.add("lexical", Date.now() - lexicalHydrateStarted);
  ...
  // INTENT: Await kbListPromise without waiting for messages.
  const activeKBs = await kbListPromise;
  ...
  const resolveMessages = async () => {
    if (messagesResolved) return messageMatches;
    messageMatches = await (messagesPromise ?? Promise.resolve([]));
  ...
  // KB search overlaps asset lookup. Graph stays on after compose.
  const kbSearchPromise = stageTimer.measure("kb", async () => {
  try {
  ...
  const assetContexts = await Promise.all([assetPromise, kbSearchPromise]).then(
    ([assets]) => assets,
  );
  ...
  memories = await stageTimer.measure("compose", () =>
    composeFinalRecallMemories(memories, {
      query,
      channel,
      projectId,
      recallIntent,
  ...
  const [graphContext, contradictedIdList] = await Promise.all([
    includeGraphContext && shapedMemories.length > 0
      ? stageTimer.measure("graph", () =>
          ctx
            .runQuery(
    ...
    ctx
      .runQuery(internal.crystal.recall.getContradictedOlderSideMemoryIds, {
        userId: auth.userId,
        memories: shapedMemories.map((m: any) => ({
          memoryId: String(m.memoryId ?? m._id ?? ""),
          createdAt: typeof m.createdAt === "number" ? m.createdAt : provenanceNow,
        })),
      })
      .catch(() => [] as string[]),
    recalledMemoryIds.length > 0
      ? Promise.all([
          scheduleMutationOrFallback(ctx, internal.crystal.mcp.bumpAccessCounts, {
            memoryIds: recalledMemoryIds,
          }),
          scheduleMutationOrFallback(
            ctx,
  ...
  const contradictedIds = new Set<string>(contradictedIdList as string[]);
  const memoriesWithProvenance = shapedMemories.map((m: any) =>
    applyProvenanceToMemory(m, contradictedIds, provenanceNow),
  );

  // ILL-245 R9: Apply cross-person filter on personal_attribute intent
  const { filtered: filteredMessageMatches, suppressedCount: crossPersonSuppressed } =
    applyCrossPersonFilter(await resolveMessages(), {
      recallIntent,
      query,
      accountHolderName: auth.userId, // TODO: resolve actual name
    });
  
  if (crossPersonSuppressed > 0) {
    recallDiagnostics.suppressions.crossPerson = crossPersonSuppressed;
  }

  // ILL-245 R6: Build coverage diagnostics
  const coverageDiagnostics = buildCoverageDiagnostics(
    memoriesWithProvenance,
    filteredMessageMatches,
    {
      recallIntent,
      requestedLimit: limit,
    }
  ...
  // ILL-245 R7: Build promotion candidates
  ...
  return json({
    memories: memoriesWithProvenance,
    ...
    // ILL-245 R7: top-level promotionCandidates
    ...
    diagnostics: {
      ...recallDiagnostics,
      // ILL-245 R6: coverage diagnostics
      coverage: coverageDiagnostics,
      timings: finalizeRecallTimings({
        stages: {
          embed: stageTimer.get("embed"),
          vectorSearch: stageTimer.get("vectorSearch"),
          lexical: stageTimer.get("lexical"),
          kb: stageTimer.get("kb"),
          messages: stageTimer.get("messages"),
          graph: stageTimer.get("graph"),
          compose: stageTimer.get("compose"),
        },
        totalMs: Date.now() - recallStartedAt,
        parallel: true,
      }),
    },
  });
...
const wakeHandler = httpAction(async (ctx, request) => {
  ...
  const coreRaw = await ctx
    .runQuery(internal.crystal.dailyDriver.getCoreMemories, {
      userId: auth.userId,
      cap: CORE_MEMORY_CAP,
    })
    .catch(() => []);
...
// ILL-107 D2 — hygiene worklist. Returns the specific stale / never-recalled
// memory IDs grouped by severity bucket (with triage context) so crystal-hygiene
// acts on a concrete list instead of re-scanning. Explicit-field return (not a
// spread) to keep the payload bounded.
export const mcpHealth = httpAction(async (ctx, request) => {
  ...
  const limit = optionalBoundedNumber(body?.limit, 1, 500);

  const worklist = await ctx.runQuery(
    internal.crystal.hygieneWorklist.getStaleWorklist,
    {
      userId: auth.userId,
  ...
  return json({
    ok: true,
    computedAt: worklist.computedAt,
    limit: worklist.limit,
    pageTruncated: worklist.pageTruncated,
