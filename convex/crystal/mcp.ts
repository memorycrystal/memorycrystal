...
import { lexicalMessageScore, sanitizeUserMessageContent, unwrapQuotedSearchQuery, type SearchMessageResult } from "./messages";
...
import { createProxyReadDescriptor, resolveAssetStorageConfig } from "./assetStorage";
...
type SensoryCaptureMode = "raw_import" | "external_observation" | "special_capture";
...
const SENSORY_CAPTURE_MODES: SensoryCaptureMode[] = ["raw_import", "external_observation", "special_capture"];
...
const ASSET_MIME_PREFIXES: Record<AssetKind, string[]> = {
  ...
  video: ["video/"],
  pdf: ["application/pdf"],
  text: ["text/", "application/json", "application/xml", "application/x-ndjson"],
};
...
function normalizeSensoryCaptureMode(value: unknown): SensoryCaptureMode | null {
  const mode = String(value ?? "").trim() as SensoryCaptureMode;
  return SENSORY_CAPTURE_MODES.includes(mode) ? mode : null;
}

function isSensoryConversationCapture(store: MemoryStore, category: MemoryCategory) {
  return store === "sensory" && category === "conversation";
}

function isLegacySensoryAutoCapture(tags: string[]) {
  const normalized = tags.map((tag) => tag.trim().toLowerCase());
  return normalized.includes("auto-capture") || normalized.includes("openclaw") || normalized.includes("turn");
}
...
function isPeerScopedKnowledgeChannel(channel?: string): boolean {
  ...
  return (prefix === "peer-coach" || prefix === "support-coach") && /^\d+$/.test(suffix);
}
...
async function isMemoryVisibleForRequestChannel(ctx: ActionCtx, memory: any, channel?: string): Promise<boolean> {
  const effectiveChannel = normalizeChannel(channel);
  if (!effectiveChannel) return true;
  const knowledgeBasesById = await loadKnowledgeBasesById(ctx as any, [memory]);
  return filterVisibleMemories([memory], knowledgeBasesById, effectiveChannel).length > 0;
}
...
const filterMessageMatchesByScope = <T extends { channel?: string; sessionKey?: string }>(
  messages: T[],
  channel?: string,
  sessionKey?: string
): T[] => {
...
function shouldIncludeEmbeddings(body: Record<string, unknown> | undefined): boolean {
  if (process.env.CRYSTAL_HTTP_INCLUDE_EMBEDDINGS === "true") return true;
  return body?.includeEmbeddings === true;
}

function shapeMessageForHttp<T extends object>(row: T, includeEmbeddings = false): Omit<T, "embedding" | "embeddingModel"> | T {
  ...
  const shaped = {
    ...rest,
    ...(typeof rest.content === "string" ? { content: redactSecrets(rest.content) } : {}),
...
function shapeMessagesForHttp<T extends object>(rows: T[], includeEmbeddings: boolean): T[] | Omit<T, "embedding" | "embeddingModel">[] {
  return rows.map((row) => shapeMessageForHttp(row, includeEmbeddings)) as T[] | Omit<T, "embedding" | "embeddingModel">[];
}
...
function shapeRecallMemoryForHttp<T extends Record<string, any>>(memory: T): T {
  return redactStringFields(memory, ["title", "content", "metadata", "summary"]);
}
...
function isAcceptedAssetMime(kind: AssetKind, mimeType: string): boolean {
  ...
  return ASSET_MIME_PREFIXES[kind].some((prefix) => normalized === prefix || normalized.startsWith(prefix));
}

function shapeTurnsForHttp(turns: MessageTurn[], includeEmbeddings: boolean): MessageTurn[] {
  return turns.map((turn) => ({
    ...turn,
    messages: turn.messages.map((message) => shapeMessageForHttp(message, includeEmbeddings)) as MessageTurn["messages"],
  }));
}

const filterMessageTurnsByScope = (turns: MessageTurn[], channel?: string, sessionKey?: string): MessageTurn[] => {
  if (!channel && !sessionKey) return turns;
  const filteredTurns: MessageTurn[] = [];
  for (const turn of turns) {
    const messages = filterMessageMatchesByScope(turn.messages, channel, sessionKey);
...
const formatRecentConversation = (messages: MessageMatch[]) => {
  ...
  return groupMessagesIntoTurns(messages).flatMap((turn) =>
    turn.messages.map((message) => {
      ...
      const text = safeContent.length > 140 ? `${safeContent.slice(0, 140)}...` : safeContent;
...
async function searchMessageMatches(
  ...
  try {
    const embedding = precomputedEmbedding ?? await embedText(query, ctx, {
      userId,
  ...
  return filterMessageMatchesByScope(dedupeMessageMatches(
    [...indexedLexicalMatches, ...semanticMatches, ...recentLexicalMatches].sort(
      (a, b) => b.score - a.score || b.timestamp - a.timestamp
    )
  ), normalizedChannel, normalizedSessionKey).slice(0, requestedLimit);
}
...
export const captureMemory = internalMutation({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    const now = Date.now();
    const tierInfo: { sensoryRawTtlDays: number } = await ctx.runQuery((internal as any).crystal.userProfiles.getUserTierInfo, {
      userId: args.userId,
    ...
    const id = await ctx.db.insert("crystalMemories", {
      ...
      ...(args.store === "sensory"
        ? {
            rawContentExpiresAt: rawContentExpiresAt(now, tierInfo.sensoryRawTtlDays),
...
export async function scheduleCaptureMemoryBackgroundWork(
  ...
  try {
    await ctx.scheduler.runAfter(0, internal.crystal.mcp.embedMemory, { memoryId: args.memoryId });
    await ctx.scheduler.runAfter(50, internal.crystal.salience.computeAndStoreSalience, { memoryId: args.memoryId });
...
export const createCheckpointExternal = internalMutation({
  ...
  handler: async (ctx, { userId, label, description, channel, sessionKey }) => {
    return await ctx.db.insert("crystalCheckpoints", {
      ...
      sessionKey: normalizeChannel(sessionKey),
      memorySnapshot: [],
...
export const listRecentMemories = internalQuery({
  ...
  handler: async (ctx, { userId, limit, channel }) => {
    ...
    const effectiveChannel = normalizeChannel(channel);
    const isPeerChannel = typeof effectiveChannel === "string" && /^[^:]+:\d+$/.test(effectiveChannel);
    const memories = isPeerChannel
      ? [
          ...await ctx.db
            .query("crystalMemories")
            .withIndex("by_user_channel_archived_last_accessed", (q) =>
              q.eq("userId", userId).eq("channel", effectiveChannel).eq("archived", false)
            )
            .order("desc")
            .take(fetch),
    ...
    const uniqueMemories = Array.from(new Map(memories.map((memory) => [String(memory._id), memory])).values());
    return filterVisibleMemories(uniqueMemories as Array<{ channel?: string; knowledgeBaseId?: string } & Record<string, any>>, knowledgeBasesById, effectiveChannel)
      .sort((a, b) => b.lastAccessedAt - a.lastAccessedAt)
      .slice(0, fetch)
      .map((memory) => ({ ...memory, content: getMemoryEffectiveText(memory as any) || memory.content }));
  },
});
...
export const listRecentCheckpoints = internalQuery({
  ...
  handler: async (ctx, { userId, limit, channel, sessionKey }) => {
    ...
    const normalizedChannel = normalizeChannel(channel);
    const normalizedSessionKey = normalizeChannel(sessionKey);
    const fetchLimit = normalizedChannel || normalizedSessionKey ? Math.min(max * 5, 500) : max;
    ...
    return checkpoints
      .filter((checkpoint: any) =>
        ...
        (!normalizedSessionKey || checkpoint.sessionKey === normalizedSessionKey)
      )
      .slice(0, max);
  },
});
...
export const mcpCapture = httpAction(async (ctx, request) => {
  ...
  if (isSensoryConversationCapture(store, category)) {
    if (sensoryCaptureModeRaw !== undefined && !sensoryCaptureMode) {
      return json({ error: "Invalid sensoryCaptureMode. Use raw_import, external_observation, or special_capture." }, 400);
    }
    if (!sensoryCaptureMode) {
      if (isLegacySensoryAutoCapture(rawTags)) {
        return json({
          ...
          message: "Ordinary conversation transcripts are stored in crystalMessages; sensory memories require sensoryCaptureMode.",
        });
      }
      return json({
...
export const mcpRecall = httpAction(async (ctx, request) => {
  ...
  for (const result of textSearchResults as Array<{ _id: string; bm25Boost?: number }>) {
    bm25BoostById.set(String(result._id), result.bm25Boost ?? 0.75);
  }

  const textCandidateIds = Array.from(bm25BoostById.keys());
  const textDocs = textCandidateIds.length > 0
    ? await ctx.runQuery(internal.crystal.mcp.getMemoriesByIds, { memoryIds: textCandidateIds as any }).catch((err: unknown) => {
        console.error("[recall] text hydration failed:", err);
        return [] as Array<Record<string, any>>;
      })
    : [] as Array<Record<string, any>>;

  const lexicalCandidatesById = new Map<string, RecallRankingCandidate & { _id: string; metadata?: string }>();
  for (const source of [...(textDocs as Array<Record<string, any>>), ...(recentMemories as Array<Record<string, any>>)]) {
    ...
    if (!content) continue;
    if (!isNonKnowledgeBaseMemoryVisibleInChannel(source.channel, channel)) continue;
    if (resolvedStores?.length && !resolvedStores.includes(source.store)) continue;
    if (resolvedCategories?.length && !resolvedCategories.includes(source.category)) continue;
    if (requestedTags?.length) {
  ...
  const activeKBs: any[] = await ctx.runQuery(
    ...
    { userId: auth.userId, includeInactive: false, agentId: effectiveAgentId, channel }
  ).then((allKBs: any[]) => allKBs.filter((kb: any) => {
    if (!kb?.isActive) return false;
    if (requestedKnowledgeBaseIds?.length && !requestedKnowledgeBaseIds.includes(String(kb._id))) return false;
    return true;
  })).catch((err: unknown) => {
  ...
  if (memories.length > 0) {
    const recalledMemoryIds = Array.from(new Set(memories.map((m: any) => String(m._id)).filter(Boolean)));
    ctx.runMutation(internal.crystal.mcp.bumpAccessCounts, {
      memoryIds: recalledMemoryIds,
    }).catch(() => {});
    ctx.runMutation(internal.crystal.organic.activityLog.logRecallActivity, {
      userId: auth.userId,
  ...
  const shapedMemories = memories.map((memory: any) => shapeRecallMemoryForHttp(memory));
  const shapedAssetContexts = assetContexts.map((asset: any) => shapeAssetContextForHttp(asset));

  return json({
    memories: shapedMemories,
    assetContexts: shapedAssetContexts,
    messageMatches: shapeMessagesForHttp(messageMatches, shouldIncludeEmbeddings(body)),
...
export const mcpGetTriggers = httpAction(async (ctx, request) => {
  ...
  const memories = memoryIds.length > 0
    ...
    : [];
  const knowledgeBasesById = await loadKnowledgeBasesById(ctx as any, memories as any[]);

  const filtered = (memories as any[]).filter((memory) => {
    if (!memory || memory.userId !== auth.userId || memory.archived) return false;
    if (!filterVisibleMemories([memory], knowledgeBasesById, channel).length) return false;
    const triggers = normalizeActionTriggers(memory.actionTriggers);
...
export const mcpSearchMessages = httpAction(async (ctx, request) => {
  ...
  const messages = filterMessageMatchesByScope(
    await searchMessageMatches(ctx, auth.userId, query, limit, channel, sessionKey, sinceMs),
    channel,
    sessionKey,
  );
  const includeEmbeddings = shouldIncludeEmbeddings(body);
  const turns = filterMessageTurnsByScope(groupMessagesIntoTurns(messages), channel, sessionKey);
  return json({
...
export const mcpRecentMessages = httpAction(async (ctx, request) => {
  ...
  const messages = filterMessageMatchesByScope(recentMessages, channel, sessionKey);
  const includeEmbeddings = shouldIncludeEmbeddings(body);
  const turns = filterMessageTurnsByScope(groupMessagesIntoTurns(messages), channel, sessionKey);
...
function shapeCheckpointForHttp(checkpoint: any) {
  return {
    ...
    memoryCount: Array.isArray(checkpoint.memorySnapshot) ? checkpoint.memorySnapshot.length : 0,
  };
}
...
export const mcpCheckpoint = httpAction(async (ctx, request) => {
  ...
  if (mode === "list") {
    ...
    return json({
      checkpoints: (checkpoints as any[]).map(shapeCheckpointForHttp),
    });
  }

  if (mode !== "create") return json({ error: "mode must be create or list" }, 400);
...
export const mcpSupersede = httpAction(async (ctx, request) => {
  ...
  if (!(await isMemoryVisibleForRequestChannel(ctx, oldMemory, requestChannel))) {
    return json({ error: "Memory not found" }, 404);
  }
...
const wakeHandler = httpAction(async (ctx, request) => {
  ...
  try {
    if (request.method === "POST") {
      wakeBody = await request.clone().json().catch(() => undefined);
    }
  } catch { /* ignore */ }
...
export const mcpLog = httpAction(async (ctx, request) => {
  ...
  if (role === "user") {
    ...
    if (sanitized.malformed || !sanitized.content.trim()) {
      return json({
        ok: true,
        skipped: true,
        reason: sanitized.malformed ? "malformed_synthetic_context" : "synthetic_context_only",
      });
    }
  }
  ...
  if (!id) return json({ ok: true, skipped: true, reason: "synthetic_context_only" });
  if (role === "assistant" && shouldScheduleMcpBackgroundWork()) {
    await ctx.scheduler.runAfter(120_000, (internal as any).crystal.ltmExtraction.runOnDemandLtmExtraction, {
      userId: auth.userId,
      channel: body?.channel ? String(body.channel) : undefined,
      sessionKey: body?.sessionKey ? String(body.sessionKey) : undefined,
      limit: 12,
...
const normalizeTurnString = (value: unknown): string => String(value ?? "").trim();

const normalizeTurnObject = (value: unknown): Record<string, unknown> | undefined => {
...
export const mcpTurn = httpAction(async (ctx, request) => {
  ...
  const captureMode = body?.captureMode === "sync-test-only" ? "sync-test-only" : "async";
  const metadata = normalizeTurnObject(body?.metadata);
  const metadataString = metadata
    ? JSON.stringify({
        ...metadata,
        ...(body?.platform ? { platform: String(body.platform) } : {}),
        ...(body?.externalUserId ? { externalUserId: String(body.externalUserId) } : {}),
      })
  ...
  if (!assistantMessage) return json({ error: "assistantMessage is required" }, 400);
  ...
  const result = await ctx.runMutation((internal as any).crystal.messages.logTurnInternal, {
    userId: auth.userId,
    sessionKey,
    channel,
    turnId,
  ...
  if (!result?.ok) {
    return json(result, result?.error?.includes("Storage limit reached") ? 403 : 400);
  }
  ...
  try {
    ...
    if (captureMode === "sync-test-only") {
      ...
      await ctx.scheduler.runAfter(120_000, (internal as any).crystal.ltmExtraction.runOnDemandLtmExtraction, extractionArgs);
      response.extraction.scheduled = true;
    } else {
    ...
    response.extraction.error = error instanceof Error ? error.message : String(error);
  }
...
export const mcpAsset = httpAction(async (ctx, request) => {
  ...
  return json({ error: "raw storageKey registration is disabled; use /api/mcp/asset/upload" }, 410);
});
...
export const mcpAssetUpload = httpAction(async (ctx, request) => {
  ...
  if (!Number.isFinite(contentLength) || contentLength <= 0) {
    return json({ error: "Content-Length is required and must be greater than zero" }, 411);
  }

  const kind = normalizeAssetKind(request.headers.get("x-crystal-asset-kind"));
  if (!kind) return json({ error: "X-Crystal-Asset-Kind must be one of: image, audio, video, pdf, text" }, 400);
  if (contentLength > ASSET_UPLOAD_CAPS_BYTES[kind]) {
    return json({ error: "asset exceeds per-kind upload limit", limitBytes: ASSET_UPLOAD_CAPS_BYTES[kind] }, 413);
  }

  const mimeType = String(request.headers.get("content-type") ?? "").split(";")[0]?.trim();
  if (!mimeType) return json({ error: "Content-Type is required" }, 400);
  if (!isAcceptedAssetMime(kind, mimeType)) {
    return json({ error: "Content-Type is not accepted for asset kind", kind, mimeType }, 415);
  }

  const idempotencyKey = request.headers.get("x-crystal-idempotency-key")?.trim() || undefined;
  if (idempotencyKey) {
    const existingId = await ctx.runQuery(internal.crystal.assets.getAssetIdByUserIdempotency, {
      userId: auth.userId,
      idempotencyKey,
    });
  ...
  const quota = await ctx.runQuery(internal.crystal.assets.getAssetQuotaAvailability, { userId: auth.userId });
  if (contentLength > quota.availableBytes) {
    return json({ error: "Asset storage quota exceeded", quotaBytes: quota.quotaBytes, usedBytes: quota.usedBytes, availableBytes: quota.availableBytes }, 413);
  }
  ...
  const checksum = normalizeSha256Checksum(request.headers.get("x-crystal-checksum"));
  if (checksum === null) return json({ error: "X-Crystal-Checksum must be a hex-encoded SHA-256 digest" }, 400);
  ...
  const sessionKey = request.headers.get("x-crystal-session-key")?.trim() || undefined;
  let storageId: Id<"_storage"> | null = null;

  try {
    const blob = await request.blob();
    if (blob.size <= 0) return json({ error: "Uploaded asset body is empty" }, 411);
    ...
    if (checksum && await sha256BlobHex(blob) !== checksum) {
      return json({ error: "X-Crystal-Checksum does not match uploaded asset body" }, 400);
    }

    storageId = await ctx.storage.store(blob);
    const storageMetadata = await ctx.storage.getMetadata(storageId).catch(() => null);
    if (!storageMetadata) throw new Error("Uploaded asset bytes not found in Convex storage after upload");
    const measuredBytes = typeof storageMetadata.size === "number" && Number.isFinite(storageMetadata.size) && storageMetadata.size > 0
    ...
    const assetStorage = await ctx.runQuery(internal.crystal.assets.getAssetStorageKeyForOwner, {
      userId: auth.userId,
      assetId: id,
    });
    if (!assetStorage || assetStorage.storageKey !== storageId) {
      await ctx.storage.delete(storageId).catch((err: unknown) => console.error("[asset.upload] duplicate cleanup failed:", err));
      storageId = null;
      return json({ ok: true, id, deduped: true });
    }

    await ctx.scheduler.runAfter(0, internal.crystal.assets.processAssetText, { assetId: id });
    storageId = null;
    return json({ ok: true, id });
  } catch (err) {
    if (storageId) {
      await ctx.storage.delete(storageId).catch((cleanupErr: unknown) => console.error("[asset.upload] cleanup failed:", cleanupErr));
    }
    console.error("[asset.upload] failed:", err);
    if (err instanceof Error && err.message.includes("Asset storage quota exceeded")) {
...
export const mcpAssetMetadata = httpAction(async (ctx, request) => {
  ...
  if (typeof assetId !== "string") return json({ error: "assetId is required" }, 400);
  const asset = await ctx.runQuery(internal.crystal.assets.getAssetMetadataForOwner, {
    userId: auth.userId,
...
export const mcpAssetReadUrl = httpAction(async (ctx, request) => {
  ...
  if (!asset) return json({ error: "Not found" }, 404);
  const assetStorage = await ctx.runQuery(internal.crystal.assets.getAssetStorageForOwner, {
    userId: auth.userId,
    assetId: assetId as any,
    channel,
    knowledgeBaseIds,
  ...
  if (asset.storageProvider !== "convex") {
    return json({ error: "unsupported storageProvider; re-upload this asset into Convex storage", provider: asset.storageProvider }, 409);
  }
  ...
  for (const knowledgeBaseId of knowledgeBaseIds ?? []) params.append("knowledgeBaseId", knowledgeBaseId);
...
export const mcpAssetDelete = httpAction(async (ctx, request) => {
  ...
  const assetId = body?.assetId;
  if (typeof assetId !== "string") return json({ error: "assetId is required" }, 400);
  const result = await ctx.runMutation(internal.crystal.assets.softDeleteAssetForOwner, {
    userId: auth.userId,
    assetId: assetId as any,
...
export const mcpAssetRetry = httpAction(async (ctx, request) => {
  ...
  const assetId = body?.assetId;
  if (typeof assetId !== "string") return json({ error: "assetId is required" }, 400);
  const result = await ctx.runMutation(internal.crystal.assets.markAssetRetryQueuedForOwner, {
    userId: auth.userId,
    assetId: assetId as any,
    channel: normalizeChannel(body?.channel),
    knowledgeBaseIds: optionalStringArray(body?.knowledgeBaseIds),
  ...
  if (!result.ok) {
    return json({ error: result.reason === "not_failed" ? "Asset is not failed" : "Not found" }, result.reason === "not_failed" ? 409 : 404);
  }
  await ctx.scheduler.runAfter(0, internal.crystal.assets.processAssetText, { assetId: assetId as any });
...
export const mcpAssetReadProxy = httpAction(async (ctx, request) => {
  ...
  const knowledgeBaseIds = url.searchParams.getAll("knowledgeBaseId").filter((value) => value.length > 0);
  if (!assetId || !Number.isFinite(expiresAt)) return json({ error: "assetId and expiresAt are required" }, 400);
  if (expiresAt <= Date.now()) return json({ error: "Read descriptor expired" }, 410);
  ...
  if (asset.storageProvider !== "convex") {
    return json({
      error: "unsupported storageProvider; re-upload this asset into Convex storage",
      provider: asset.storageProvider,
    }, 409);
  }

  const storageUrl = await ctx.storage.getUrl(assetStorage.storageKey as Id<"_storage">);
  if (!storageUrl) return json({ error: "Asset bytes are no longer available", provider: asset.storageProvider }, 404);
  await auditLog(ctx, auth.userId, auth.keyHash, "asset.read_proxy", { assetId, provider: asset.storageProvider });
...
export const mcpUploadUrl = httpAction(async (ctx, request) => {
  ...
  const rateLimitResponse = await withRateLimit(ctx, auth.keyHash);
  if (rateLimitResponse) return rateLimitResponse;
  return json({ error: "direct upload URLs are disabled; use /api/mcp/asset/upload" }, 410);
});
...
export const mcpReflect = httpAction(async (ctx, request) => {
  ...
  const openrouterCredential = await ctx.runQuery((internal as any).crystal.providerSettings.resolveOpenRouterKeyForUser, {
    userId: auth.userId,
    includeShared: false,
  }) as { apiKey: string | null };
  if (!openrouterCredential.apiKey) {
    return json({ error: "Reflection not available: add your OpenRouter API key in Settings" }, 503);
  }
...
export const embedMemory = internalAction({
  ...
  handler: async (ctx, { memoryId }) => {
    ...
    try {
      ...
      if (err instanceof ConvexError && (err.data as any)?.code === "embedding_cap_exceeded") {
        const userId = (err.data as any)?.userId;
        const dailyLimit = (err.data as any)?.dailyLimit;
        console.log(`[embedMemory] daily embedding cap exceeded for user ${userId} (limit=${dailyLimit}); skipping`);
...
async function dualWriteEnabled(ctx: any): Promise<boolean> {
  return await resolveFeatureFlag(ctx, "embeddingDualWriteEnabled", "MC_EMBEDDING_DUAL_WRITE_ENABLED", true);
}
...
export const patchMemoryEmbedding = internalMutation({
  ...
  handler: async (ctx, { memoryId, embedding }) => {
    ...
    if (await dualWriteEnabled(ctx) && embedding.length === EMBEDDING_DIMENSIONS) {
...
export const updateMemory = internalMutation({
  ...
  handler: async (ctx, { memoryId, userId, updates }) => {
    ...
    if ((updates.store !== undefined && updates.store !== existing.store) || refreshDerived) {
    ...
    if (updates.strength !== undefined && updates.strength !== existing.strength) {
      await applyDashboardTotalsDelta(
        ctx,
        userId,
    ...
    if (refreshDerived) {
      await scheduleMemoryDerivedRefresh(ctx, memoryId, userId, updates.store ?? existing.store);
    }
...
export const supersedeMemory = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await scheduleMemoryDerivedRefresh(ctx, successorId, args.userId, args.store);
...
export const backfillEmbeddings = internalAction({
  ...
  handler: async (ctx, { limit }): Promise<{ processed: number; succeeded: number; done: boolean }> => {
    ...
    while (succeeded < target) {
      const page: { page: Array<{ _id: any; userId?: string; content: string; embedding?: number[] }>; isDone: boolean; continueCursor?: string } = await ctx.runQuery(internal.crystal.mcp.listMemoriesPageForEmbeddingBackfill, {
