...
async function isMemoryVisibleForRequestChannel(
  ...
  if (memory?.knowledgeBaseId) {
    ...
    return Array.isArray(visibleKnowledgeBases) && visibleKnowledgeBases.length > 0;
  }
...
function recallDedupeKey(memory: any, collapseNearDuplicates: boolean): string {
  const normContent = normalizeMemoryContentForHash(String(memory?.content ?? ""));
...
async function searchMessageMatches(
  ...
  options?: { vectorDepth?: number; textAllowed?: boolean },
...
export const listRecentMemories = internalQuery({
  ...
  handler: async (ctx, { userId, limit, channel, sessionKey, scopeToSession }) => {
...
export const mcpRecall = httpAction(async (ctx, request) => {
  ...
  try {
    if (Array.isArray(activeKBs) && activeKBs.length > 0) {
      const MAX_KBS = recallIntent === "factual_framework" ? 12 : 8;
  ...
  const graphContext =
    body?.includeGraphContext !== false && shapedMemories.length > 0
      ? await ctx
          .runQuery(
