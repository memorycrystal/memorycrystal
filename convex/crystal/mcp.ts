...
const filterMessageMatchesByChannel = <T extends { channel?: string }>(messages: T[], channel?: string): T[] => {
...
function shapeMessageForHttp<T extends object>(row: T): Omit<T, "embedding" | "embeddingModel"> {
  const { embedding: _embedding, embeddingModel: _embeddingModel, ...rest } = row as T & {
    embedding?: unknown;
...
function shapeMessagesForHttp<T extends object>(rows: T[], includeEmbeddings: boolean): T[] | Omit<T, "embedding" | "embeddingModel">[] {
  return includeEmbeddings ? rows : rows.map(shapeMessageForHttp);
}

function shapeTurnsForHttp(turns: MessageTurn[], includeEmbeddings: boolean): MessageTurn[] {
  if (includeEmbeddings) return turns;
  return turns.map((turn) => ({
    ...turn,
    messages: turn.messages.map(shapeMessageForHttp) as MessageTurn["messages"],
  }));
}

const filterMessageTurnsByChannel = (turns: MessageTurn[], channel?: string): MessageTurn[] => {
  if (!channel) return turns;
  const filteredTurns: MessageTurn[] = [];
  for (const turn of turns) {
    const messages = filterMessageMatchesByChannel(turn.messages, channel);
...
export const mcpRecall = httpAction(async (ctx, request) => {
  ...
  try {
    ...
    const activeKBs = allKBs.filter((kb: any) => {
      ...
      return kb.agentIds.includes("*") || kb.agentIds.includes(effectiveAgentId);
    });
...
export const mcpSearchMessages = httpAction(async (ctx, request) => {
  ...
  const messages = filterMessageMatchesByChannel(
    await searchMessageMatches(ctx, auth.userId, query, limit, channel, sinceMs),
    channel,
  );
  const includeEmbeddings = shouldIncludeEmbeddings(body);
  const turns = filterMessageTurnsByChannel(groupMessagesIntoTurns(messages), channel);
  return json({
...
export const mcpRecentMessages = httpAction(async (ctx, request) => {
  ...
  const messages = filterMessageMatchesByChannel(recentMessages, channel);
  const includeEmbeddings = shouldIncludeEmbeddings(body);
  const turns = filterMessageTurnsByChannel(groupMessagesIntoTurns(messages), channel);
