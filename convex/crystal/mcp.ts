...
export const mcpRecall = httpAction(async (ctx, request) => {
  ...
  // Batched: these three are independent, and the identity lookup rides along so
  // naming the account in diagnostics costs no extra round trip on the hot path.
  const [activePolicyWeights, userTier, accountIdentity] = await Promise.all([
    ctx
      .runQuery(
        (internal as any).crystal.organic.policyTuner.getActivePolicyWeights,
        { userId: auth.userId },
  ...
  // ILL-104 — annotate provenance & staleness so crystal_recall's production
  // (API-key /api/mcp/recall) path and the plugin receive the same age/source +
  // stale/contradicted/superseded flags the recallMemories action produces. The
  // contradiction lookup is a single batched query over the whole set (no N+1),
  // reusing recall.ts:getContradictedOlderSideMemoryIds.
