  ...
  handler: async (ctx, args) => {
    ...
    while (candidates.length < limit) {
      const result = await ctx.db
        ...
        .withIndex("by_store_archived_salience", (q) =>
          q
            .eq("userId", args.userId)
            .eq("store", args.store)
            .eq("archived", false)
            .lte("salienceScore", args.maxSalienceScore)
        )
        .paginate({ numItems: 100, cursor });

      for (const memory of result.page) {
        if (memory.salienceScore === undefined) continue;
        if (memory.store === "sensory" && isProtectedSensoryCapture(memory)) continue;
...
export const promoteLowSalienceMemory = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing.store === "sensory" && isProtectedSensoryCapture(existing)) {
      return;
    }

    const previousStore = existing.store;
    const newStrength = clamp01(args.strength);
    ...
    // M7 — track totalStrength delta when promotion changes strength.
    if (newStrength !== existing.strength) {
      await applyDashboardTotalsDelta(
        ctx,
        existing.userId,
        buildStrengthDelta(existing.strength, newStrength),
      );
    }
