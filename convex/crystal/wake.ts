...
function formatRecentMessage(message: WakeRecentMessage): string {
  ...
  const shortContent = safeContent.length > 120 ? `${safeContent.slice(0, 120)}...` : safeContent;
  return `[${at}] ${message.role}: ${shortContent}`;
}

function redactRecentMessage(message: WakeRecentMessage): WakeRecentMessage {
  return {
    ...message,
...
const wakeInput = v.object({
  ...
  // Accepted for scoped KB visibility (mirrors the crystal_wake tool schema).
  // The MCP SDK/JWT wake path forwards a resolved agentId; a strict validator
  // would otherwise reject the extra field with an ArgumentValidationError.
...
const composeBriefing = (
  ...
  // ILL-108 G2: the always-on L0 core set, rendered first and independent of
  // the query-driven buckets below (its own bounded budget).
  const coreLines = coreMemories.length
    ? ["", "## 🧭 Core (always on)", ...coreMemories.map((memory) => `- [${memory.category}] ${memory.title}`)]
    : [];
  ...
  lines.push(
    "",
    `${coreMemories.length + openGoals.length + recentDecisions.length + guardrailMemories.length} memories surfaced | Use crystal_recall to search all memories.`
  );
...
export const getWakePrompt = action({
  ...
  handler: async (ctx, args) => {
    ...
    const agentId = args.agentId?.trim() || undefined;
    ...
    // ILL-108 G2: resolve the caller once to fetch the always-inject L0 core set
    // on its OWN bounded budget, independent of the query-driven buckets above.
    // Opt-in: no core-tagged memories → []. (G3's overnight brief is a dedicated
    // daily cron, not a per-wake recompute across both wake paths.)
    const identity = await ctx.auth.getUserIdentity();
    const userId = identity ? stableUserId(identity.subject) : null;
    // Fail-open: the L0 core set is advisory — a fetch failure must never break
    // the wake briefing, it just omits the Core section.
    const coreRaw = userId
      ? await ctx
          .runQuery("crystal/dailyDriver:getCoreMemories" as any, {
            userId,
            cap: CORE_MEMORY_CAP,
          })
          .catch(() => [])
    ...
    const rawRecentMessages = await ctx.runQuery(
      "crystal/messages:getRecentMessages" as any,
      { limit: 20, channel, sinceMs: now - 24 * 60 * 60 * 1000 }
    ) as WakeRecentMessage[];
    const recentMessages = (rawRecentMessages ?? []).map(redactRecentMessage);
    ...
    const injectedMemoryIds = [
      ...coreMemories.map((memory) => memory.memoryId),
      ...openGoals.map((memory) => memory.memoryId),
      ...recentDecisions.map((memory) => memory.memoryId),
      ...guardrailMemories.map((memory) => memory.memoryId),
    ];
