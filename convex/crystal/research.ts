...
export const listCollectionsInternal = internalQuery({
  args: {
    userId: v.string(), status: v.optional(collectionStatus), domain: v.optional(v.string()),
    workspaceId: v.optional(v.string()), agentId: v.optional(v.string()), channel: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    const base = args.status
      ...
      : args.domain
        ? ctx.db.query("crystalResearchCollections").withIndex("by_user_domain_updated", (q: any) => q.eq("userId", args.userId).eq("domain", args.domain))
        : args.workspaceId
          ? ctx.db.query("crystalResearchCollections").withIndex("by_user_workspace_updated", (q: any) => q.eq("userId", args.userId).eq("workspaceId", args.workspaceId))
          : args.agentId
            ? ctx.db.query("crystalResearchCollections").withIndex("by_user_agent_updated", (q: any) => q.eq("userId", args.userId).eq("agentId", args.agentId))
            : args.channel
              ? ctx.db.query("crystalResearchCollections").withIndex("by_user_channel_updated", (q: any) => q.eq("userId", args.userId).eq("channel", args.channel))
    ...
    return {
      collections: page.page.filter((row: any) =>
        (args.status === undefined || row.status === args.status)
        && (args.domain === undefined || row.domain === args.domain)
        && (args.workspaceId === undefined || row.workspaceId === args.workspaceId)
        && (args.agentId === undefined || row.agentId === args.agentId)
        && (args.channel === undefined || row.channel === args.channel)),
...
export const upsertItemInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const row = {
      ...
      primaryMarket: args.item.markets[0],
      primaryAsset: args.item.assets[0],
      primaryTimeframe: args.item.timeframes[0],
      primaryDirection: args.item.directions[0],
      normalizedRuleJson,
...
export const queryItemsInternal = internalQuery({
  args: {
    ...
    itemType: v.optional(v.string()), reviewStatus: v.optional(v.string()), securityStatus: v.optional(v.string()),
    market: v.optional(v.string()), asset: v.optional(v.string()), timeframe: v.optional(v.string()), direction: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    ...
    const base = args.market
      ? ctx.db.query("crystalResearchItems").withIndex("by_collection_market_review", (q: any) => q.eq("collectionId", args.collectionId).eq("primaryMarket", args.market).eq("reviewStatus", status))
      : args.asset
        ? ctx.db.query("crystalResearchItems").withIndex("by_collection_asset_review", (q: any) => q.eq("collectionId", args.collectionId).eq("primaryAsset", args.asset).eq("reviewStatus", status))
        : args.timeframe
          ? ctx.db.query("crystalResearchItems").withIndex("by_collection_timeframe_review", (q: any) => q.eq("collectionId", args.collectionId).eq("primaryTimeframe", args.timeframe).eq("reviewStatus", status))
          : args.direction
            ? ctx.db.query("crystalResearchItems").withIndex("by_collection_direction_review", (q: any) => q.eq("collectionId", args.collectionId).eq("primaryDirection", args.direction).eq("reviewStatus", status))
            : args.securityStatus
              ? ctx.db.query("crystalResearchItems").withIndex("by_collection_security_review", (q: any) => q.eq("collectionId", args.collectionId).eq("securityStatus", args.securityStatus).eq("reviewStatus", status))
              : args.itemType
    ...
    const page = await base.order("desc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return {
      items: page.page.filter((row: any) => row.userId === args.userId
        && isVisible(row, args, args.allowOwnerWide)
        && (args.itemType === undefined || row.itemType === args.itemType)
        && (args.securityStatus === undefined || row.securityStatus === args.securityStatus)
        && (args.market === undefined || row.markets.includes(args.market))
        && (args.asset === undefined || row.assets.includes(args.asset))
        && (args.timeframe === undefined || row.timeframes.includes(args.timeframe))
        && (args.direction === undefined || row.directions.includes(args.direction))),
      continueCursor: page.continueCursor, isDone: page.isDone,
    };
  },
});
  ...
  handler: async (ctx, args): Promise<{ items: any[]; count: number }> => {
    ...
    if (!vector && args.query?.trim()) {
      vector = await embedTextWithUserOpenRouter(ctx, args.query, {
        userId: args.userId, source: "research.semanticSearch",
      }) ?? undefined;
    }
...
export const queryRunsInternal = internalQuery({
  args: {
    ...
    itemId: v.optional(v.id("crystalResearchItems")), variantId: v.optional(v.id("crystalResearchVariants")), datasetId: v.optional(v.id("crystalResearchDatasets")),
    minRankingScore: v.optional(v.number()), minRobustnessScore: v.optional(v.number()), minSharpe: v.optional(v.number()),
    maxDrawdown: v.optional(v.number()), minTradeCount: v.optional(v.number()),
    startedFrom: v.optional(v.number()), startedTo: v.optional(v.number()), completedFrom: v.optional(v.number()), completedTo: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    const indexName = args.itemId ? "by_item_status"
      : args.variantId ? "by_variant_status"
        : args.datasetId ? "by_dataset_status"
          : args.sortBy === "robustnessScore" || args.minRobustnessScore !== undefined ? "by_collection_status_robustness"
            : args.sortBy === "sharpe" || args.minSharpe !== undefined ? "by_collection_status_sharpe"
              : args.sortBy === "maxDrawdown" || args.maxDrawdown !== undefined ? "by_collection_status_drawdown"
                : args.sortBy === "tradeCount" || args.minTradeCount !== undefined ? "by_collection_status_trades"
                  : args.sortBy === "startedAt" || args.startedFrom !== undefined || args.startedTo !== undefined ? "by_collection_status_started"
                    : args.sortBy === "completedAt" || args.completedFrom !== undefined || args.completedTo !== undefined ? "by_collection_status_completed"
                      : "by_collection_status_score";
    const page = await (ctx.db.query("crystalResearchRuns") as any)
      .withIndex(indexName, (q: any) => args.itemId
        ? q.eq("itemId", args.itemId).eq("status", status)
        : args.variantId
          ? q.eq("variantId", args.variantId).eq("status", status)
          : args.datasetId
            ? q.eq("datasetId", args.datasetId).eq("status", status)
            : q.eq("collectionId", args.collectionId).eq("status", status))
      .order("desc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return {
      runs: page.page.filter((row: any) => row.userId === args.userId
        && row.collectionId === args.collectionId
        && isVisible(row, args, args.allowOwnerWide)
        && (args.itemId === undefined || row.itemId === args.itemId)
        && (args.variantId === undefined || row.variantId === args.variantId)
        && (args.datasetId === undefined || row.datasetId === args.datasetId)
        && (args.minRankingScore === undefined || (row.rankingScore ?? -Infinity) >= args.minRankingScore)
        && (args.minRobustnessScore === undefined || (row.robustnessScore ?? -Infinity) >= args.minRobustnessScore)
        && (args.minSharpe === undefined || (row.sharpe ?? -Infinity) >= args.minSharpe)
        && (args.maxDrawdown === undefined || (row.maxDrawdown ?? Infinity) <= args.maxDrawdown)
        && (args.minTradeCount === undefined || (row.tradeCount ?? -Infinity) >= args.minTradeCount)
        && (args.startedFrom === undefined || row.startedAt >= args.startedFrom)
        && (args.startedTo === undefined || row.startedAt <= args.startedTo)
        && (args.completedFrom === undefined || (row.completedAt ?? Infinity) >= args.completedFrom)
        && (args.completedTo === undefined || (row.completedAt ?? -Infinity) <= args.completedTo)),
      continueCursor: page.continueCursor, isDone: page.isDone,
    };
  },
});
...
async function createJobImpl(ctx: any, args: any) {
  ...
  if (capacity.researchIngestRecordsPerJob !== null && args.totalCount > capacity.researchIngestRecordsPerJob) {
    throw new Error(`Research job exceeds capacity policy limit of ${capacity.researchIngestRecordsPerJob} records`);
  }
  ...
  const jobId = await ctx.db.insert("crystalResearchJobs", {
    ...
    preparedCount: 0, importedCount: 0, duplicateCount: 0, rejectedCount: 0, quarantinedCount: 0, failedCount: 0,
    createdAt: now, updatedAt: now,
  });
...
export const enqueueShardInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const shardId = await ctx.db.insert("crystalResearchJobShards", {
      ...
      progressCount: 0, totalCount: Math.max(0, Math.trunc(args.totalCount)),
      importedCount: 0, duplicateCount: 0, rejectedCount: 0, quarantinedCount: 0, failedCount: 0,
      createdAt: now, updatedAt: now,
    });
...
export const prepareIngestRecordInternal = internalMutation({
  args: {
    userId: v.string(), shardId: v.id("crystalResearchJobShards"), workerId: v.string(),
    recordType: v.string(), idempotencyKey: v.string(), payloadHash: v.string(), payloadJson: v.string(),
  },
  handler: async (ctx, args) => {
    const shard = await ctx.db.get(args.shardId);
    if (!shard || shard.userId !== args.userId || shard.status !== "leased" || shard.leaseOwner !== args.workerId) {
      throw new Error("Research shard lease not held by worker");
    }
    if ((shard.leaseExpiresAt ?? 0) <= Date.now()) throw new Error("Research shard lease expired");
    const job = await ctx.db.get(shard.jobId);
    if (!job || job.userId !== args.userId || job.status === "paused" || job.status === "cancelled" || job.status === "dead_letter") {
      throw new Error("Research job is not accepting ingest records");
    }
    const existing = await ctx.db.query("crystalResearchIngestRecords")
      .withIndex("by_job_idempotency", (q) => q.eq("jobId", shard.jobId).eq("idempotencyKey", args.idempotencyKey)).first();
    if (existing) {
      if (existing.payloadHash !== args.payloadHash) throw new Error("Idempotency key payload hash conflict");
      return { recordId: existing._id, collectionId: existing.collectionId, created: false, status: existing.status, resultingEntityId: existing.resultingEntityId };
    }
    if ((shard.progressCount ?? 0) >= shard.totalCount) throw new Error("Research shard manifest is already fully accounted");
    const now = Date.now();
    const recordId = await ctx.db.insert("crystalResearchIngestRecords", {
      userId: args.userId, collectionId: shard.collectionId, jobId: shard.jobId, shardId: shard._id,
      recordType: normalizedString(args.recordType, "recordType", 100),
      idempotencyKey: normalizedString(args.idempotencyKey, "idempotencyKey", 500),
      payloadHash: normalizedString(args.payloadHash, "payloadHash", 200),
      payloadJson: parseJsonObject(args.payloadJson, "payloadJson"), status: "prepared", createdAt: now, updatedAt: now,
    });
    await ctx.db.patch(job._id, { preparedCount: job.preparedCount + 1, updatedAt: now });
    return { recordId, collectionId: shard.collectionId, created: true, status: "prepared" };
  },
});

export const commitIngestRecordInternal = internalMutation({
  args: {
    userId: v.string(), recordId: v.id("crystalResearchIngestRecords"), workerId: v.string(),
    outcome: v.union(v.literal("imported"), v.literal("duplicate"), v.literal("rejected"), v.literal("quarantined"), v.literal("failed")),
    resultingEntityId: v.optional(v.string()), rejectionReason: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    const record = await ctx.db.get(args.recordId);
    if (!record || record.userId !== args.userId) throw new Error("Research ingest record not found");
    if (record.status !== "prepared") return { status: record.status, created: false, resultingEntityId: record.resultingEntityId };
    const shard = await ctx.db.get(record.shardId);
    if (!shard || shard.status !== "leased" || shard.leaseOwner !== args.workerId || (shard.leaseExpiresAt ?? 0) <= Date.now()) {
      throw new Error("Research shard lease not held by worker");
    }
    ...
    await ctx.db.patch(record._id, {
      status: args.outcome, resultingEntityId: args.resultingEntityId,
      rejectionReason: args.rejectionReason?.slice(0, 2_000), updatedAt: now,
    });
    const shardField = `${args.outcome}Count`;
    const jobField = `${args.outcome}Count`;
    await ctx.db.patch(shard._id, {
      [shardField]: ((shard as any)[shardField] ?? 0) + 1,
      progressCount: shard.progressCount + 1, updatedAt: now,
    } as any);
    await ctx.db.patch(job._id, { [jobField]: ((job as any)[jobField] ?? 0) + 1, updatedAt: now } as any);
    return { status: args.outcome, created: true, resultingEntityId: args.resultingEntityId };
  },
});
...
export const completeShardInternal = internalMutation({
  args: {
    userId: v.string(), shardId: v.id("crystalResearchJobShards"), workerId: v.string(),
    importedCount: v.optional(v.number()), duplicateCount: v.optional(v.number()), rejectedCount: v.optional(v.number()),
    quarantinedCount: v.optional(v.number()), failedCount: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    const authoritative = {
      importedCount: shard.importedCount ?? 0,
      duplicateCount: shard.duplicateCount ?? 0,
      rejectedCount: shard.rejectedCount ?? 0,
      quarantinedCount: shard.quarantinedCount ?? 0,
      failedCount: shard.failedCount ?? 0,
    };
    const accounted = Object.values(authoritative).reduce((sum, value) => sum + value, 0);
    if (!args.error && accounted !== shard.totalCount) {
      throw new Error(`Research shard accounting mismatch: manifest=${shard.totalCount}, committed=${accounted}`);
    }
    for (const [key, value] of Object.entries(authoritative)) {
      const submitted = (args as any)[key];
      if (submitted !== undefined && Math.trunc(submitted) !== value) {
        throw new Error(`Worker-supplied ${key} does not match committed ingest records`);
      }
    }
    await ctx.db.patch(args.shardId, {
      status: terminalStatus, progressCount: accounted, leaseOwner: undefined, leaseExpiresAt: undefined,
      lastError: args.error, updatedAt: now,
    });
...
export const getShardStatusInternal = internalQuery({
  args: { userId: v.string(), shardId: v.id("crystalResearchJobShards") },
  handler: async (ctx, args) => {
    const shard = await ctx.db.get(args.shardId);
    if (!shard || shard.userId !== args.userId) return null;
    return shard;
  },
});
...
export const listJobsInternal = internalQuery({
  args: {
    userId: v.string(), collectionId: v.id("crystalResearchCollections"), status: v.optional(v.string()),
    cursor: v.optional(v.string()), limit: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    await requireCollection(ctx, args.userId, args.collectionId);
    ...
    const page = await ctx.db.query("crystalResearchJobs")
      .withIndex("by_collection_status_updated", (q: any) => q.eq("collectionId", args.collectionId).eq("status", status))
      .order("desc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return { jobs: page.page.filter((row) => row.userId === args.userId), continueCursor: page.continueCursor, isDone: page.isDone };
  },
});

export const listJobShardsInternal = internalQuery({
  args: {
    userId: v.string(), jobId: v.id("crystalResearchJobs"), status: v.optional(v.string()),
    cursor: v.optional(v.string()), limit: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    const job = await ctx.db.get(args.jobId);
    ...
    const page = await ctx.db.query("crystalResearchJobShards")
      .withIndex("by_job_status_ordinal", (q: any) => q.eq("jobId", args.jobId).eq("status", status))
      .order("asc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return { shards: page.page.filter((row) => row.userId === args.userId), continueCursor: page.continueCursor, isDone: page.isDone };
  },
});

export const listRollupsInternal = internalQuery({
  args: {
    userId: v.string(), collectionId: v.id("crystalResearchCollections"), dimensionType: v.optional(v.string()),
    cursor: v.optional(v.string()), limit: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    await requireCollection(ctx, args.userId, args.collectionId);
    const limit = Math.min(Math.max(Math.trunc(args.limit ?? 25), 1), 100);
    const base = args.dimensionType
      ? ctx.db.query("crystalResearchRollups").withIndex("by_collection_dimension_score", (q: any) => q.eq("collectionId", args.collectionId).eq("dimensionType", args.dimensionType))
      : ctx.db.query("crystalResearchRollups").withIndex("by_user_collection", (q: any) => q.eq("userId", args.userId).eq("collectionId", args.collectionId));
    const page = await base.order("desc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return { rollups: page.page.filter((row: any) => row.userId === args.userId), continueCursor: page.continueCursor, isDone: page.isDone };
  },
});

export const listPromotionsInternal = internalQuery({
  args: {
    userId: v.string(), collectionId: v.id("crystalResearchCollections"), status: v.optional(v.string()),
    cursor: v.optional(v.string()), limit: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    await requireCollection(ctx, args.userId, args.collectionId);
    const limit = Math.min(Math.max(Math.trunc(args.limit ?? 25), 1), 100);
    const status = args.status ?? "promoted";
    const page = await ctx.db.query("crystalResearchPromotions")
      .withIndex("by_collection_status", (q: any) => q.eq("collectionId", args.collectionId).eq("status", status))
      .order("desc").paginate({ cursor: args.cursor ?? null, numItems: limit });
    return {
      promotions: page.page.filter((row) => row.userId === args.userId).map(({ knowledgeBaseId, memoryId, ...row }) => ({
        ...row,
        hasKnowledgeBaseProjection: knowledgeBaseId !== undefined,
        hasMemoryProjection: memoryId !== undefined,
      })),
      continueCursor: page.continueCursor, isDone: page.isDone,
    };
  },
...
export const getPromotionInternal = internalQuery({
  args: { userId: v.string(), promotionId: v.id("crystalResearchPromotions") },
  handler: async (ctx, args) => {
    const promotion = await ctx.db.get(args.promotionId);
    return promotion?.userId === args.userId ? promotion : null;
  },
});
...
export const getTraceInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const [rollups, promotions, runRelations, itemRelations, rejections] = await Promise.all([
      ctx.db.query("crystalResearchRollups").withIndex("by_user_collection", (q) => q.eq("userId", args.userId).eq("collectionId", run.collectionId)).take(100),
      ctx.db.query("crystalResearchPromotions").withIndex("by_user_collection", (q) => q.eq("userId", args.userId).eq("collectionId", run.collectionId)).take(100),
      ctx.db.query("crystalResearchRelations").withIndex("by_from", (q) => q.eq("collectionId", run.collectionId).eq("fromType", "run").eq("fromId", String(run._id))).take(100),
      ctx.db.query("crystalResearchRelations").withIndex("by_from", (q) => q.eq("collectionId", run.collectionId).eq("fromType", "item").eq("fromId", String(item._id))).take(100),
      ctx.db.query("crystalResearchRejections").withIndex("by_collection_created", (q) => q.eq("collectionId", run.collectionId)).order("desc").take(100),
    ]);
    const applicablePromotions = promotions.filter((row) => row.itemId === item._id || row.runId === run._id);
    const applicableRollups = rollups.filter((row) => row.dimensionKey === String(item._id)
      || row.dimensionKey === String(variant._id)
      || row.dimensionKey === variant.market
      || row.dimensionKey === variant.timeframe
      || row.dimensionType === "survival_funnel");
    const gateVerdicts = {
      itemQualified: item.reviewStatus === "qualified",
      itemSecurityClean: item.securityStatus === "clean",
      runCompleted: run.status === "completed",
      datasetCausalityValid: dataset.causalityStatus === "valid",
      sourcesAccepted: sources.every((source) => source?.licenseDisposition === "acceptable"
        && source?.securityDisposition === "clean" && source?.quarantineState === "released"),
      artifactsAccepted: artifacts.every((artifact) => artifact?.status === "accepted"),
    };
    return {
      run, item, variant, dataset,
      sources: sources.filter((row) => row?.userId === args.userId).map(({ storageId: _storageId, ...row }: any) => row),
      artifacts: artifacts.filter((row) => row?.userId === args.userId).map(({ storageId: _storageId, ...row }: any) => ({
        ...row,
        trustedDisposition: row.status === "accepted" ? "trusted" : "untrusted",
      })),
      rollups: applicableRollups,
      funnelStage: applicableRollups.find((row) => row.dimensionType === "survival_funnel") ?? null,
      gateVerdicts,
      relations: [...runRelations, ...itemRelations],
      rejectionHistory: rejections.filter((row) => row.idempotencyKey === run.idempotencyKey || row.idempotencyKey === item.idempotencyKey),
      promotion: applicablePromotions[0]
        ? {
          status: applicablePromotions[0].status,
          paperTradingState: applicablePromotions[0].paperTradingState,
          eligible: applicablePromotions[0].paperTradingState === "eligible" || applicablePromotions[0].paperTradingState === "active",
        }
        : { status: "not_promoted", paperTradingState: "ineligible", eligible: false },
      qualified: Object.values(gateVerdicts).every(Boolean),
    };
  },
});
...
export const claimPromotionInternal = internalMutation({
  args: {
    userId: v.string(), collectionId: v.id("crystalResearchCollections"), itemId: v.id("crystalResearchItems"),
    runId: v.id("crystalResearchRuns"), datasetId: v.id("crystalResearchDatasets"), dedupeKey: v.string(), reason: v.string(),
  },
  handler: async (ctx, args) => {
    const existing = await ctx.db.query("crystalResearchPromotions")
      .withIndex("by_collection_dedupe", (q) => q.eq("collectionId", args.collectionId).eq("dedupeKey", args.dedupeKey)).first();
    if (existing) return { claimed: false, promotion: existing };
    const now = Date.now();
    const promotionId = await ctx.db.insert("crystalResearchPromotions", {
      userId: args.userId, collectionId: args.collectionId, itemId: args.itemId, runId: args.runId,
      datasetId: args.datasetId, dedupeKey: args.dedupeKey, status: "pending", reason: args.reason,
      paperTradingState: "ineligible", lineageJson: "{}", createdAt: now, updatedAt: now,
    });
    return { claimed: true, promotion: await ctx.db.get(promotionId) };
  },
});

export const failPromotionInternal = internalMutation({
  args: { userId: v.string(), promotionId: v.id("crystalResearchPromotions"), reason: v.string() },
  handler: async (ctx, args) => {
    const promotion = await ctx.db.get(args.promotionId);
    if (!promotion || promotion.userId !== args.userId) return;
    await ctx.db.patch(args.promotionId, { status: "failed", reason: args.reason.slice(0, 2_000), updatedAt: Date.now() });
  },
});
...
export const getActivePromotionByDedupeInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    const promotion = await ctx.db.query("crystalResearchPromotions")
      ...
      .first();
    if (!promotion || promotion.userId !== args.userId || promotion.status !== "promoted" || !promotion.knowledgeBaseId || !promotion.memoryId) return null;
...
async function promoteImpl(ctx: any, args: any): Promise<any> {
  ...
  const claim: any = await ctx.runMutation((internal as any).crystal.research.claimPromotionInternal, {
    userId: args.userId, collectionId: args.collectionId, itemId: args.itemId, runId: args.runId,
    datasetId: args.datasetId, dedupeKey, reason: normalizedString(args.reason, "reason", 2_000),
  });
  if (!claim.claimed) {
    const promotion = claim.promotion;
    if (promotion?.status === "promoted" && promotion.knowledgeBaseId && promotion.memoryId) {
      return { promotionId: promotion._id, knowledgeBaseId: promotion.knowledgeBaseId, memoryId: promotion.memoryId, dedupeKey };
    }
    return { promotionId: promotion._id, dedupeKey, status: promotion.status, retryable: promotion.status === "pending" };
  }
  const claimedPromotionId = claim.promotion._id as Id<"crystalResearchPromotions">;
  try {
  ...
  return { promotionId, knowledgeBaseId, memoryId: inserted.memoryId, dedupeKey };
    ...
    await ctx.runMutation((internal as any).crystal.research.failPromotionInternal, {
      userId: args.userId,
      promotionId: claimedPromotionId,
