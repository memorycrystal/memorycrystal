...
const GRAPH_STATUS_COUNT_CAP = 1_000;
...
export const getUserGraphStatus = internalQuery({
  ...
  handler: async (ctx, { userId }) => {
    const nodes = await ctx.db
      ...
      .take(GRAPH_STATUS_COUNT_CAP + 1);

    const relations = await ctx.db
      .query("crystalRelations")
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .take(GRAPH_STATUS_COUNT_CAP + 1);
    const nodeCountLowerBound = nodes.length > GRAPH_STATUS_COUNT_CAP;
    const relationCountLowerBound = relations.length > GRAPH_STATUS_COUNT_CAP;
    const displayedNodeCount = Math.min(nodes.length, GRAPH_STATUS_COUNT_CAP);
    const displayedRelationCount = Math.min(relations.length, GRAPH_STATUS_COUNT_CAP);

    // Read graph coverage from the dashboard aggregate. This is one small
    // document, not a scan over embedding-heavy memory rows.
    const totals = { activeMemories: 0, totalMemories: 0, activeMemoriesByStore: {}, activeStoreCount: 0 };
    const totalMemoryCount = totals.activeMemories;
    const enrichedCount = totals.enrichedMemories;
    const pendingCount = totals.graphEligiblePendingMemories;
    const skippedCount = totals.graphSkippedMemories;
    const actionableMemoryCount = Math.max(0, enrichedCount + pendingCount);
    const overallEnrichmentPercent =
      totalMemoryCount > 0 ? Math.round((enrichedCount / totalMemoryCount) * 100) : 0;
    const actionableEnrichmentPercent =
      actionableMemoryCount > 0 ? Math.round((enrichedCount / actionableMemoryCount) * 100) : 0;
    const activeKnowledgeBases = await ctx.db
    ...
    return {
      totalNodes: displayedNodeCount,
      totalRelations: displayedRelationCount,
      countsAreLowerBounds: {
        totalNodes: nodeCountLowerBound,
        totalRelations: relationCountLowerBound,
      },
      enrichedMemories: enrichedCount,
      totalMemories: totalMemoryCount,
      enrichmentPercent: overallEnrichmentPercent,
      overallEnrichmentPercent,
      actionableEnrichmentPercent,
      graphCoverage: {
        activeMemoryCount: totalMemoryCount,
        eligibleMemoryCount: actionableMemoryCount,
        actionableMemoryCount,
        enrichedMemoryCount: enrichedCount,
        pendingMemoryCount: pendingCount,
        skippedMemoryCount: skippedCount,
        enrichmentPercent: actionableEnrichmentPercent,
        overallEnrichmentPercent,
        actionableEnrichmentPercent,
        activeMemoriesByStore: totals.activeMemoriesByStore,
      },
      knowledgeBaseCoverage: {
        activeKnowledgeBaseCount: activeKnowledgeBases.length,
        activeKnowledgeBaseCountCapped: activeKnowledgeBases.length === 100,
        activeKnowledgeBaseMemoryCount: activeKnowledgeBases.reduce(
          (sum, knowledgeBase) => sum + (knowledgeBase.memoryCount ?? 0),
          0,
        ),
      },
      backfill: {
        memoryGraphAction: "crystal.graphEnrich.backfillGraphEnrichment",
        knowledgeBaseGraphAction:
          "crystal.knowledgeBases.backfillKBGraphEnrichment",
      },
    };
  },
});
