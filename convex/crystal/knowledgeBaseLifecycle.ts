...
export const startInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing && ["queued", "running", "paused"].includes(existing.status)) {
      if (args.frontendJobId && !existing.frontendJobId) {
        await ctx.db.patch(existing._id, { frontendJobId: args.frontendJobId });
      }
      return {
        jobId: existing._id,
        frontendJobId: existing.frontendJobId ?? args.frontendJobId,
        created: false,
      };
    }
    const now = Date.now();
    ...
    return { jobId, frontendJobId: args.frontendJobId, created: true };
  },
});
...
export const advanceInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (job.frontendJobId) {
      const nextStatus = args.status ?? job.status;
      const processedCount =
        job.processedCount + Math.max(0, Math.trunc(args.processedDelta ?? 0));
      const terminal = nextStatus === "completed" || nextStatus === "failed";
      const crossedProgressBoundary =
        Math.floor(processedCount / PAGE_SIZE) > Math.floor(job.processedCount / PAGE_SIZE);
      if (job.status === "queued" || terminal || crossedProgressBoundary) {
        const frontendJob = await ctx.db.get(job.frontendJobId);
        if (frontendJob && frontendJob.userId === job.userId) {
          await ctx.db.patch(frontendJob._id, {
            status: terminal ? nextStatus : "running",
            ...
            progressLabel:
              nextStatus === "completed"
                ...
                : nextStatus === "failed"
                  ? "Failed"
                  : `${processedCount.toLocaleString()} chunks processed`,
            error: args.error?.slice(0, 1000),
            startedAt: frontendJob.startedAt ?? now,
            completedAt: terminal ? frontendJob.completedAt ?? now : frontendJob.completedAt,
            updatedAt: now,
          });
        }
      }
