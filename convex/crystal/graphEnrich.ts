...
async function callOpenRouterGraphExtractor(
  ctx: any,
  userId: string,
  prompt: string,
  apiKey: string,
  keyLast4: string,
  model: string,
): Promise<{ text: string; errorReason?: string }> {
  try {
    const result = await requestOpenRouter(ctx, {
      ...
      source: "graphEnrich.callOpenRouterGraphExtractor",
      body: {
        model,
    ...
    if (!result.ok) {
      console.log(`[graphEnrich] OpenRouter error ${result.status}: ${result.errorMessage ?? "unknown"}`);
      return { text: "", errorReason: `openrouter_${result.status}` };
    }
...
export const listUnenrichedMemoriesForUser = internalQuery({
  ...
  handler: async (ctx, args) => {
    const falseRows = await ctx.db
      .query("crystalMemories")
      .withIndex("by_graph_enriched", (q) =>
        q.eq("graphEnriched", false).eq("userId", args.userId).eq("archived", false)
      )
      .take(args.limit);
    ...
    const undefinedRows = await ctx.db
      .query("crystalMemories")
      .withIndex("by_graph_enriched", (q) =>
        q.eq("graphEnriched", undefined).eq("userId", args.userId).eq("archived", false)
      )
...
export const listUnenrichedMemoriesPageForUser = internalQuery({
  ...
  handler: async (ctx, args): Promise<GraphBackfillPage> => {
    ...
    // archived + enrichmentSkippedReason are part of by_graph_enriched, so this
    // is a pure index seek: permanently-skipped rows (max_attempts_exceeded,
    // sensory_skip, etc.) are never read. The previous post-index .filter()
    // re-read every skipped row on every hourly tick (2026-07-07 cost audit).
    const page: any = await ctx.db
      .query("crystalMemories")
      .withIndex("by_graph_enriched", (q) =>
        q
          .eq("graphEnriched", graphEnrichedValue)
          .eq("userId", args.userId)
          .eq("archived", false)
          .eq("enrichmentSkippedReason", undefined)
      )
      .paginate({
...
export const enrichMemoryGraph: ReturnType<typeof internalAction> = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (openRouterKey?.apiKey) {
      const result = await callOpenRouterGraphExtractor(
        ctx,
        args.userId,
        prompt,
        openRouterKey.apiKey,
        openRouterKey.apiKey.slice(-4),
        openRouterModel,
      );
    ...
    if (!rawContent) {
      // ILL-184: a missing credential is an explicit, recorded failure at the
      // choke point so the alert policy can notify the user.
      await recordMissingOpenRouterKey(ctx, { userId: args.userId });
  ...
  handler: async (ctx, args): Promise<{
    ...
    const resolveUserCap = async (userId: string) => {
      ...
      const capacity = await ctx.runQuery(
        (internal as any).crystal.capacityPolicy.getForUser,
        { userId },
      ) as { graphEnrichmentsPerHour?: number | null } | null;
      if (capacity?.graphEnrichmentsPerHour === null) {
        capCache.set(userId, globalTarget);
        return globalTarget;
      }
      if (capacity?.graphEnrichmentsPerHour === 0) {
        capCache.set(userId, 0);
        return 0;
      }
      ...
      const cap = clamp(
        Math.min(
          ...
          capacity?.graphEnrichmentsPerHour ?? DEFAULT_GRAPH_ENRICHMENT_HOURLY_CAP,
        ),
        1,
