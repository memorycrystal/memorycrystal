...
async function callOpenRouterGraphExtractor(prompt: string, apiKey: string): Promise<{ text: string; errorReason?: string }> {
  try {
...
export const enrichMemoryGraph: ReturnType<typeof internalAction> = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (openRouterKey?.apiKey) {
      const result = await callOpenRouterGraphExtractor(prompt, openRouterKey.apiKey);
