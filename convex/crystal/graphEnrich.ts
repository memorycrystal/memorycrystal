...
const OPENROUTER_GRAPH_MODEL = "google/gemini-2.0-flash-001";
...
const GRAPH_BACKFILL_GLOBAL_SUCCESS_CAP = 500;
      ...
      try {
        ...
        for (const row of drained) {
          ...
          if (result?.enriched) {
            ...
            if (result?.deterministicSkip === true || DETERMINISTIC_SKIP_REASONS.has(reason)) {
              consecutiveFailures = 0;
            } else {
