  ...
  handler: async (ctx, args) => {
    ...
    for (const window of windows) {
      try {
        ...
        const extracted = (args.mockMemories ?? await extractWindowMemories(window, openRouterCredential.apiKey as string))
...
export const runOnDemandLtmExtraction = internalAction({
  ...
  handler: async (ctx, args): Promise<OnDemandLtmExtractionResult> => {
    ...
    if (openRouterCredential.source !== "personal") {
      ...
      if (!throttle.allowed) {
        ...
        return { scanned: 0, windows: 0, inserted: 0, deduped: 0, skipped: 1, blockedContentSkipped: 0, errors: 0, reason: throttle.reason, dryRun: args.dryRun ?? false };
      }
    }
    if (!args.dryRun) {
