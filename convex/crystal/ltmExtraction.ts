...
const MISSING_USER_OPENROUTER_KEY_REASON = "OPENROUTER_API_KEY not set for user";
...
const PROACTIVE_DISTILLATION_TRIGGER = "message_distillation";
const DEFAULT_PROACTIVE_MIN_AGE_HOURS = 24;
const DEFAULT_PROACTIVE_USERS_LIMIT = 15;
const DEFAULT_PROACTIVE_MESSAGES_PER_USER = 40;
const MAX_PROACTIVE_MIN_AGE_HOURS = 24 * 30;
...
type OpenRouterCredential = { apiKey: string | null; source: "personal" | "shared" | null; keyPrefix?: string | null };
...
type ProactiveDistillationConfig = {
  enabled: boolean;
  allowlist: string[];
  allowAll: boolean;
  minAgeHours: number;
...
export function parseProactiveDistillationConfig(env: {
  MEMORY_CRYSTAL_PROACTIVE_LTM_ENABLED?: string;
  MEMORY_CRYSTAL_PROACTIVE_LTM_ALLOWLIST?: string;
  MEMORY_CRYSTAL_PROACTIVE_LTM_MIN_AGE_HOURS?: string;
  MEMORY_CRYSTAL_PROACTIVE_LTM_USERS_LIMIT?: string;
  MEMORY_CRYSTAL_PROACTIVE_LTM_MESSAGES_PER_USER?: string;
}): ProactiveDistillationConfig {
  const enabledValue = (env.MEMORY_CRYSTAL_PROACTIVE_LTM_ENABLED ?? "").trim().toLowerCase();
  const enabled = ["1", "true", "on", "enabled"].includes(enabledValue);
  const rawAllowlist = (env.MEMORY_CRYSTAL_PROACTIVE_LTM_ALLOWLIST ?? "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  const allowAll = rawAllowlist.includes("*");
  const allowlist = Array.from(new Set(rawAllowlist.filter((value) => value !== "*")));
  const minAgeHours = clampInt(
    Number(env.MEMORY_CRYSTAL_PROACTIVE_LTM_MIN_AGE_HOURS),
    1,
    MAX_PROACTIVE_MIN_AGE_HOURS,
    DEFAULT_PROACTIVE_MIN_AGE_HOURS,
  );
  const usersLimit = clampInt(
    Number(env.MEMORY_CRYSTAL_PROACTIVE_LTM_USERS_LIMIT),
    1,
    MAX_USERS_PER_RUN,
    DEFAULT_PROACTIVE_USERS_LIMIT,
  );
  const messagesPerUser = clampInt(
    Number(env.MEMORY_CRYSTAL_PROACTIVE_LTM_MESSAGES_PER_USER),
    1,
    MAX_MESSAGES_PER_USER,
    DEFAULT_PROACTIVE_MESSAGES_PER_USER,
  );
  return { enabled, allowlist, allowAll, minAgeHours, usersLimit, messagesPerUser };
}
...
export const buildExtractionPrompt = (
  ...
  const priorSection = recentDigests.length > 0
    ? `\nAlready extracted from earlier turns in this same session and channel:\n${recentDigests
        ...
        .join("\n")}\n`
    : "";
  return `You extract durable long-term memories from short-term conversation logs.
...
async function extractWindowMemories(
  ...
  const prompt = buildExtractionPrompt(window, recentDigests);
  ...
  if (!response.ok) {
    throw new Error(`OpenRouter extraction failed: ${response.status} ${JSON.stringify(payload?.error ?? null).slice(0, 200)}`);
  }
      ...
      try {
        ...
        const extractionCall: ExtractionCallResult = args.mockMemories
          ...
          : await extractWindowMemories(
              window,
              openRouterCredential.apiKey as string,
              recentDigests,
            );
        ...
        const extracted = extractionCall.memories.slice(0, memoryCapForWindow(window));
...
export const runOnDemandLtmExtraction = internalAction({
  ...
  handler: async (ctx, args): Promise<OnDemandLtmExtractionResult> => {
    ...
    for (const window of windows) {
      let phase: LtmTelemetryPhase = "durable_signal";
      try {
        if (!hasDurableSignal(window)) {
          skipped += 1;
          lowSignalSkipped += 1;
          ...
          if (!args.dryRun) {
            phase = "mark_messages";
            await markWindowExtracted(ctx, window, "low_signal");
          }
          continue;
        }
...
export const getProactiveDistillationCandidates = internalQuery({
  args: {
    userId: v.string(),
  ...
  handler: async (ctx, args): Promise<MessageRecord[]> => {
    const limit = clampInt(args.limit, 1, MAX_MESSAGES_PER_USER, DEFAULT_PROACTIVE_MESSAGES_PER_USER);
    return await ctx.db
      .query("crystalMessages")
      .withIndex("by_user_ltm_extracted_time", (q) =>
        q
...
export const runProactiveMessageDistillation = internalAction({
  args: {
  ...
  handler: async (ctx, args): Promise<any> => {
    const config = parseProactiveDistillationConfig(
      typeof process !== "undefined" ? (process.env as any) : {},
    );
    if (!config.enabled) {
      console.log("[proactive-ltm] disabled by MEMORY_CRYSTAL_PROACTIVE_LTM_ENABLED");
      return { enabled: false, reason: "disabled", users: 0, results: [] };
    }
    if (!config.allowAll && config.allowlist.length === 0) {
      console.log("[proactive-ltm] no pilot users configured");
      return { enabled: true, reason: "empty_allowlist", users: 0, results: [] };
    }
    if (args.userId && !config.allowAll && !config.allowlist.includes(args.userId)) {
      return { enabled: true, reason: "user_not_allowlisted", users: 0, results: [] };
    }
    ...
    const messagesPerUser = clampInt(
      args.messagesPerUser,
      1,
      MAX_MESSAGES_PER_USER,
      config.messagesPerUser,
    );
    const beforeTimestamp = now - config.minAgeHours * 60 * 60 * 1000;

    let allUserIds: string[];
    if (args.userId) {
      allUserIds = [args.userId];
    } else if (!config.allowAll) {
      allUserIds = config.allowlist;
    } else {
      allUserIds = [];
    ...
    const start = args.userId || uniqueUserIds.length <= usersLimit
      ? 0
      : (Math.floor(now / CRON_ROTATION_MS) * usersLimit) % uniqueUserIds.length;
    const userIds = args.userId
      ? uniqueUserIds
      : [...uniqueUserIds.slice(start), ...uniqueUserIds.slice(0, start)].slice(0, usersLimit);
    ...
    for (const userId of userIds) {
      const tierInfo = await ctx.runQuery(
        ...
        { userId },
      ) as { tier?: string; sensoryRawTtlDays?: number } | null;
      const credential = await resolveUserOpenRouterCredential(ctx, userId);
      const keySource = credential.source === "personal"
        ? "user_byok"
        : credential.source === "shared"
          ? "platform"
          : "unknown";
      const payer = credential.source === "personal"
        ? "user"
        : credential.source === "shared"
          ? "company"
          : "unknown";
      const reflectionRunId = await ctx.runMutation(
        ...
        {
          userId,
          trigger: PROACTIVE_DISTILLATION_TRIGGER,
          ...
          metadata: JSON.stringify({
            source: "proactive_message_distillation",
            beforeTimestamp,
      ...
      try {
        const candidates = await ctx.runQuery(
          (internal as any).crystal.ltmExtraction.getProactiveDistillationCandidates,
          { userId, beforeTimestamp, limit: messagesPerUser },
        ) as MessageRecord[];
        const scoped = candidates.filter((message) =>
          isProactiveDistillationChannelEligible(message.channel)
        );
        const unscoped = candidates.filter((message) =>
          !isProactiveDistillationChannelEligible(message.channel)
        );
        ...
        const extraction = scoped.length > 0
          ? await ctx.runAction(
            ...
            ) as OnDemandLtmExtractionResult
          : {
              scanned: 0,
              windows: 0,
              inserted: 0,
        ...
        const messagesProcessed = unscoped.length + extraction.scanned;
        const messagesDiscarded = unscoped.length + extraction.discardedMessages;
        ...
        results.push({ userId, reflectionRunId, error: errorMessage });
      }
    }
