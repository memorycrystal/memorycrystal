...
export const searchMemoriesByText = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const filterToKb = args.knowledgeBaseId
      ? (collection: Array<any>) =>
          collection.filter((doc) => String(doc.knowledgeBaseId ?? "") === args.knowledgeBaseId)
      : (collection: Array<any>) => collection;
    // Over-fetch when KB-scoped so the post-filter can still yield `limit` hits.
    const searchTake = args.knowledgeBaseId ? Math.min(limit * 4, 100) : limit;
    const [contentResults, recallTextResults, titleResults] = await Promise.all([
      ...
      ctx.db
        ...
        .withSearchIndex("search_recall_text", (q) =>
          q.search("recallText", args.query).eq("userId", args.userId).eq("archived", false)
        )
        .take(searchTake)
        .catch((error) => {
    ...
    const scopedContentResults = filterToKb(contentResults).slice(0, limit);
    const scopedRecallTextResults = filterToKb(recallTextResults).slice(0, limit);
    const scopedTitleResults = filterToKb(titleResults).slice(0, limit);
...
export const recallMemories = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Peer-first fallback: recall is a peer-facing action. If a caller omits
    // `channel`, we fail-closed by passing "" — isKnowledgeBaseVisibleToAgent
    // will reject any KB with a concrete peer scope. Using the management
    // sentinel here would silently upgrade unscoped callers (e.g. a plugin
    // that forgot to forward channel) to management-level visibility, which
    // was the 0.7.15 B2 regression we fixed.
    ...
    const [vectorResults, textSearchResults, traceMatches] = await Promise.all([
      ...
      textQuery.trim().length > 0 && !textBudget?.emergency
      ...
      textQuery.trim().length > 0 && !recallBudget?.emergency
    ...
    const rankedFiltered = rankRecallCandidates(
      rawResults
        ...
        .map((candidate) => ({
          ...
          content: getMemoryEffectiveText(candidate),
