...
import {
  CURRENT_INTENT_FRESHNESS_BOOST,
  CURRENT_INTENT_FRESHNESS_CEIL,
  RECALL_SCORE_FLOOR,
...
import { organicTracesEnabled } from "./organic/flags";
...
// ILL-104 — provenance/staleness math lives in recallProvenance.ts (imported at
// top), shared with the /api/mcp/recall (mcpRecall) path so both surfaces flag
// identical status to the model. These only *flag* — they do not change ranking.
...
type RecallCandidateDocument = {
  ...
  chunkKind?: string;
  // ILL-104 — already present on the hydrated doc (getMemoriesByIds returns full
  // rows); exposed on the type so recall can surface provenance/staleness.
  source?: string;
...
/**
 * ILL-104 — batched contradiction lookup for the recall payload. Given the
 * recalled memories (id + createdAt), returns the subset that are the **older
 * side of an unresolved `contradicts` edge**: a memory that contradicts an
 * ACTIVE (non-archived) counterpart that is strictly newer. The newer side and
 * memories whose only contradiction counterpart is archived/superseded (i.e.
 * resolved) are not flagged. Batched (O(K) index reads via Promise.all, then a
 * single batched get of counterparts), mirroring getNodesForMemories — no N+1
 * per-memory query loop (ILL-104 AC-4).
 */
export const getContradictedOlderSideMemoryIds = internalQuery({
  args: {
    userId: v.string(),
    memories: v.array(v.object({ memoryId: v.string(), createdAt: v.number() })),
  },
  handler: async (ctx, args) => {
    if (args.memories.length === 0) return [] as string[];
    // 1) Gather contradicts edges touching each recalled memory (both directions).
    const perMemory = await Promise.all(
      args.memories.map(async (m) => {
        const [fromEdges, toEdges] = await Promise.all([
          ctx.db.query("crystalAssociations").withIndex("by_from", (q) => q.eq("fromMemoryId", m.memoryId as any)).collect(),
          ctx.db.query("crystalAssociations").withIndex("by_to", (q) => q.eq("toMemoryId", m.memoryId as any)).collect(),
        ]);
        const edges = [...fromEdges, ...toEdges].filter(
          (e) => e.relationshipType === "contradicts" && (e.userId === undefined || e.userId === args.userId),
        );
        return { memoryId: m.memoryId, createdAt: m.createdAt, edges };
      }),
    );
    // 2) Batch-fetch the counterpart memories (createdAt + archived + ownership).
    const counterpartIds = new Set<string>();
    for (const item of perMemory) {
      for (const e of item.edges) {
        const other = String(e.fromMemoryId) === item.memoryId ? String(e.toMemoryId) : String(e.fromMemoryId);
        if (other !== item.memoryId) counterpartIds.add(other);
      }
    }
    const counterpartDocs = await Promise.all([...counterpartIds].map((id) => ctx.db.get(id as any)));
    const counterpartById = new Map<string, { createdAt?: number; archived?: boolean; supersededByMemoryId?: string }>();
    for (const doc of counterpartDocs) {
      if (doc && (doc as any).userId === args.userId) {
        counterpartById.set(String((doc as any)._id), doc as any);
      }
    }
    // 3) Flag the older side of any edge to an active, strictly-newer counterpart.
    //    "Active" = not archived and not itself superseded (a superseded/archived
    //    counterpart means the contradiction is already resolved).
    const flagged = new Set<string>();
    for (const item of perMemory) {
      for (const e of item.edges) {
        const otherId = String(e.fromMemoryId) === item.memoryId ? String(e.toMemoryId) : String(e.fromMemoryId);
        const other = counterpartById.get(otherId);
        if (!other || other.archived || other.supersededByMemoryId) continue; // resolved / one-sided → not flagged
        if (typeof other.createdAt === "number" && other.createdAt > item.createdAt) {
          flagged.add(item.memoryId);
        }
      }
    }
    return [...flagged];
...
export const recallMemories = action({
  ...
  handler: async (ctx, args) => {
    ...
    // ILL-104 — annotate a result set with provenance & staleness the consuming
    // model can weight/cite: age + source (always) and stale / contradicted /
    // superseded flags (only when true). The contradiction check is a single
    // batched query over the whole set (no N+1). Additive: unknown fields are
    // ignored by older clients.
    const annotateProvenance = async (results: RecallResult[]): Promise<RecallResult[]> => {
      ...
      const contradictedIds = new Set<string>(
        (await ctx
          .runQuery(internal.crystal.recall.getContradictedOlderSideMemoryIds, {
            userId,
            memories: results.map((r) => ({ memoryId: r.memoryId, createdAt: r.createdAt ?? now })),
          })
          .catch(() => [] as string[])) as string[],
      );
      return results.map((r) => applyProvenanceToMemory(r, contradictedIds, now));
    };
    ...
    // ILL-105 (B) — query-intent recency: when the query asks for the CURRENT
    // state ("current/now/latest/still/..."), widen the freshness weight for this
    // query only (bounded by a ceiling) so a fresh answer wins decisively over a
    // strong-but-stale one. Absent those tokens, ranking is unchanged. This is a
    // per-query boost layered on the per-user policy weights, not a stored change.
    if (hasCurrentIntent(args.query)) {
      scoringWeights.freshnessWeight = Math.min(
        scoringWeights.freshnessWeight * CURRENT_INTENT_FRESHNESS_BOOST,
        CURRENT_INTENT_FRESHNESS_CEIL,
      );
    }
    ...
    const [vectorResults, textSearchResults, traceMatches] = await Promise.all([
      ...
      // Prospective traces are opt-in (CRYSTAL_TRACES_ENABLED=1). Default-off
      // skips the extra action, embed, and vector search — they have never
      // validated (0/7,295) and tax every normal-budget recall.
      organicTracesEnabled() &&
      textQuery.trim().length > 0 &&
      reachPolicy.budgetLevel === "normal"
    ...
    // ILL-105 (A) — resolve, once and batched, which candidates are the older
    // side of an unresolved contradiction, so scoreRecallCandidate can fade them
    // below their fresher/higher-confidence counterpart. Computed over the whole
    // hydrated candidate set (superset of the ranked set) so any contradicted
    // candidate is faded before the diversity/limit cut. Reuses the ILL-104
    // batched internalQuery (no N+1).
    const contradictedOlderSideIds = new Set<string>(
      (await ctx
        .runQuery(internal.crystal.recall.getContradictedOlderSideMemoryIds, {
          userId,
          memories: rawResults.map((candidate) => ({
            memoryId: String(candidate._id),
    ...
    const rankedFiltered = rankRecallCandidates(
      rawResults
        ...
        .map((candidate) => ({
          ...
          lastAccessedAt: candidate.lastAccessedAt,
          createdAt: candidate.createdAt,
          // ILL-104 — carry provenance through ranking to the output builders.
        ...
        })),
      {
        ...
        contradictedOlderSideIds,
        scoreFloor: RECALL_SCORE_FLOOR,
      }
    )
      .map((result) => {
      ...
      .filter((result) => result.scoreValue >= RECALL_SCORE_FLOOR);
    ...
    const ranked = diversityFilter(rankedFiltered, normalizedLimit)
      ...
      .map((result) => ({
        ...
        // ILL-104 — provenance carried to the annotation step below.
        createdAt: (result as { createdAt?: number }).createdAt,
        source: (result as { source?: string }).source,
        supersededByMemoryId: (result as { supersededByMemoryId?: string }).supersededByMemoryId,
      } as RecallResult));
    ...
    if (!includeAssociations || ranked.length === 0) {
      finalMemories = await annotateProvenance(finalMemories);
    ...
    for (let i = 0; i < finalMemories.length; i++) {
      ...
      for (const assoc of assocCandidates) {
        ...
        expanded.push({
          ...
          // ILL-104 — provenance for association-expanded entries too.
          createdAt: candidate.createdAt,
          source: candidate.source,
          supersededByMemoryId: candidate.supersededByMemoryId,
        });
    ...
    const memories = await annotateProvenance(
      dedupeById([...finalMemories, ...expanded])
