...
// limit and the recall mode has no preset. Bumped 8 -> 10 for richer context.
// Overridable via MEMORY_CRYSTAL_MAX_MEMORIES (clamped to [minLimit, maxLimit]).
 ...
 */
const resolveDefaultLimit = (): number => {
...
const isKnowledgeBaseCandidateVisible = (
  ...
  return isKnowledgeBaseVisibleToAgent(knowledgeBase, effectiveAgentId, queryChannel ?? "");
};
      ...
      rawResults
        .filter((candidate) => {
          ...
          if (candidate.knowledgeBaseId) {
            ...
            if (!knowledgeBase || !isKnowledgeBaseCandidateVisible(knowledgeBase, { agentId: explicitAgentId, channel: args.channel })) {
              return false;
            }
