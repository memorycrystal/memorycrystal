...
import { emitFutureRecallQuestionsForCheckpoint } from "./dailyDriver";
...
async function createCheckpointForUser(ctx: any, args: {
  ...
  // ILL-108 F1: emit heuristic future-recall questions this checkpoint's window
  // uniquely answers, each linked to a memory id — persisted as one idempotent
  // organicIdea per checkpoint. The checkpoint tool spreads the response, so
  // futureQuestions reaches the caller with no mcp-server change. Advisory —
  // wrapped so a questions-emission failure can never abort a checkpoint.
  let futureQuestions: Array<{ question: string; memoryId: string }> = [];
  try {
    const emitted = await emitFutureRecallQuestionsForCheckpoint(ctx, {
      userId: args.userId,
      checkpointId: String(id),
      memories,
    });
    futureQuestions = emitted.questions;
  } catch {
    /* advisory only — the checkpoint itself already succeeded */
  }
