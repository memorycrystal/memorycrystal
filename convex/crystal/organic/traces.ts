...
import { organicTracesEnabled } from "./flags";
 ...
 *
 * Matching is opt-in (`CRYSTAL_TRACES_ENABLED=1`). `CRYSTAL_TRACES_DISABLED=1`
 * remains a hard off. Recall skips the action entirely unless traces are
 * enabled, so default traffic pays no extra embed or vector search.
 * Generation continues so the switch is reversible without a data gap.
 */
...
export const matchProspectiveTraces = internalAction({
  ...
  handler: async (ctx, args): Promise<MatchTracesReturn> => {
    ...
    const queryEmbedding = await embedText(args.query, {
      ctx,
      userId: args.userId,
      source: "organic.traces.matchProspectiveTraces",
    }).catch(() => null);
