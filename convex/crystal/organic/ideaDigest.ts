...
const DASHBOARD_IDEAS_URL = "https://memorycrystal.ai/dashboard/organic/ideas";
  ...
  handler: async (ctx) => {
    ...
    for (const tickState of eligibleUsers) {
      ...
      if (tickState.lastIdeaNotificationAt) {
        const timeSinceLast = Date.now() - tickState.lastIdeaNotificationAt;
      ...
      const pendingIdeas = await ctx.runQuery(
        ...
        { userId: tickState.userId, delayMs }
      );
