...
// Cost containment: cap Phase-1 vector searches per run; disable Phase-2 (per-ensemble re-scan) by default.
const MAX_PHASE1_VECTOR_SEARCHES = 8;
const ENSEMBLE_PHASE2_DEFAULT_ENABLED = false;
...
async function callModelForLabel(
  ...
  ctx?: ActionCtx,
  userId?: string,
): Promise<{ label: string; summary: string; spend: EstimatedSpend }> {
...
Return JSON: {"label": "...", "summary": "..."}`;
  ...
  const text = await callOrganicModel(prompt, preset, { ctx, userId, apiKeyOverride });
...
export const processEnsembleTick = internalAction({
  args: { userId: v.string(), tickId: v.string(), organicModel: v.optional(v.string()), openrouterApiKey: v.optional(v.string()), phase2Enabled: v.optional(v.boolean()), maxPhase1VectorSearches: v.optional(v.number()) },
  handler: async (ctx, args) => {
    ...
    const phase2Enabled = args.phase2Enabled ?? ENSEMBLE_PHASE2_DEFAULT_ENABLED;
    const maxP1 = args.maxPhase1VectorSearches ?? MAX_PHASE1_VECTOR_SEARCHES;
    let phase1VectorSearches = 0;
    let phase2VectorSearches = 0;
    let phase2Skipped = !phase2Enabled;
    let ensembleBudgetExhausted = false;

    let estimatedInputTokens = 0;
    let estimatedOutputTokens = 0;
    let estimatedCostUsd = 0;

    if (unclustered.length === 0 && !phase2Enabled) {
      ...
      return {
        ...
        estimatedCostUsd,
        phase1VectorSearches,
        phase2VectorSearches,
        phase2Skipped,
        ensembleBudgetExhausted,
        ensembleSkippedReason: "no_unclustered_memories" as string | undefined,
      };
    }
    ...
    for (const memory of unclustered) {
      ...
      if (phase1VectorSearches >= maxP1) {
        ensembleBudgetExhausted = true;
        break;
      }
      ...
      if (similar.length >= MIN_CLUSTER_SIZE - 1) {
        ...
        let summary: string;
        try {
          const result = await callModelForLabel(memberSummary, preset, apiKeyOverride, ctx, userId);
          label = result.label;
    ...
    for (const ensemble of ensembles) {
      ...
      // PHASE 2 (vector re-scan): grow ensembles by re-searching for new neighbors.
      // This is the unbounded qGB cost driver — disabled by default via phase2Enabled.
      if (!phase2Enabled) continue;
    ...
    return {
      ...
      phase2Skipped,
      ensembleBudgetExhausted,
      ensembleSkippedReason: undefined as string | undefined,
    };
  },
});
