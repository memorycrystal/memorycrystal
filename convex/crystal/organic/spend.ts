...
/** TEMP cost-containment floor (12h). Revert to `60 * 60 * 1000` (1h) to restore normal cadence. */
export const ORGANIC_MIN_INTERVAL_MS = 12 * 60 * 60 * 1000;

export const LIVE_TICK_INTERVAL_MS = 0;
export const MIN_TICK_INTERVAL_MS = ORGANIC_MIN_INTERVAL_MS;
export const MAX_TICK_INTERVAL_MS = 24 * 60 * 60 * 1000;
export const DEFAULT_TICK_INTERVAL_MS = 24 * 60 * 60 * 1000;

/** Legacy sub-hour pulse tiers are shown as disabled in the dashboard. */
export const DISABLED_FAST_PULSE_INTERVAL_TIERS_MS: number[] = [
  60 * 1000,
  5 * 60 * 1000,
  10 * 60 * 1000,
  30 * 60 * 1000,
...
/** Allowed pulse interval tiers (ms). Only tiers >= ORGANIC_MIN_INTERVAL_MS are active during cost-containment. */
...
export function estimateRunsPerPeriod(tickIntervalMs: number) {
  ...
  if (intervalMs === LIVE_TICK_INTERVAL_MS) {
    return {
      daily: 0,
      weekly: 0,
      monthly: 0,
