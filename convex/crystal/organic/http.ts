...
/** Returns true when the user is eligible for Organic features; returns a Response when not. */
async function checkOrganicEligible(
  ctx: ActionCtx,
  userId: string
): Promise<true | Response> {
  const tier = await ctx.runQuery(internal.crystal.userProfiles.getUserTier, { userId });
  if (!isOrganicEligibleTier(tier)) {
    return json({ disabled: true, reason: "organic_requires_ultra" });
  }
  return true;
}
...
export const organicListIdeas = httpAction(async (ctx, request) => {
  ...
  const eligible = await checkOrganicEligible(ctx, auth.userId);
