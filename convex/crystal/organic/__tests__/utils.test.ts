...
const modules = {
  ...
  "crystal/providerGateway": () => import("../../providerGateway"),
};
...
describe("callOrganicModel — Convex override vs env", () => {
  ...
  it("uses Convex override over env for OpenRouter", async () => {
    ...
    await t.run(async (ctx) => {
      await callOrganicModel("test prompt", OPENROUTER_PRESET, { ctx, userId: "user-1" });
    });
  ...
  it("sends new OpenRouter-only presets through the JSON object request shape", async () => {
    ...
    for (const key of keys) {
      ...
      await t.run((ctx) =>
        callOrganicModel("Return {\"ok\":true}", preset, {
          ctx,
          userId: "user-1",
          apiKeyOverride: "test-openrouter-request-key",
        }),
      );
  ...
  it("does not fall back to direct Gemini when OpenRouter is missing", async () => {
    ...
    await expect(t.run((ctx) => callOrganicModel("test prompt", GEMINI_PRESET, { ctx, userId: "user-1" })))
      .resolves.toBe("");
  ...
  it("does not fall back to direct OpenAI when OpenRouter is missing", async () => {
    ...
    await expect(t.run((ctx) => callOrganicModel("test prompt", OPENAI_PRESET, { ctx, userId: "user-1" })))
      .resolves.toBe("");
  ...
  it("does not fall back to direct Anthropic when OpenRouter is missing", async () => {
    ...
    await expect(t.run((ctx) => callOrganicModel("test prompt", ANTHROPIC_PRESET, { ctx, userId: "user-1" })))
      .resolves.toBe("");
