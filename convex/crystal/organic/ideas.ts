...
export const getPendingIdeasInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    for (const idea of ideas) {
      if (!Array.isArray(idea.sourceMemoryIds) || idea.sourceMemoryIds.length === 0) {
        continue;
      }

      let allSourcesInChannel = true;
      for (const memoryId of idea.sourceMemoryIds) {
        const memory = await ctx.db.get(memoryId);
        if (!memory || memory.userId !== args.userId || memory.channel !== channel) {
          allSourcesInChannel = false;
          break;
        }
      }

      if (allSourcesInChannel) {
        scopedIdeas.push(idea);
        if (scopedIdeas.length >= 10) break;
      }
    }
