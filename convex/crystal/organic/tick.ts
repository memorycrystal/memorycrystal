...
type OpenRouterResolution = { apiKey: string | null; source: "personal" | "shared" | null } | null;
...
export const triggerConversationPulse = internalAction({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    try {
      ...
      const result = await callOrganicWithTelemetry(prompt, preset, openrouterApiKey);
      ...
      const ltmExtraction: any = await ctx.runAction((internal as any).crystal.ltmExtraction.runOnDemandLtmExtraction, {
        userId,
        channel: channelKey,
        limit: 12,
      }).catch((error) => ({
        error: error instanceof Error ? error.message : "LTM extraction failed",
      }));
