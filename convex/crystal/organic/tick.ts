...
import { organicPolicyTunerEnabled } from "./flags";
...
import { resolveOpenRouterApiKey, resolveOrganicDefaultPresetKey } from "../adminSettings/resolvers";
import { recordMissingOpenRouterKey } from "../providerGateway";
...
async function getUserTierFromProfiles(ctx: any, userId: string) {
...
type OrganicCoreProcessorTasks<TEnsemble, TContradiction, TResonance, TProcedural> = {
  ensemble: () => Promise<TEnsemble>;
  contradiction: () => Promise<TContradiction>;
  resonance: () => Promise<TResonance>;
  procedural: () => Promise<TProcedural>;
};
...
async function openRouterCostAttribution(ctx: any, apiKeyOverride?: string | null): Promise<{ keySource: CostKeySource; payer: CostPayer }> {
  if (apiKeyOverride) return { keySource: "user_byok", payer: "user" };
  return { keySource: "unknown", payer: "unknown" };
}

export async function runOrganicCoreProcessorsInTickOrder<
  TEnsemble,
  TContradiction,
  TResonance,
  TProcedural,
>(
  processors: OrganicCoreProcessorTasks<TEnsemble, TContradiction, TResonance, TProcedural>,
) {
  const ensembleResultPromise = settle(processors.ensemble());
  const resonanceResultPromise = settle(processors.resonance());
  const proceduralResultPromise = settle(processors.procedural());

  const ensembleResult = await ensembleResultPromise;
  const contradictionResult = await settle(processors.contradiction());
  const [resonanceResult, proceduralResult] = await Promise.all([
    resonanceResultPromise,
    proceduralResultPromise,
  ]);

  return [ensembleResult, contradictionResult, resonanceResult, proceduralResult] as const;
}
...
async function callOrganicWithTelemetry(
  prompt: string,
  preset: ModelPreset,
  options: { ctx: any; userId: string; keyLast4?: string | null; apiKeyOverride?: string },
): Promise<{
  ...
  const text = await callOrganicModel(prompt, preset, options);
...
export const markTickSkipped = internalMutation({
  args: {
    userId: v.string(),
    reason: v.string(),
  },
...
export const recordEnsembleTickWatermark = internalMutation({
  args: { userId: v.string(), at: v.number() },
  handler: async (ctx, args) => {
    const existing = await ctx.db
      .query("organicTickState")
      .withIndex("by_user", (q) => q.eq("userId", args.userId))
      .first();
    if (!existing) return;
    await ctx.db.patch(existing._id, { lastEnsembleTickAt: args.at });
  },
});
...
export const createTickRun = internalMutation({
  ...
  handler: async (ctx, args) => {
    return ctx.db.insert("organicTickRuns", {
      ...
      estimatedCostUsd: 0,
      keySource: args.keySource ?? "unknown",
      payer: args.payer ?? "unknown",
    });
  },
});
...
export const finalizeTickRun = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(run._id, {
      ...
      keySource: args.keySource ?? run.keySource ?? "unknown",
      payer: args.payer ?? run.payer ?? "unknown",
      errorMessage: args.errorMessage,
      activeTraceAgeBuckets,
      phase1VectorSearches: args.phase1VectorSearches,
      phase2VectorSearches: args.phase2VectorSearches,
      phase2Skipped: args.phase2Skipped,
      ensembleBudgetExhausted: args.ensembleBudgetExhausted,
      ensembleSkippedReason: args.ensembleSkippedReason,
    });
    return run._id;
  },
});
...
export const queueMemoryWritePulse = internalMutation({
  args: { userId: v.string() },
  handler: async (ctx, args) => {
    const tier = await getUserTierFromProfiles(ctx, args.userId);
    if (!isOrganicEligibleTier(tier)) {
      return { queued: false, reason: "organic_requires_ultra" };
    }
    const state = await ctx.db
      .query("organicTickState")
      .withIndex("by_user", (q) => q.eq("userId", args.userId))
      .first();
    if (!state || !state.enabled || state.pulseMode !== "on_write") {
      return { queued: false, reason: "disabled" };
    }

    const now = Date.now();
    const leaseActive = Boolean(state.isRunning && (state.leaseExpiresAt ?? 0) > now);
    if (leaseActive) {
      await ctx.db.patch(state._id, { onWritePendingAt: now, updatedAt: now });
...
export const consumeOnWritePending = internalMutation({
  ...
  handler: async (ctx, args) => {
    const state = await ctx.db
      ...
      .withIndex("by_user", (q) => q.eq("userId", args.userId))
      .first();
    if (!state?.onWritePendingAt || state.onWritePendingAt <= args.startedAt) {
      return false;
    }
    await ctx.db.patch(state._id, {
      onWritePendingAt: undefined,
      updatedAt: Date.now(),
    });
    return true;
  },
...
export const processUserTick: any = internalAction({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    if (!isOrganicEligibleTier(tierInfo.tier)) {
      console.log(`[organic-tick] ${userId}: Organic requires Ultra, skipping`);
      return { status: "skipped", reason: "organic_requires_ultra" };
    }
    const budget = await ctx.runQuery(
      internal.crystal.organic.budget.checkOrganicMonthlyBudget,
      { userId, tier: tierInfo.tier }
    );
    if (!budget.allowed) {
      console.log(`[organic-tick] ${userId}: monthly budget exhausted, skipping`);
      await ctx.runMutation(internal.crystal.organic.tick.markTickSkipped, {
        userId,
        reason: "organic_monthly_budget_exhausted",
      });
      return {
        status: "skipped",
        reason: "budget_exhausted_monthly",
        used: budget.used,
        cap: budget.cap,
      };
    }
    ...
    let ensemblePhase1VectorSearches: number | undefined;
    let ensemblePhase2VectorSearches: number | undefined;
    let ensemblePhase2Skipped: boolean | undefined;
    let ensembleBudgetExhausted: boolean | undefined;
    let ensembleSkippedReason: string | undefined;
    ...
    if (preTickState.enabled !== true) {
      console.log(`[organic-tick] ${userId}: disabled, skipping`);
      return;
    }
    ...
    try {
      ...
      if (tickState.enabled !== true) {
        console.log(`[organic-tick] ${userId}: disabled after lease, skipping`);
        await ctx.runMutation(internal.crystal.organic.tick.releaseTickLease, { userId }).catch(() => {});
        return;
      }
      ...
      const openRouterResolution = await ctx.runQuery(
        (internal as any).crystal.providerSettings.resolveOpenRouterKeyForUser,
        { userId, includeShared: false },
      ) as OpenRouterResolution;
      const openrouterApiKey = openRouterResolution?.apiKey ?? tickState.openrouterApiKey ?? undefined;
      const organicCostAttribution = await openRouterCostAttribution(ctx, openrouterApiKey);
      if (!openrouterApiKey) {
        // ILL-184: route the silent skip through the choke point so the alert
        // policy applies (missing_openrouter_key is actionable) — AC10.
        await recordMissingOpenRouterKey(ctx, {
          userId,
          keyLast4: openRouterResolution?.keyLast4 ?? null,
        }).catch(() => {});
        console.warn(`[organic-tick] ${userId}: no OpenRouter API key set, skipping tick`);
        await ctx.runMutation(internal.crystal.organic.tick.markTickSkipped, {
          userId,
          reason: "missing_user_openrouter_key",
        }).catch(() => {});
        await ctx.runMutation(internal.crystal.organic.tick.releaseTickLease, { userId }).catch(() => {});
      ...
      const presetKey = tickState.organicModel ?? await resolveOrganicDefaultPresetKey(ctx, tierInfo.tier);
      ...
      await ctx.runMutation(internal.crystal.organic.tick.createTickRun, {
        ...
        startedAt,
        keySource: organicCostAttribution.keySource,
        payer: organicCostAttribution.payer,
      });
        ...
        if (kbs.length > 0) {
          ...
          for (const kb of kbs) {
            ...
            for (const m of kbMemories) {
              kbLines.push(`[${kb.name}] ${m.title}: ${getMemoryEffectiveText(m).slice(0, 120)}`);
            }
          }
          if (kbLines.length > 0) {
      ...
      const recentMemoriesSummary = recentMemories.length > 0
        ? recentMemories
          .map((m: MemoryDoc) =>
            `[${m.store}/${m.category}] ${m.title}: ${getMemoryEffectiveText(m).slice(0, 120)}`
          )
          .join("\n")
      ...
      const tickGemini = await callOrganicWithTelemetry(prompt, preset, {
        ctx,
        userId,
        keyLast4: openRouterResolution?.keyLast4 ?? null,
      ...
      if (traces.length > 0) {
        const validTraces = await Promise.all(traces.map(async (t) => {
          ...
          const embedding = docDesc.trim() ? await embedText(docDesc, {
            ctx,
            userId: args.userId,
            source: "organic.tick.traceEmbedding",
          }).catch(() => null) : null;
          return {
      ...
      // 9. Run core processors. Contradiction scanning depends on ensemble writes,
      // while resonance and procedural extraction can run independently.
      const organicModel = tickState.organicModel;
      const [ensembleResult, contradictionResult, resonanceResult, proceduralResult] =
        await runOrganicCoreProcessorsInTickOrder({
          ensemble: () => ctx.runAction(internal.crystal.organic.ensembles.processEnsembleTick, {
            phase2Enabled: process.env.ORGANIC_ENSEMBLE_PHASE2_ENABLED === "true",
            userId,
            tickId,
            organicModel,
            openrouterApiKey,
          }),
          contradiction: () => ctx.runAction(internal.crystal.organic.contradictions.scanContradictions, {
            userId,
          ...
          resonance: () => ctx.runAction(internal.crystal.organic.resonance.detectResonance, {
            userId,
            tickId,
            budget: 2,
            organicModel,
          ...
          procedural: () => ctx.runAction(internal.crystal.organic.proceduralExtraction.extractProcedurals, {
            userId,
            tickId,
            organicModel,
            openrouterApiKey,
      ...
      if (ensembleResult.status === "fulfilled") {
        ...
        ensemblePhase1VectorSearches = ensembleResult.value.phase1VectorSearches;
        ensemblePhase2VectorSearches = ensembleResult.value.phase2VectorSearches;
        ensemblePhase2Skipped = ensembleResult.value.phase2Skipped;
        ensembleBudgetExhausted = ensembleResult.value.ensembleBudgetExhausted;
        ensembleSkippedReason = ensembleResult.value.ensembleSkippedReason;
        spendItems.push({
      ...
      if (
        ...
        Date.now() - lastPolicyTune > POLICY_TUNE_INTERVAL
      ) {
        try {
      ...
      await ctx.runMutation(internal.crystal.organic.tick.finalizeTickRun, {
        ...
        phase1VectorSearches: ensemblePhase1VectorSearches,
        phase2VectorSearches: ensemblePhase2VectorSearches,
        phase2Skipped: ensemblePhase2Skipped,
        ensembleBudgetExhausted: ensembleBudgetExhausted,
        ensembleSkippedReason: ensembleSkippedReason,
      });
      await ctx.runMutation(
        internal.crystal.organic.tick.recordEnsembleTickWatermark,
        { userId, at: Date.now() }
      );
      ...
      if (tickSucceeded && args.triggerSource === "memory_write") {
        try {
          const hasPendingWrite = await ctx.runMutation(internal.crystal.organic.tick.consumeOnWritePending, {
            userId,
            startedAt,
          });
          const freshState = await ctx.runQuery(internal.crystal.organic.tick.getTickStateByUser, { userId });
          if (hasPendingWrite && freshState?.enabled && freshState.pulseMode === "on_write") {
            await ctx.scheduler.runAfter(0, (internal as any).crystal.organic.tick.processUserTick, {
              userId,
              lastTickAt: Date.now(),
              triggerSource: "memory_write" as const,
              tickIntervalMs: freshState.tickIntervalMs,
            });
          }
        } catch (schedErr) {
          console.error(`[organic-tick] ${userId}: on-write follow-up failed:`, schedErr);
        }
      }

      // Self-schedule next tick on success
      if (tickSucceeded && args.triggerSource !== "memory_write" && preTickState.pulseMode !== "on_write") {
        try {
          const freshState = await ctx.runQuery(internal.crystal.organic.tick.getTickStateByUser, { userId });
          if (freshState && freshState.enabled && freshState.pulseMode !== "on_write") {
...
export const runTick = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const paidTickStates = [];
    for (const state of tickStates) {
      const tier = await getUserTierFromProfiles(ctx, state.userId);
      if (isOrganicEligibleTier(tier)) paidTickStates.push(state);
    }

    const recoveryStates = paidTickStates.filter((state) => {
      if (state.pulseMode === "on_write") return false;
...
export const triggerConversationPulse = internalAction({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    if (!isOrganicEligibleTier(tierInfo.tier)) {
      return {
        success: false,
        error: "Conversation pulse skipped because Organic requires Ultra",
        tracesCreated: 0,
        topics: [],
      };
    }
    ...
    const organicCostAttribution = await openRouterCostAttribution(ctx, openrouterApiKey);
    if (!openrouterApiKey) {
      // ILL-184: route the silent skip through the choke point (AC10).
      await recordMissingOpenRouterKey(ctx, {
        userId,
        keyLast4: openRouterResolution?.keyLast4 ?? null,
      }).catch(() => {});
      ...
      return {
        success: false,
        error: "Conversation pulse skipped because no user OpenRouter API key is configured",
        tracesCreated: 0,
        topics: [],
      };
    }
    ...
    const rateCheck = await ctx.runMutation(
      internal.crystal.organic.tick.rateLimitAndCreateConversationPulse,
      {
        ...
        tickIntervalMs: 0,
        startedAt,
        cooldownMs: CONVERSATION_PULSE_COOLDOWN_MS,
        keySource: organicCostAttribution.keySource,
        payer: organicCostAttribution.payer,
      }
    );
    ...
    try {
      ...
      if (recentMemories.length > 0) {
        ...
        const related = recentMemories.filter((m: MemoryDoc) => {
          const memWords = `${m.title} ${getMemoryEffectiveText(m)}`.toLowerCase();
          return Array.from(topicWords).some((w) => memWords.includes(w));
        }).slice(0, 15);

        if (related.length > 0) {
          relatedMemoriesSummary = related
            .map((m: MemoryDoc) => `[${m.store}/${m.category}] ${m.title}: ${getMemoryEffectiveText(m).slice(0, 120)}`)
            .join("\n");
