...
import { applyDashboardTotalsDelta, buildMemoryTransitionDelta, buildStrengthDelta } from "../dashboardTotals";
...
const activateApprovedSkillMemory = async (
  ...
  if (existingMemory && existingMemory.userId === params.userId) {
    const nextStrength = Math.max(existingMemory.strength, 0.95);
    ...
    await applyDashboardTotalsDelta(ctx, params.userId, {
      ...buildMemoryTransitionDelta({
        oldArchived: Boolean(existingMemory.archived),
        oldStore: existingMemory.store,
        oldGraphEnriched: existingMemory.graphEnriched === true,
        oldEnrichmentSkippedReason: existingMemory.enrichmentSkippedReason,
        oldAccessCount: existingMemory.accessCount,
        oldKnowledgeBaseId: existingMemory.knowledgeBaseId,
        newArchived: false,
        newStore: existingMemory.store,
        newGraphEnriched: existingMemory.graphEnriched === true,
        newEnrichmentSkippedReason: existingMemory.enrichmentSkippedReason,
        newAccessCount: existingMemory.accessCount,
      }),
      ...buildStrengthDelta(existingMemory.strength, nextStrength),
    });
...
export const analyzeSkillGaps = internalAction({
  ...
  handler: async (ctx, args) => {
      ...
      const results = await Promise.all(
        batch.map(async (row) => {
          const embedding = await embedText(row.query, {
            ctx,
            userId: args.userId,
            source: "organic.skillSuggestions.failedQuery",
          });
    ...
    const existingEmbeddings = await Promise.all(
      existingSuggestions.map(async (suggestion: { skillName: string; description: string }) => {
        const embedding = await embedText(`${suggestion.skillName}\n${suggestion.description}`, {
          ctx,
          userId: args.userId,
          source: "organic.skillSuggestions.existingSuggestion",
        });
        return embedding ?? [];
      })
    );
    ...
    for (const cluster of clusters) {
      ...
      const generated = await generateSkillSuggestion(cluster, successfulQueries, preset, apiKeyOverride, ctx, args.userId);
...
Output JSON:
{
  ...
  const response = await callOrganicModel(prompt, preset, { ctx, userId, apiKeyOverride });
