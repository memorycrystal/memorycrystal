...
export const reinforceProceduralMemory = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const previousAccessCount = Math.max(0, Math.floor(memory.accessCount ?? 0));
    const nextAccessCount = Math.max(previousAccessCount + 1, Math.floor(args.observationCount));
    ...
    const activeRecallCountDelta = !memory.archived ? nextAccessCount - previousAccessCount : 0;
    const activeRecalledMemoriesDelta = !memory.archived && previousAccessCount === 0 && nextAccessCount > 0 ? 1 : 0;
    const aggregateDelta = {
      ...buildStrengthDelta(memory.strength, newStrength),
