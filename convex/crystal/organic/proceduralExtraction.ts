...
import { applyDashboardTotalsDelta, buildStrengthDelta } from "../dashboardTotals";
...
export const reinforceProceduralMemory = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // ILL-183 — this is a background observation incrementer, not genuine
    // recall. Do not bump accessCount or lastRecalledAt; Memory Value treats
    // accessCount as a use signal and lastAccessedAt remains a last-touch field.
    ...
    const aggregateDelta = buildStrengthDelta(memory.strength, newStrength);

    if (
      aggregateDelta.totalStrengthDelta !== 0 ||
      aggregateDelta.activeRecallCountDelta !== 0 ||
      aggregateDelta.activeRecalledMemoriesDelta !== 0
    ) {
  ...
  handler: async (ctx, args) => {
    ...
    for (const candidate of eligibleCandidates.slice(0, MAX_PROCEDURALS_PER_TICK)) {
      ...
      if (duplicateMatch) {
        ...
        try {
          await ctx.runMutation(internal.crystal.memories.createMemoryInternal, {
            userId,
            store: "procedural",
            category: "workflow",
  ...
  handler: async (ctx, args) => {
    ...
    for (const skill of skills) {
      ...
      try {
        ...
        const text = await callOrganicModel(prompt, preset, {
          ctx,
          userId: args.userId,
          apiKeyOverride,
        });
