...
describe("LTM extraction persistence", () => {
  ...
  it("caps on-demand turn extraction to two durable memories", async () => {
    const timestamp = Date.now() - 5 * 60 * 1000;
    await t.run(async (ctx) => {
      await ctx.db.insert("crystalMessages", {
        userId,
        role: "user",
        content: "Remember that I prefer concise client-facing summaries and the launch deadline is May 15.",
        channel: "coach:123",
        sessionKey: "s-on-demand",
        turnId: "turn-1",
        turnMessageIndex: 0,
        timestamp,
        embedded: false,
        expiresAt: timestamp + 100_000,
      });
      await ctx.db.insert("crystalMessages", {
        userId,
        role: "assistant",
        content: "Got it. I will keep the client summaries concise and track the May 15 launch deadline.",
        channel: "coach:123",
        sessionKey: "s-on-demand",
        turnId: "turn-1",
        turnMessageIndex: 1,
    ...
    const mockMemories = [0, 1, 2, 3].map((index) => ({
      store: "semantic" as const,
      category: "fact" as const,
      title: `Durable fact ${index}`,
      content: `Durable extracted fact ${index}.`,
      tags: ["on-demand"],
      confidence: 0.9,
      strength: 0.85,
    }));

    const result = await t.action((internal as any).crystal.ltmExtraction.runOnDemandLtmExtraction, {
      userId,
      sessionKey: "s-on-demand",
      mockMemories,
    });
    ...
    const memories = await t.run((ctx) =>
      ((ctx.db.query("crystalMemories") as any)
        .withIndex("by_user", (q: any) => q.eq("userId", userId).eq("archived", false))
        .collect() as any)
    ) as any[];
    expect(memories.filter((memory: any) => memory.tags?.includes("ltm-on-demand"))).toHaveLength(2);

    const throttled = await t.action((internal as any).crystal.ltmExtraction.runOnDemandLtmExtraction, {
      userId,
      sessionKey: "s-on-demand",
      mockMemories,
    });

    expect(throttled.reason).toBe("cooldown");
    expect(throttled.skipped).toBe(1);
  });
