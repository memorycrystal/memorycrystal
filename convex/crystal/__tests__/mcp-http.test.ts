...
function makeSearchMessagesRequest(body: object) {
  return new Request("https://example.test/api/mcp/search-messages", {
    method: "POST",
    headers: {
      authorization: `Bearer ${AUTH_TOKEN}`,
      "content-type": "application/json",
...
function makeCheckpointRequest(body: object) {
  return new Request("https://example.test/api/mcp/checkpoint", {
    method: "POST",
    headers: {
      authorization: `Bearer ${AUTH_TOKEN}`,
      "content-type": "application/json",
...
function makeLogRequest(body: object) {
  return new Request("https://example.test/api/mcp/log", {
    method: "POST",
    headers: {
      authorization: `Bearer ${AUTH_TOKEN}`,
      "content-type": "application/json",
...
function makeSearchMessagesCtx() {
  const runQuery = vi.fn(async (ref: unknown) => {
    const name = getFunctionName(ref as any);
    if (name === "crystal/mcp:getApiKeyRecord") {
      return { _id: "key-id", active: true, userId: "search-user" };
    }
    if (name === "crystal/messages:searchMessagesByTextForUser") {
      return [
        { messageId: "lexical-right", role: "user", content: "right lexical", channel: "morrow-coach:511172388", timestamp: 1, score: 0.9 },
        { messageId: "lexical-wrong", role: "user", content: "wrong lexical", channel: "morrow-coach:999", timestamp: 2, score: 1 },
      ];
    }
    if (name === "crystal/messages:getRecentMessagesForUser") {
      return [
        { _id: "recent-right", role: "assistant", content: "right recent birthday", channel: "morrow-coach:511172388", timestamp: 3 },
        { _id: "recent-wrong", role: "assistant", content: "wrong recent birthday", channel: "morrow-coach:999", timestamp: 4 },
      ];
    }
    throw new Error(`Unexpected query ref: ${name}`);
  });
  ...
  const runAction = vi.fn(async (ref: unknown) => {
    ...
    if (name === "crystal/messages:searchMessagesForUser") {
      return [
        { messageId: "semantic-right", role: "user", content: "right semantic", channel: "morrow-coach:511172388", timestamp: 5, score: 0.8 },
        { messageId: "semantic-wrong", role: "user", content: "wrong semantic", channel: "morrow-coach:999", timestamp: 6, score: 0.99 },
      ];
    }
    throw new Error(`Unexpected action ref: ${name}`);
  });
...
function makeCheckpointCtx() {
  const runQuery = vi.fn(async (ref: unknown, args?: any) => {
    const name = getFunctionName(ref as any);
    if (name === "crystal/mcp:getApiKeyRecord") {
      return { _id: "key-id", active: true, userId: "checkpoint-user" };
    }
...
describe("mcpCheckpoint", () => {
  it("lists checkpoints without requiring a label", async () => {
    const ctx = makeCheckpointCtx();

    const response = await (mcpCheckpoint as any)(ctx, makeCheckpointRequest({
      mode: "list",
      channel: "codex:local",
      sessionId: "session-a",
      limit: 3,
...
describe("mcpLog", () => {
  it("skips wrapper-only synthetic context before storage quota checks", async () => {
    const ctx = makeCtx({ currentCount: 10_000 });

    const response = await (mcpLog as any)(
      ctx,
      makeLogRequest({
        role: "user",
        content: "<recalled_context><user>old context</user></recalled_context>",
      }),
    );
    ...
    await expect(response.json()).resolves.toEqual({
      ok: true,
      skipped: true,
      reason: "synthetic_context_only",
    });
    expect(ctx.runQuery).not.toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ userId: "snapshot-user" }),
    );
...
describe("mcpSearchMessages", () => {
  it("filters final messages and turns to exact requested channel before responding", async () => {
    makeEmbeddingFetchSpy();
    const ctx = makeSearchMessagesCtx();

    const response = await (mcpSearchMessages as any)(ctx, makeSearchMessagesRequest({
      query: "birthday",
      limit: 10,
    ...
    expect(payload.messages.map((message: any) => message.messageId).sort()).toEqual([
      "lexical-right",
      "recent-right",
    ].sort());
    expect(JSON.stringify(payload)).not.toContain("morrow-coach:999");
    expect(JSON.stringify(payload)).not.toContain("wrong");
    expect(payload.turns.every((turn: any) =>
      turn.messages.every((message: any) => message.channel === "morrow-coach:511172388")
    )).toBe(true);
  });
});
...
describe("mcpEdit", () => {
  it("skips legacy sensory auto-capture before rate limit, quota, insert, and contradiction work", async () => {
    const runQuery = vi.fn(async (ref: unknown) => {
      const name = getFunctionName(ref as any);
    ...
    expect(runMutation.mock.calls.map(([ref]) => getFunctionName(ref as any))).not.toContain("crystal/mcp:checkAndIncrementRateLimit");
    expect(runMutation.mock.calls.map(([ref]) => getFunctionName(ref as any))).not.toContain("crystal/mcp:captureMemory");
    expect(runMutation.mock.calls.map(([ref]) => getFunctionName(ref as any))).not.toContain("crystal/mcp:writeAuditLog");
    expect(runQuery.mock.calls.map(([ref]) => getFunctionName(ref as any))).not.toContain("crystal/userProfiles:getUserTier");
    expect(runQuery.mock.calls.map(([ref]) => getFunctionName(ref as any))).not.toContain("crystal/mcp:getMemoryCount");
    expect(runAction).not.toHaveBeenCalled();
  });

  it("allows explicit sensory raw imports and persists sensory mode audit tags", async () => {
    const runQuery = vi.fn(async (ref: unknown) => {
      const name = getFunctionName(ref as any);
      if (name === "crystal/mcp:getApiKeyRecord") return { _id: "key-id", active: true, userId: "capture-user" };
    ...
    const runMutation = vi.fn(async (ref: unknown) => {
      ...
      if (name === "crystal/apiKeys:touchLastUsedAt") return null;
      if (name === "crystal/mcp:writeAuditLog") return null;
      if (name === "crystal/mcp:captureMemory") return { id: "raw-import-memory" };
      throw new Error(`Unexpected mutation ref: ${name}`);
    });
    const runAction = vi.fn();
    ...
    const response = await (mcpCapture as any)(
      { runQuery, runMutation, runAction } as any,
      makeCaptureRequest({
        title: "Imported raw observation",
        content: "Imported transcript excerpt from explicit support workflow.",
        store: "sensory",
        category: "conversation",
        sensoryCaptureMode: "raw_import",
        tags: ["jsonl-backfill"],
      }),
    );
    ...
    expect(runMutation).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        store: "sensory",
        category: "conversation",
        tags: expect.arrayContaining(["jsonl-backfill", "sensory-mode:raw_import"]),
      }),
    );
  });
