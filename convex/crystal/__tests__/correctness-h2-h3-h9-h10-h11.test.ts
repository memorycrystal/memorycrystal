...
describe("H-9e: plugin recall-hook.js delegates embedding to backend recall", () => {
  ...
  it("uses backend-owned recall instead of local provider embedding", () => {
    expect(recallHookSrc).toContain("/api/mcp/recall");
    expect(recallHookSrc).toContain("MEMORY_CRYSTAL_API_KEY");
    expect(recallHookSrc).not.toContain("GEMINI_EMBEDDING_ENDPOINT");
    expect(recallHookSrc).not.toContain("REQUIRED_EMBEDDING_DIMENSIONS");
  });
});
