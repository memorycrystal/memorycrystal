...
describe("organic conflict inbox", () => {
  ...
  it("records selected correct memories when resolving a conflict", async () => {
    const now = Date.now();
    const { ensembleId, firstId } = await t.run(async (ctx) => {
      const firstId = await insertMemory(ctx, "Correct budget", "Budget is $75K.", now);
      const secondId = await insertMemory(ctx, "Stale budget", "Budget is $50K.", now + 1);
      const pairKey = `${firstId}::${secondId}`;
      const ensembleId = await ctx.db.insert("organicEnsembles", {
        userId: user.subject,
        ensembleType: "conflict_group",
        label: "Conflict: Correct budget vs Stale budget",
        summary: "Budget memories disagree.",
        memberMemoryIds: [firstId, secondId],
        centroidEmbedding: Array.from({ length: 3072 }, () => 0),
        strength: 0.9,
    ...
    await t.withIdentity(user).mutation(api.crystal.organic.conflicts.updateConflictStatus, {
      ...
      resolutionChoice: "A",
      note: "The newer budget is correct.",
    });
    ...
    expect(fullList[0].metadata).toMatchObject({
      status: "resolved",
      resolvedByAction: "resolved",
      resolutionChoice: "A",
      selectedMemoryIds: [String(firstId)],
      resolutionMemoryId: String(firstId),
      resolutionNote: "The newer budget is correct.",
    });
  });
