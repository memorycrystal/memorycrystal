...
async function provisionPaidOrganicUsers(t: ReturnType<typeof convexTest>, userIds: string[]) {
  const now = Date.now();
...
describe("organic dashboard interval settings", () => {
  ...
  beforeEach(async () => {
    ...
    await provisionPaidOrganicUsers(t, [userA.subject, userB.subject]);
  });

  it("rejects Organic enablement for free users", async () => {
    await expect(t.mutation(internal.crystal.organic.adminTick.setOrganicEnabled, {
      userId: "free_organic_user",
      enabled: true,
    })).rejects.toThrow("Organic Memory is available on paid plans only");
  });
  ...
  it("lets each authenticated user manage their own interval", async () => {
    ...
    // Interval mode clamps to the supported one-hour-and-up cadence.
  ...
  it("supports pulse-named dashboard wrappers without changing tick storage fields", async () => {
    ...
    expect(dashboard?.disabledFastPulseIntervalTiers).toEqual([
      60 * 1000,
      5 * 60 * 1000,
      10 * 60 * 1000,
      30 * 60 * 1000,
  ...
  it("clamps user interval settings correctly for the tier system", async () => {
    ...
    // Sub-hour values clamp to the minimum tier.
    await t.withIdentity(userA).mutation(api.crystal.organic.adminTick.setMyOrganicTickInterval, {
    ...
    expect(dashboard?.tickState.pulseMode).toBe("interval");
    ...
    // 0ms also clamps to the minimum supported interval.
    await t.withIdentity(userA).mutation(api.crystal.organic.adminTick.setMyOrganicTickInterval, {
      tickIntervalMs: 0,
    });
