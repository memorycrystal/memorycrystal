...
const TERMINAL_LTM_SKIP_REASONS = new Set([
  "low_signal",
  "no_durable_memory",
...
export const deleteRetiredMessages = internalMutation({
  args: {
    messageIds: v.array(v.id("crystalMessages")),
    retiredAt: v.number(),
    // Force deletion of messages that reached MAX_RETIREMENT_ATTEMPTS even if
    // they never terminally LTM-processed. The caller builds a retirement
    // capsule first so conversation continuity is preserved.
  ...
  handler: async (ctx, args) => {
    ...
    for (const messageId of args.messageIds) {
      ...
      if (!message) continue;
      const terminallyProcessed = Boolean(
        message.ltmExtractedAt ||
        (message.ltmExtractionSkippedReason &&
          TERMINAL_LTM_SKIP_REASONS.has(message.ltmExtractionSkippedReason)),
      );
      if (!terminallyProcessed && !args.force) {
        blocked += 1;
        continue;
      }
      ...
      // hot path this cost fix targets. args.retiredAt is retained in the arg
      // signature for callers/telemetry.
...
const RETIREMENT_RETRY_AFTER_MS = 60 * 60 * 1000;
// Max retirement attempts before a message that can never terminally LTM-process
// is force-retired (capsuled + deleted) so it drains instead of churning forever.
const MAX_RETIREMENT_ATTEMPTS = 5;

// Background scheduling (retire windows + self-scheduled continuations) is skipped
// under test so scheduled functions don't run and write outside the harness
// transaction. Mirrors shouldScheduleRetirementBackgroundWork in messageRetirement.ts.
...
// Groups messages by (userId, channel, sessionKey), patches retirementQueuedAt,
// and schedules a retire window per group. Shared by the never-queued expiry
// scan and the stale-queued retry sweep.
async function queueMessagesForRetirement(
  ctx: any,
  messages: Array<{ _id: any; userId: string; channel?: string; sessionKey?: string; retiredAt?: number }>,
  now: number,
): Promise<number> {
  const groups = new Map<string, Array<(typeof messages)[number]>>();
  for (const message of messages) {
    const key = [message.userId, message.channel ?? "", message.sessionKey ?? ""].join("|");
  ...
  for (const group of groups.values()) {
    const messageIds = group.map((message) => message._id);
    let groupQueued = 0;
    for (const message of group) {
      if (message.retiredAt) continue;
      // Bump retirementQueuedAt so the row leaves both the never-queued range
      // (retirementQueuedAt == undefined) and the retry-eligible range
      // (retirementQueuedAt < retryBefore) until the retry window elapses again.
      await ctx.db.patch(message._id, { retirementQueuedAt: now });
      groupQueued += 1;
    }
    if (groupQueued > 0) {
      queued += groupQueued;
      if (shouldScheduleRetirementWork()) {
        await ctx.scheduler.runAfter(
          0,
          internal.crystal.messageRetirement.retireExpiredMessageWindow,
          { messageIds },
        );
      }
    }
...
export const expireOldMessages = internalMutation({
  ...
  handler: async (ctx, _args) => {
    const now = Date.now();
    // Indexed SEEK of never-queued expired rows only. Previously this used
    // by_expires (the entire expired range) + a post-read .filter() on
    // retirementQueuedAt, so every self-scheduled pass re-read the whole
    // expired backlog — O(N^2) Database I/O. The by_retirement_queue index
    // lets us touch only the rows we actually need to queue.
    const expiredMessages = await ctx.db
      .query("crystalMessages")
      .withIndex("by_retirement_queue", (q) =>
        q.eq("retirementQueuedAt", undefined).lte("expiresAt", now),
      )
      .take(EXPIRE_MESSAGES_BATCH);

    const queued = await queueMessagesForRetirement(ctx, expiredMessages, now);

    // If the batch was full, more never-queued expired rows remain. Self-schedule
    // a continuation to converge tier-based retention (Free 7d / Pro 30d / Ultra 90d).
    // Each pass now reads only O(batch) rows (the just-queued rows leave the
    // undefined range), so the drain is O(N) total instead of O(N^2).
    if (expiredMessages.length === EXPIRE_MESSAGES_BATCH && shouldScheduleRetirementWork()) {
...
// Retry sweep for messages that were queued but not yet retired (their retire
// window failed transiently or is still pending). Indexed seek of stale-queued
// rows only — never re-reads the whole backlog. retireExpiredMessageWindow
// increments retirementAttempts and force-retires at MAX_RETIREMENT_ATTEMPTS,
// so stuck messages drain instead of churning.
export const retryStuckRetirements = internalMutation({
  args: {},
  handler: async (ctx, _args) => {
    const now = Date.now();
    const retryBefore = now - RETIREMENT_RETRY_AFTER_MS;
    const staleQueued = await ctx.db
      .query("crystalMessages")
      .withIndex("by_retirement_queue", (q) =>
        // retirementQueuedAt in [1, retryBefore): numeric-queued (excludes the
        // undefined range, which sorts below 1) and older than the retry window.
        q.gte("retirementQueuedAt", 1).lt("retirementQueuedAt", retryBefore),
      )
      .take(EXPIRE_MESSAGES_BATCH);

    const requeued = await queueMessagesForRetirement(ctx, staleQueued, now);

    if (staleQueued.length === EXPIRE_MESSAGES_BATCH && shouldScheduleRetirementWork()) {
      await ctx.scheduler.runAfter(0, internal.crystal.messages.retryStuckRetirements, {});
    }
...
// Increment retirement attempts for a blocked group and LEAVE the rows queued
// (bumped to now). This is what breaks the churn loop: blocked messages are no
// longer un-queued (retirementQueuedAt reset to undefined) back into the
// never-queued scan every hour. Returns the maxed message ids so the caller can
// force-retire any that reached MAX_RETIREMENT_ATTEMPTS.
export const markRetirementBlocked = internalMutation({
  args: { messageIds: v.array(v.id("crystalMessages")) },
  handler: async (ctx, args) => {
    const now = Date.now();
    const maxedMessageIds: Array<Id<"crystalMessages">> = [];
    for (const messageId of args.messageIds) {
      const message = await ctx.db.get(messageId);
      if (!message || message.retiredAt) continue;
      const attempts = (message.retirementAttempts ?? 0) + 1;
      await ctx.db.patch(messageId, {
        retirementAttempts: attempts,
        retirementQueuedAt: now,
      });
      if (attempts >= MAX_RETIREMENT_ATTEMPTS) {
        maxedMessageIds.push(messageId);
      }
    }
    return { maxedMessageIds };
  },
});
