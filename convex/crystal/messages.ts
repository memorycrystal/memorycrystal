...
const findLeadingRecalledContextBoundary = (value: string): { starts: boolean; end: number } => {
...
export const sanitizeUserMessageContent = (input: string): {
  content: string;
  stripped: boolean;
...
const filterSearchResultsByChannel = <T extends { channel?: string }>(results: T[], channel?: string): T[] => {
  ...
  handler: async (ctx, args) => {
    ...
    const effectiveTtlDays =
      typeof requestedTtlDays === "number" && Number.isFinite(requestedTtlDays) && requestedTtlDays > 0
        ...
        : TIER_TTL_DAYS[tier];
    const ttlMs = effectiveTtlDays * 24 * 60 * 60 * 1000;
    const normalizedContent = normalizeLogMessageContent(rest.role, rest.content);
    if (normalizedContent.skipped) return null;
    const content = truncateContent(normalizedContent.content);
...
const prepareTurnMessage = async (
  ...
  const sessionKey = normalizeText(args.sessionKey);
  const turnId = normalizeText(args.turnId);
  const turnMessageIndex = normalizeTurnMessageIndex(args.turnMessageIndex) ?? args.turnMessageIndex;
  const contentHash = await sha256Hex(buildMessageHashInput({ role: args.role, content }));
  const dedupeScopeHash = await sha256Hex(buildMessageDedupeScopeInput({
    userId: args.userId,
    role: args.role,
...
export const logTurnInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.userMessage !== undefined) {
      prepared.push(await prepareTurnMessage(ctx, {
        userId: args.userId,
        role: "user",
    ...
    const required = prepared.filter((message: any) => !message.skipped && message.inserted).length;
    if (limit !== null && required > 0) {
      const currentCount = await ctx.runQuery(internal.crystal.messages.getMessageCount, { userId: args.userId });
    ...
    if (insertedCount > 0) {
      await applyDashboardTotalsDelta(ctx, args.userId, { totalMessagesDelta: insertedCount });
    }
...
export const clearMessagesRetirementQueued = internalMutation({
  args: {
    messageIds: v.array(v.id("crystalMessages")),
  },
  handler: async (ctx, args) => {
    ...
    for (const messageId of args.messageIds) {
      const existing = await ctx.db.get(messageId);
      if (!existing || existing.retiredAt) continue;
      await ctx.db.patch(messageId, { retirementQueuedAt: undefined });
      updated += 1;
    }
    return { updated };
  },
  ...
  handler: async (ctx, args) => {
    ...
    for (const messageId of args.messageIds) {
      ...
      const terminallyProcessed = Boolean(
        ...
        (message.ltmExtractionSkippedReason && TERMINAL_LTM_SKIP_REASONS.has(message.ltmExtractionSkippedReason))
      );
      if (!terminallyProcessed) {
        blocked += 1;
        continue;
      }
      await ctx.db.patch(messageId, { retiredAt: args.retiredAt });
...
const appendCleanupMetadata = (metadata: string | undefined, timestamp: number): string => {
...
const findMessagesForRecalledContextCleanup = async (
  ...
  if (channel && sessionKey) {
    throw new Error("Cleanup scope is ambiguous: provide channel or sessionKey, not both");
  }

  const applyTimestampBounds = (query: any) => {
    let bounded = query;
    if (args.sinceMs !== undefined) bounded = bounded.gte("timestamp", args.sinceMs);
    if (args.beforeMs !== undefined) bounded = bounded.lte("timestamp", args.beforeMs);
    return bounded;
  };
  ...
  if (channel) {
    return await ctx.db
      .query("crystalMessages")
      .withIndex("by_channel_time", (q: any) => applyTimestampBounds(q.eq("userId", args.userId as never).eq("channel", channel as never)))
      .order("desc")
      .take(limit);
  }
  ...
  if (sessionKey) {
    return await ctx.db
      .query("crystalMessages")
      .withIndex("by_session_time", (q: any) => applyTimestampBounds(q.eq("userId", args.userId as never).eq("sessionKey", sessionKey as never)))
      .order("desc")
      .take(limit);
  }
  ...
  return await ctx.db
    .query("crystalMessages")
    .withIndex("by_user_time", (q: any) => applyTimestampBounds(q.eq("userId", args.userId as never)))
    .order("desc")
    .take(limit);
};
    ...
    for (const message of rows) {
      ...
      if (sanitized.malformed || cleanedLength === 0) {
        ...
        if (args.applyMode === "apply") {
          ...
          await applyDashboardTotalsDelta(ctx, message.userId, { totalMessagesDelta: -1 });
          summary.deleted += 1;
        }
        continue;
      }

      const contentHash = await sha256Hex(buildMessageHashInput({ role: message.role, content: cleanedContent }));
      const dedupeScopeHash = await sha256Hex(buildMessageDedupeScopeInput({
...
export const searchMessagesByTextForUser = internalQuery({
  ...
  handler: async (ctx, args): Promise<SearchMessageResult[]> => {
    ...
    const scanLimit = Math.min(limit * (channel !== undefined || sessionKey !== undefined || args.sinceMs !== undefined ? 12 : 8), 200);
    ...
    for (const message of resultSets.flat()) {
      ...
      if (sessionKey !== undefined && message.sessionKey !== sessionKey) continue;
...
export const getUnembeddedMessagesForUserInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query("crystalMessages")
      .withIndex("by_user_embedded_time", (q) => q.eq("userId", args.userId).eq("embedded", false))
      .order("asc")
      .take(requestedLimit);
  },
});
...
export const expireOldMessages = internalMutation({
  ...
  handler: async (ctx, _args) => {
    const now = Date.now();
    const retryQueuedBefore = now - RETIREMENT_RETRY_AFTER_MS;
    const expiredMessages = await ctx.db
      .query("crystalMessages")
      .withIndex("by_expires", (q) => q.lte("expiresAt", now))
      .filter((q) =>
        q.or(
          q.eq(q.field("retirementQueuedAt"), undefined),
          q.lt(q.field("retirementQueuedAt"), retryQueuedBefore),
        )
      )
      .take(EXPIRE_MESSAGES_BATCH);

    let queued = 0;
    const groups = new Map<string, Array<(typeof expiredMessages)[number]>>();
    ...
    for (const group of groups.values()) {
      ...
      if (groupQueued > 0) {
        queued += groupQueued;
        await ctx.scheduler.runAfter(0, internal.crystal.messageRetirement.retireExpiredMessageWindow, {
          messageIds,
        });
      }
    }
    ...
    if (expiredMessages.length === EXPIRE_MESSAGES_BATCH) {
...
export const searchMessages = action({
  ...
  handler: async (ctx, args): Promise<SearchMessageResult[]> => {
    ...
    const vectorTake = Math.min(
      limit * (channel !== undefined || sessionKey !== undefined || args.sinceMs !== undefined ? 16 : 1),
      256
    );
    ...
    const messages: Array<SearchMessageResult | null> = await Promise.all(
      vectorResults.map(async (result) => {
        ...
        if (sessionKey !== undefined && message.sessionKey !== sessionKey) return null;
