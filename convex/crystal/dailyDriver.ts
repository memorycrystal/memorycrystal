...
/** Bounded strength-ordered window scanned to find core-tagged memories. */
export const CORE_SCAN_WINDOW = 300;

function hasCoreTag(tags: unknown): boolean {
  return (
    Array.isArray(tags) &&
...
/**
 * G2 — the L0 core set. Core memories are high-value (high strength), so we scan
 * a bounded strength-ordered window and filter the reserved tag. User-global
 * (identity applies on every channel). Opt-in: none tagged → []. Bounded to
 * CORE_SCAN_WINDOW reads, once per wake, independent of query recall.
 */
export const getCoreMemories = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const window = await ctx.db
      ...
      .order("desc")
      .take(CORE_SCAN_WINDOW);
    return selectCoreMemories(window, cap).map((m: any) => ({
...
export const getOvernightBrief = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const recentIdeas = await ctx.db
      ...
      .order("desc")
      .take(20);
    const notableIdeas = recentIdeas.slice(0, 5).map((idea: any) => ({
      id: String(idea._id),
      title: idea.title,
...
export const emitOvernightBriefIdea = internalMutation({
  ...
  handler: async (ctx, args) => {
    const pulseId = `overnight-brief:${args.dateKey}`;
