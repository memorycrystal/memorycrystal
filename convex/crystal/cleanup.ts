...
async function summarizeBatchForManualBackfill(
  ...
  if (!result.ok) {
    console.log(`[manual-backfill] OpenRouter summary failed: status=${result.status} ${(result.errorMessage ?? "").slice(0, 200)}`);
    return summaries;
  }
...
export const archiveWeakMemory = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await archiveMemoryAndSyncCleanupProjection(
      ctx,
      existing,
      args.archivedAt ?? existing.archivedAt ?? Date.now(),
    );
  },
});
  ...
  frontendJobId?: any,
    ...
    if (!args.dryRun) {
      if (missingSummary.length > 0 && !openrouterCredential.apiKey) {
        ...
        await recordMissingOpenRouterKey(ctx, { userId }).catch(() => {});
...
export const startMyRetentionCleanup: any = action({
  args: {
    retentionDays: manualRetentionDays,
    dryRun: v.boolean(),
    limit: v.optional(v.number()),
