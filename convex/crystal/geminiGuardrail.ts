...
function shouldRecordUnlimitedUsage(): boolean {
  return process.env.MC_GEMINI_UNLIMITED_USAGE_ACCOUNTING_ENABLED !== "0";
}
...
export const incrementAndCheck = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ allowed: boolean; callCount: number; cap: number | null }> => {
    ...
    if (hasRealUser) {
      const capacity = await getCapacityPolicyForUser(ctx, bucketUserId);
      ...
      cap = capacity.productQuotaExempt ? null : effectiveCap(tier, userCustomCap);
    }
    ...
    // An uncapped account does not need a serialized daily counter to make an
    // admission decision. Self-hosted operators may disable this telemetry to
    // avoid turning concurrent embedding requests into a hot-row bottleneck.
    // Finite tier/global caps always retain exact synchronous accounting.
    if (cap === null && !shouldRecordUnlimitedUsage()) {
      return { allowed: true, callCount: 0, cap };
    }
