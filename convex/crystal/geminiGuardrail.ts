...
export const incrementAndCheck = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ allowed: boolean; callCount: number; cap: number | null }> => {
    ...
    let userCustomCap: number | null = null;
    if (args.userId) {
      tier = await ctx.runQuery(internal.crystal.userProfiles.getUserTier, { userId: args.userId }) as UserTier;
      ...
      const tickState = await ctx.db
        ...
        .first();
      userCustomCap = (tickState as any)?.geminiDailyCap ?? null;
    }

    const cap = effectiveCap(tier, userCustomCap);
