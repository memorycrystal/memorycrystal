...
async function resolveAssetQuotaBytes(ctx: any, userId: string): Promise<number> {
  ...
  if (capacity.storageAdmissionBytes === null) return Number.MAX_SAFE_INTEGER;
  return capacity.storageAdmissionBytes;
}
...
export const retryMyAsset = mutation({
  ...
  handler: async (ctx, { assetId }) => {
    ...
    const frontendJobId = await ctx.db.insert("crystalFrontendJobs", {
      userId,
      kind: "asset_processing",
      resourceId: String(assetId),
      title: `Processing ${asset.title || asset.kind || "asset"}`,
      route: "/storage",
      status: "queued",
      progressCurrent: 0,
    ...
    await ctx.scheduler.runAfter(0, (internal as any).crystal.assets.processFrontendAsset, {
      assetId,
      userId,
      frontendJobId,
    });
    return { ok: true as const, frontendJobId };
  },
});
...
export const processFrontendAsset: any = internalAction({
  args: {
    assetId: v.id("crystalAssets"),
    userId: v.string(),
  ...
  handler: async (ctx, args) => {
    await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
      ...
      userId: args.userId,
      status: "running",
      progressLabel: "Embedding asset content",
    });
    try {
      const result: any = await ctx.runAction(internal.crystal.assets.processAssetText, {
        assetId: args.assetId,
      });
      ...
      await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
        ...
        progressLabel: failed ? "Asset processing failed" : "Asset processing complete",
        error: failed ? String(result?.reason ?? "Asset processing failed") : undefined,
      });
      return result;
    } catch (error) {
      ...
      await ctx.runMutation((internal as any).crystal.frontendJobs.updateInternal, {
        ...
        userId: args.userId,
        status: "failed",
        progressLabel: "Asset processing failed",
        error: message,
      });
...
async function upsertMemoryEmbedding(ctx: any, memoryId: any, embedding: number[]) {
  const existing = await ctx.db
    ...
    .first();
  // M9 — mirror the parent memory's scalar filter fields (userId,
  // knowledgeBaseId) so the side-table vector index can serve scoped searches.
