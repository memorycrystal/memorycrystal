...
export default defineSchema({
  ...
  crystalMemories: defineTable({
    ...
    // ILL-183 — genuine recall only. Written by updateMemoryAccess,
    // updateMemoryAccessInternal, and mcp.bumpAccessCounts. Never stamped at
    // creation. Memory Value uses this; lastAccessedAt stays a last-touch field
    // so existing hygiene / wake / stale readers keep their current meaning.
    ...
    extractionSessionKey: v.optional(v.string()),
    ...
    // ILL-108 G2 — scalar core-tier flag for exact indexed core-memory lookup.
    // Written true when the reserved `core` tag is added, cleared when removed.
    // Absent means not-core (backwards-compatible default).
    coreTier: v.optional(v.boolean()),
  })
    ...
    // ILL-108 G2 — exact indexed seek for core memories, fastest-first.
    // coreTier IN the index (not post-filter) so the query never scans non-core rows.
    .index("by_user_core", ["userId", "coreTier", "archived"])
    ...
    .index("by_user_channel_extraction_session_created", ["userId", "channel", "extractionSessionKey", "archived", "createdAt"])
  ...
  crystalCleanupProjectionState: defineTable({
    ...
    scanAfterCreatedAt: v.optional(v.number()),
  ...
  crystalMessages: defineTable({
    ...
    // Discrete "has been distilled" flag so expireOldMessages can SEEK
    // distilled-and-expired rows without scanning undistilled expired rows.
    // Fail-closed: a message without this flag never enters the retirement
    // index range. Set atomically with ltmExtractedAt.
    ltmExtracted: v.optional(v.boolean()),
    retirementQueuedAt: v.optional(v.number()),
    // Legacy attempt counter from the pre-ILL-182 retry/force-retire path.
    // Retirement no longer increments this; the field stays so existing rows
    // remain readable.
    ...
    // Fail-closed retirement seek (ILL-182). eq(ltmExtracted, true).lte(expiresAt, now)
    // returns only already-distilled expired rows. Undistilled messages — even
    // with expiresAt in the past — never appear in this range, so expire cannot
    // scan them looking for work (the 2026-07-02 +294 GB pathology).
    .index("by_distilled_expires", ["ltmExtracted", "expiresAt"])
    // Legacy never-queued / stale-queued seek. Kept so existing rows remain
    // indexed; expireOldMessages no longer reads this index.
  ...
  crystalDashboardTotals: defineTable({
    ...
    // ILL-179 — Count of ACTIVE memories that carry a knowledgeBaseId, i.e. the
    // subset of `activeMemories` that is imported reference material rather than
    // conversational memory. Non-KB active memories (what the Memory Allowance
    // is measured against) = activeMemories − knowledgeBaseMemories; read it via
    // dashboardTotals.getNonKbActiveMemories, never by subtracting inline.
    // Optional so rows written before this field existed read as 0 with no migration.
    knowledgeBaseMemories: v.optional(v.number()),
  ...
  crystalReflectionRuns: defineTable({
    ...
    trigger: v.union(
      ...
      // ILL-181 — operator-invoked historical backlog drain (one campaign
      // record per drain invocation; per-iteration engine records use
      // "reflection_cycle").
  ...
  crystalApiKeys: defineTable({
    ...
    immutableScope: v.optional(v.boolean()),
    rotationId: v.optional(v.id("crystalApiKeyRotations")),
  })
    .index("by_user", ["userId"])
    .index("by_key_hash", ["keyHash"]),

  crystalApiKeyRotations: defineTable({
    userId: v.string(),
    label: v.string(),
    ...
    // Inbound lookup for delete integrity: find every rotation that references a key.
  ...
  knowledgeBases: defineTable({
    ...
    // ILL-79 — per-agent recall priority multipliers. agentId (or "*" wildcard)
    // → [0.05, 1]; scales the KB's contribution to AMBIENT recall ranking for
    // that agent. Never affects visibility; deliberate single-KB queries are
    // exempt. Absent map (or absent key with no "*") = 1.0 = today's behavior.
    agentPriorities: v.optional(v.record(v.string(), v.float64())),
    scope: v.optional(v.string()),
  ...
  crystalKnowledgeBaseLifecycleJobs: defineTable({
    ...
    frontendJobId: v.optional(v.id("crystalFrontendJobs")),
  ...
  crystalFrontendJobs: defineTable({
    userId: v.string(),
    kind: v.union(
    ...
    .index("by_user_dismissed_updated", ["userId", "dismissedAt", "updatedAt"])
    .index("by_user_status_updated", ["userId", "status", "updatedAt"])
    .index("by_user_kind_resource_updated", ["userId", "kind", "resourceId", "updatedAt"]),
  ...
  crystalResearchCollections: defineTable({
    ...
    .index("by_user_status_updated", ["userId", "status", "updatedAt"])
    .index("by_user_domain_updated", ["userId", "domain", "updatedAt"])
    .index("by_user_agent_updated", ["userId", "agentId", "updatedAt"])
    .index("by_user_channel_updated", ["userId", "channel", "updatedAt"])
  ...
  crystalResearchArtifacts: defineTable({
    ...
    actualChecksum: v.optional(v.string()),
  ...
  crystalResearchItems: defineTable({
    ...
    primaryAsset: v.optional(v.string()),
    primaryTimeframe: v.optional(v.string()),
    primaryDirection: v.optional(v.string()),
    ...
    .index("by_collection_market_review", ["collectionId", "primaryMarket", "reviewStatus", "updatedAt"])
    .index("by_collection_asset_review", ["collectionId", "primaryAsset", "reviewStatus", "updatedAt"])
    .index("by_collection_timeframe_review", ["collectionId", "primaryTimeframe", "reviewStatus", "updatedAt"])
    .index("by_collection_direction_review", ["collectionId", "primaryDirection", "reviewStatus", "updatedAt"])
    .index("by_collection_security_review", ["collectionId", "securityStatus", "reviewStatus", "updatedAt"])
    .index("by_user_collection", ["userId", "collectionId"])
  ...
  crystalResearchRuns: defineTable({
    ...
    .index("by_collection_status_robustness", ["collectionId", "status", "robustnessScore"])
    .index("by_collection_status_sharpe", ["collectionId", "status", "sharpe"])
    .index("by_collection_status_drawdown", ["collectionId", "status", "maxDrawdown"])
    .index("by_collection_status_trades", ["collectionId", "status", "tradeCount"])
    .index("by_collection_status_started", ["collectionId", "status", "startedAt"])
    .index("by_collection_status_completed", ["collectionId", "status", "completedAt"])
  ...
  crystalResearchIngestRecords: defineTable({
    ...
    status: v.union(v.literal("prepared"), v.literal("imported"), v.literal("duplicate"), v.literal("rejected"), v.literal("quarantined"), v.literal("failed")),
    resultingEntityId: v.optional(v.string()),
    rejectionReason: v.optional(v.string()),
    createdAt: v.number(),
    updatedAt: v.number(),
  })
  ...
  crystalResearchPromotions: defineTable({
    ...
    dedupeKey: v.string(),
    status: v.union(v.literal("pending"), v.literal("promoted"), v.literal("demoted"), v.literal("failed")),
    reason: v.string(),
  ...
  // ILL-184: per-user, per-failure-class suppression state for OpenRouter
  // provider alerts. Actionable classes email on first occurrence and stay
  // suppressed until a success is observed; transient classes email only after
  // two consecutive days of blocked cycles. Rows are tiny and keyed by user.
  providerFailureState: defineTable({
    userId: v.string(),
    failureClass: v.string(),
    status: v.union(v.literal("failure"), v.literal("recovered")),
    // emailScheduledAt: an alert send was *dispatched*. emailSentAt: the send
    // was *confirmed delivered* (crystalEmailLog status "sent"). Only the
    // latter suppresses the class — a scheduled-but-failed send must not be
    // able to silence the alarm forever (ILL-184 P2-1).
    emailScheduledAt: v.optional(v.number()),
    emailSentAt: v.optional(v.number()),
    // Start of the current uninterrupted success streak. A recovery email is
    // only sent once the streak outlives RECOVERY_STABLE_MS, so a class that
    // flaps success/failure every cycle cannot generate recovery mail (P3-1).
    recoveryPendingSince: v.optional(v.number()),
    firstFailureAt: v.number(),
    lastFailureAt: v.number(),
    lastFailureDayKey: v.optional(v.string()),
    consecutiveBlockedDays: v.number(),
    keyLast4: v.optional(v.string()),
    lastSuccessAt: v.optional(v.number()),
    updatedAt: v.number(),
  })
    .index("by_user", ["userId"])
    .index("by_user_failure_class", ["userId", "failureClass"]),
  ...
  // Vector index REMOVED 2026-08-05 — it was a full duplicate of
  // crystalMemories.by_embedding held resident in backend memory for nothing.
  // No read path targets this table: all ten memory vectorSearch sites hit
  // crystalMemories.by_embedding, and M9 Phase B (the read switch behind
  // EMBEDDING_TABLE_AS_PRIMARY) was never wired. At ~92k rows x 3072 dims that
  // index cost roughly 1-2 GB resident on convex-backend-prod, which was
  // averaging 21 GB against a 16 GB cap. Rows and dual-write are retained, so
  // Phase B re-adds this block and rebuilds rather than re-mirroring every
  // vector from scratch. See convex/crystal/M9_CUTOVER.md.
  //   .vectorIndex("by_embedding", {
  //     vectorField: "embedding",
  //     dimensions: 3072,
