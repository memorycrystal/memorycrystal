...
export default defineSchema({
  ...
  // ── M8 — Embedding extraction (cost-reduction plan §M8) ──────────────────
  // Hot read paths (recall list, dashboard, evalStats) currently pull the full
  // 24KB inline `crystalMemories.embedding` even when they only need scalar
  // fields. M8 extracts the embedding into its own table so non-vector reads
  // can project it away. Dual-write is active during the migration window;
  // the inline `crystalMemories.embedding` field stays during M8 and is
  // dropped in a future M9 only after `reconcileEmbeddingDualWrite` reports
  // 0 fixed rows for 24 consecutive hours (48 ticks at 30-min cadence) per
  // §9 gate #10.
