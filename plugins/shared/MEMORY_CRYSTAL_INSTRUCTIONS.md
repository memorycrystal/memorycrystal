...
short-term memory. Inject-on hosts can also inject context before model
responses when the operator enables it (MEMORY_CRYSTAL_INJECT_RECALL=true).
Grok Build is Capture tier: automatic capture and tools
work, but the harness cannot inject context, so there is no automatic recall —
call `crystal_recall` (or `crystal_what_do_i_know`) yourself. The tools below
are for *deliberate* recall, saving, and scoping on top of the automatic layer.

**Asymmetric contract for all hosts:**
- **Recall (read):** call `crystal_recall` / `crystal_what_do_i_know` / `crystal_why_did_we` when the task depends on prior decisions, people, commitments, or missing context. Skip self-contained requests.
- **Remember (write):** if this turn produced a durable fact, decision, preference, lesson, rule, or goal, call `crystal_remember` immediately (or `crystal_update` / `crystal_supersede`). Passive capture is not a substitute.
...
- **`agentId`** — gates **knowledge-base** visibility. A KB with
  ...
  "coach"`. The installer writes a platform-slug `agentId` to
  `~/.memory-crystal/config.json` (`claude-code`, `codex`, `cursor`, …).
  `MEMORY_CRYSTAL_AGENT_ID` still overrides that config value. If you omit
  `agentId`, the backend derives one from the channel prefix, which can
  mismatch and silently return **0 KB hits**. Always pass an explicit,
  resolved `agentId` on `recall` / `wake` / `preflight` /
  `query_knowledge_base` when the host knows it. To grant this identity a
  restricted KB, add the slug to the KB's agent allow-list (dashboard
  Knowledge → Agents, or `crystal_set_knowledge_base_access` with
  `action=add`).
...
recognize the hosts `codex`, `claude`, `factory`, `cursor`, and `grok`. **Grok
is Capture tier**: lifecycle hooks must not carry a `matcher`, and the runtime
does not inject recall (stdout is ignored on passive events). **Codex Desktop
needs no separate host key**: it shares `~/.codex/config.toml` and
`~/.codex/hooks.json` with the Codex CLI, so the `codex` host wiring configures
both. Adding a distinct `codex-desktop` host to the hook installers is
unnecessary and would only duplicate the same file writes.
