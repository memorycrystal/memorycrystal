...
test('claude helper removes Memory Crystal hooks and preserves unrelated user hooks', () => {
  const result = run('claude', {
    hooks: {
      SessionStart: [
        { hooks: [{ type: 'command', command: 'node ~/.memory-crystal/crystal-hooks.mjs', timeout: 15 }] },
        { hooks: [{ type: 'command', command: 'keep-session', timeout: 5 }] },
      ],
      UserPromptSubmit: [
        { hooks: [{ type: 'command', command: 'other-hook', timeout: 5 }] },
        { hooks: [{ type: 'command', command: 'node ~/.memory-crystal/crystal-hooks.mjs', timeout: 10 }] },
  ...
  assert.equal(result.hooks.SessionStart[0].hooks[0].command, 'keep-session');
  assert.equal(result.hooks.UserPromptSubmit.length, 1);
  assert.equal(result.hooks.UserPromptSubmit[0].hooks[0].command, 'other-hook');
  assert.equal(result.hooks.Stop, undefined);
  assert.equal(result.hooks.PreToolUse[0].hooks[0].command, 'existing');
  const serialized = JSON.stringify(result);
  assert.doesNotMatch(serialized, /crystal-hooks\.mjs/);
});

test('grok helper removes Memory Crystal hooks and deletes empty lifecycle events', () => {
  const result = run('grok', {
    hooks: {
      SessionStart: [{ hooks: [{ type: 'command', command: 'node ~/.memory-crystal/crystal-hooks.mjs', timeout: 15 }] }],
      UserPromptSubmit: [
        { hooks: [{ type: 'command', command: 'keep-prompt', timeout: 5 }] },
        { hooks: [{ type: 'command', command: 'node ~/.memory-crystal/crystal-hooks.mjs', timeout: 10 }] },
      ],
      Stop: [{ hooks: [{ type: 'command', command: 'node ~/.memory-crystal/crystal-hooks.mjs', timeout: 10 }] }],
    },
  });
  assert.equal(result.hooks.SessionStart, undefined);
  assert.equal(result.hooks.Stop, undefined);
  assert.equal(result.hooks.UserPromptSubmit.length, 1);
  assert.equal(result.hooks.UserPromptSubmit[0].hooks[0].command, 'keep-prompt');
  assert.doesNotMatch(JSON.stringify(result), /crystal-hooks\.mjs/);
});

test('cursor helper removes native Memory Crystal commands and preserves unrelated hooks', () => {
  const result = run('cursor', {
    version: 1,
    hooks: {
      sessionStart: [
        { command: 'existing-session' },
        { command: 'node ~/.memory-crystal/cursor-hooks.mjs' },
      ],
      beforeSubmitPrompt: [{ command: 'node ~/.memory-crystal/cursor-hooks.mjs' }],
    },
  });
  assert.equal(result.hooks.sessionStart.length, 1);
  assert.equal(result.hooks.sessionStart[0].command, 'existing-session');
  assert.equal(result.hooks.beforeSubmitPrompt, undefined);
});
