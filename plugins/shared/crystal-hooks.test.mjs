...
import { buildChannel, buildSessionStartContext, classifyIntent, deriveProjectContext, recall, resolveAgentId, resolvePlatform, resolveSessionKey, wake } from './crystal-hooks.mjs';
...
test('resolveAgentId prefers explicit input and env/config fallback', () => {
  ...
  assert.equal(resolveAgentId({ agentId: 'config-agent' }, {}), 'config-agent');
  if (original) process.env.MEMORY_CRYSTAL_AGENT_ID = original;
});
