...
export const AUTO_RECALL_TIMEOUT_MS = 16_000;
export const DISCIPLINE_FOOTER =
...
/**
 * Append the one-line per-turn discipline footer, reserving it from maxChars
 * so recalled memories are trimmed first. Empty recall still emits the footer.
 */
export function applyDisciplineFooter(payload, maxChars = HOOK_DISPLAY_CEILING_CHARS) {
  ...
  if (body.length > bodyBudget) {
    if (bodyBudget <= 0) trimmedBody = "";
    else if (bodyBudget > marker.length) trimmedBody = body.slice(0, bodyBudget - marker.length) + marker;
    else trimmedBody = body.slice(0, bodyBudget);
  }
...
function estimateTranscriptChars(input) {
...
function normalizeHookEventName(raw) {
  const value = String(raw || "").trim();
  if (!value) return value;
  if (GROK_HOOK_EVENT_ALIASES[value]) return GROK_HOOK_EVENT_ALIASES[value];
  const collapsed = value.toLowerCase().replace(/[_-]/g, "");
  return GROK_HOOK_EVENT_ALIASES[collapsed] || value;
}
...
async function captureStopTurn({ config, input, platform, channel, sessionKey, agentId, projectContext, ts }) {
  ...
  const firstTurn = extractFirstTurn(transcriptPath);
  ...
  const assistantText = firstString(
    ...
    getLastAssistantResponse(transcriptPath),
  );
  const writes = [];
...
export async function main() {
  ...
  const event = normalizeHookEventName(
    input.hook_event_name || input.hookEventName || process.env.GROK_HOOK_EVENT,
  );
  const platform = isGrokInvocation() ? "grok" : resolvePlatform(config, input);
  const cwd = input.cwd || input.workspaceRoot || process.cwd();
  // Gate: Grok (Capture tier) ignores stdout on passive events.
  // Non-Grok: check injectRecallEnabled (env/config, default false).
  const inject = injectRecallEnabled(config);
  ...
  switch (event) {
    ...
    case "UserPromptSubmit": {
      ...
      if (classifySelfContained(prompt)) break;
      if (!inject) break;
      ...
      // AC9: adaptive budget based on conversation history
      const historyChars = estimateTranscriptChars(input);
      const adaptiveMaxChars = adaptiveInjectionBudget(historyChars, budget.maxChars);
      const context = applyDisciplineFooter(formatMemories(memories), adaptiveMaxChars);
      if (context) outputContext("UserPromptSubmit", context);
      break;
    }
