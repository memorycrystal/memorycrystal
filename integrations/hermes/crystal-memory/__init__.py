...
# Fleet-wide recall-quality defaults (operator-validated 12x800 in production):
# surface 12 memories with ~800-char previews inside a ~12.5k-char context
# budget, give automatic recall a 16s budget (slow recall beats none), and
# require 5 consecutive hard failures to open the breaker. Do not raise
# automatic recall past 16s.
MAX_MEMORIES = 12
...
# Timeouts get their own, more tolerant counter: a slow backend response must
# not slam the breaker shut the way hard errors do.
DEFAULT_TIMEOUT_FAILURE_THRESHOLD = 10
...
TOOL_PREAMBLE = (
    ...
    "facts (crystal_remember), and search messages for exact prior wording."
)

MIN_SCORE = 0.35

# Adaptive injection budget floor and max-history window used to shrink the
# injected block when the conversation history is large (AC9).
ADAPTIVE_BUDGET_FLOOR = 2000
ADAPTIVE_WINDOW_CHARS = 32000
ADAPTIVE_REDUCTION_CAP = 0.75

TRIVIAL_PROMPTS = {"ok", "okay", "thanks", "thank you", "yes", "no", "yep", "nope", "sure", "thx", "ty", "got it", "i see", "understood", "makes sense", "right", "correct", "restart", "restart gateway", "restart this gateway"}
...
class SessionState:
    ...
    last_injected_chars: int = 0
    last_injected_est_tokens: int = 0
...
def _inject_min_score() -> float:
    raw = os.environ.get("MEMORY_CRYSTAL_INJECT_MIN_SCORE")
    if raw:
        try:
            parsed = float(raw)
            if 0.0 <= parsed <= 1.0:
...
def _adaptive_budget(context_length: int | None, max_chars: int) -> int:
    """Shrink the injection budget when the conversation history is large."""
    if not context_length or context_length <= 0:
        return max_chars
    clamped = min(context_length, ADAPTIVE_WINDOW_CHARS)
    reduction = min(ADAPTIVE_REDUCTION_CAP, clamped / ADAPTIVE_WINDOW_CHARS)
    adaptive = int(max_chars * (1.0 - reduction))
    return max(ADAPTIVE_BUDGET_FLOOR, adaptive)
...
def _recall_enabled() -> bool:
    return _env_bool("MEMORY_CRYSTAL_INJECT_RECALL", False)
...
def _max_memories() -> int:
    return _env_int("MEMORY_CRYSTAL_MAX_MEMORIES", MAX_MEMORIES)


def _preview_chars() -> int:
    return _env_int("MEMORY_CRYSTAL_PREVIEW_CHARS", MEMORY_PREVIEW_CHARS)


def _failure_threshold() -> int:
    return _env_int("MEMORY_CRYSTAL_FAILURE_THRESHOLD", DEFAULT_FAILURE_THRESHOLD)


def _timeout_failure_threshold() -> int:
    return _env_int("MEMORY_CRYSTAL_TIMEOUT_FAILURE_THRESHOLD", DEFAULT_TIMEOUT_FAILURE_THRESHOLD)
    ...
    def request(self, path: str, payload: dict[str, Any] | None = None, method: str = "POST", timeout: float | None = None) -> dict[str, Any]:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "memory-crystal-hermes-plugin/0.8.18",
            },
        )
        try:
            ...
            return {"ok": False, "error": str(err) or "timed out", "timedOut": True}
...
def _agent_id(kwargs: dict[str, Any]) -> str:
    explicit = _first_kwarg(kwargs, "agent_id", "agentId") or _clean_text(os.environ.get("MEMORY_CRYSTAL_AGENT_ID"))
    if explicit:
        return explicit
    # Default the agent identity to the Hermes profile name so agent-scoped KB
    # visibility works without operators exporting MEMORY_CRYSTAL_AGENT_ID.
    profile = _first_kwarg(kwargs, "profile") or _clean_text(os.environ.get("HERMES_PROFILE"))
    if profile:
        return _slug(os.path.basename(profile.rstrip("/")) or profile, "")
    return ""
...
def _format_memories(memories: list[dict[str, Any]]) -> str:
    ...
    for memory in memories[: _max_memories()]:
        ...
        content = _clean_text(memory.get("content") or memory.get("text") or "")[:preview_chars]
...
def _bounded_context(parts: list[str], max_chars: int | None = None) -> str:
    limit = max_chars if max_chars is not None else _context_budget()
...
def _log_injection_stats(state, skip_reason, query_length, content_length=0, latency_ms=None) -> None:
    """AC11: Log per-turn cost metrics. Call once per _recall_context exit."""
    est_tokens = max(0, (content_length + 3) // 4)
    lat = latency_ms if latency_ms is not None else (state.last_recall_latency_ms or 0)
    LOGGER.info(
        "crystal-memory per-turn: injected_chars=%d est_tokens=%d latency_ms=%d skip_reason=%s",
        content_length, est_tokens, lat, skip_reason or "none",
    )


# AC10: background prefetch for wake so pre_llm_call does not block first-token
# on wake when inject is on and the cache is cold.
_PREFETCH_WAKE_LOCK = threading.Lock()
_PREFETCH_WAKE_THREAD: threading.Thread | None = None
_PREFETCH_WAKE_RESULT: dict[str, Any] | None = None
_PREFETCH_WAKE_SESSION: str = ""
_PREFETCH_WAKE_AGE: float = 0.0

# AC10: background prefetch for recall so the calling thread never blocks
_PREFETCH_RECALL_LOCK = threading.Lock()
_PREFETCH_RECALL_THREAD: threading.Thread | None = None
_PREFETCH_RECALL_SESSION: str = ""
_PREFETCH_RECALL_CACHE_KEY: str = ""


def _prefetch_wake(session_id: str, channel: str, client: Any, *, kwarg_scope: dict[str, Any] | None = None) -> None:
    """Background-warm the wake result so a timed-out refresh does not disable
    the next turn's prefetch. The calling thread returns immediately."""
    global _PREFETCH_WAKE_THREAD, _PREFETCH_WAKE_RESULT, _PREFETCH_WAKE_SESSION, _PREFETCH_WAKE_AGE

    state = _session_state(session_id)
    with _PREFETCH_WAKE_LOCK:
        # Do not respawn a thread that is still running
        if _PREFETCH_WAKE_THREAD and _PREFETCH_WAKE_THREAD.is_alive():
            return
        _PREFETCH_WAKE_SESSION = session_id
        _PREFETCH_WAKE_AGE = time.time()

        def _refresh_wake():
            try:
                result = client.wake(session_id, channel, automatic=True, **(kwarg_scope or {}))
                _record_backend_result(state, session_id, result)
                _PREFETCH_WAKE_RESULT = result
                state.wake_injected = True
            except Exception as err:
                _record_circuit_failure(state, timed_out=isinstance(err, TimeoutError) or _is_timeout_error(err))
                _record_error(session_id, err)
                # A failed prefetch does not disable the next turn's prefetch

        _PREFETCH_WAKE_THREAD = threading.Thread(target=_refresh_wake, daemon=True)
        _PREFETCH_WAKE_THREAD.start()


def _consume_prefetched_wake(session_id: str) -> dict[str, Any] | None:
    """Return the prefetched wake result if it belongs to this session
    and is recent (under 30s). Clears the prefetch regardless."""
    global _PREFETCH_WAKE_RESULT
    with _PREFETCH_WAKE_LOCK:
        if _PREFETCH_WAKE_SESSION == session_id and _PREFETCH_WAKE_RESULT is not None:
            result = _PREFETCH_WAKE_RESULT
            _PREFETCH_WAKE_RESULT = None
            return result
        _PREFETCH_WAKE_RESULT = None
        return None

def _prefetch_recall(session_id: str, channel: str, client: Any, query: str, cache_key: str, *, kwarg_scope: dict[str, Any] | None = None) -> None:
    """Background-warm the recall result so the calling thread returns immediately.
    A timed-out refresh must not disable the next turn's prefetch."""
    global _PREFETCH_RECALL_THREAD, _PREFETCH_RECALL_SESSION, _PREFETCH_RECALL_CACHE_KEY

    state = _session_state(session_id)
    with _PREFETCH_RECALL_LOCK:
        if _PREFETCH_RECALL_THREAD and _PREFETCH_RECALL_THREAD.is_alive():
            return
        _PREFETCH_RECALL_SESSION = session_id
        _PREFETCH_RECALL_CACHE_KEY = cache_key

        def _refresh_recall():
            try:
                result = client.recall(query, session_id, channel, limit=_max_memories(), automatic=True, **(kwarg_scope or {}))
                _record_backend_result(state, session_id, result)
                ...
                _record_circuit_failure(state, timed_out=isinstance(err, TimeoutError) or _is_timeout_error(err))
                _record_error(session_id, err)
                # A failed prefetch must not disable the next turn's prefetch

        _PREFETCH_RECALL_THREAD = threading.Thread(target=_refresh_recall, daemon=True)
        _PREFETCH_RECALL_THREAD.start()
...
def _recall_context(session_id: str, query: str, platform: str | None = None, *, is_first_turn: bool = False, **kwargs: Any) -> str:
    ...
    if not text:
        state.last_skip_reason = "empty_query"
        _log_injection_stats(state, "", len(text))
        return ""
    ...
    # AC9: self-contained / trivial prompt — skip recall
    ...
    # AC9: adaptive budget based on conversation history size
    history_chars = kwargs.get("_history_chars", 0)
    max_chars = _adaptive_budget(history_chars, MAX_CONTEXT_CHARS)
    ...
    if cache_key in state.recall_cache:
        ...
        _log_injection_stats(state, "", len(text))
        return _bounded_context(parts, max_chars=max_chars)
    if _circuit_open(state):
        ...
        _log_injection_stats(state, state.last_skip_reason, len(text))
        return _bounded_context(parts, max_chars=max_chars)

    start = time.time()
    try:
        client = _client()
        # AC10: use prefetched/stale wake when available; do not block on wake
        if is_first_turn or not state.wake_injected:
            prefetched = _consume_prefetched_wake(session_id)
            if prefetched:
                _record_backend_result(state, session_id, prefetched)
                parts.append(_format_wake(prefetched))
                state.wake_injected = True
            elif state.last_recall_context:
                # Stale context is better than blocking first-token on wake
                parts.append(state.last_recall_context)
                _prefetch_wake(session_id, channel, client, kwarg_scope=_memory_scope(kwargs))
            else:
                # AC10: cache miss — kick background prefetch instead of blocking
                _prefetch_wake(session_id, channel, client, kwarg_scope=_memory_scope(kwargs))

        # AC10: when stale context exists, return it immediately and refresh in background
        if state.last_recall_context:
            parts.append(state.last_recall_context)
            _prefetch_recall(session_id, channel, client, text, cache_key, kwarg_scope=_memory_scope(kwargs))
            state.last_recall_cache_status = "stale"
            context_text = _bounded_context(parts, max_chars=max_chars)
            state.last_injected_chars = len(context_text)
            state.last_injected_est_tokens = max(0, (len(context_text) + 3) // 4)
            state.last_recall_latency_ms = 0
            _log_injection_stats(state, "prefetch_queue", len(text), content_length=len(context_text), latency_ms=0)
            return context_text

        # AC10: cache miss on recall — kick background prefetch, return preamble/tools only
        _prefetch_recall(session_id, channel, client, text, cache_key, kwarg_scope=_memory_scope(kwargs))
        state.last_recall_cache_status = "miss"
        context_text = _bounded_context(parts, max_chars=max_chars)
        state.last_injected_chars = len(context_text)
        state.last_injected_est_tokens = max(0, (len(context_text) + 3) // 4)
        state.last_recall_latency_ms = 0
        _log_injection_stats(state, "cache_miss_prefetch", len(text), content_length=len(context_text), latency_ms=0)
        return context_text
    except Exception as err:
        _record_circuit_failure(state, timed_out=isinstance(err, TimeoutError) or _is_timeout_error(err))
    ...
    state.last_injected_chars = len(context_text)
    state.last_injected_est_tokens = max(0, (len(context_text) + 3) // 4)
    _log_injection_stats(state, state.last_skip_reason, len(text), content_length=len(context_text), latency_ms=state.last_recall_latency_ms)
    return context_text
...
def pre_llm_call(
    ...
    **kwargs: Any,
    ...
    # AC10: measure conversation_history for adaptive budget, then pass to _recall_context
    history_chars = 0
    if conversation_history:
        history_chars = sum(
            len(str(msg.get("content", ""))) if isinstance(msg, dict) else len(str(msg))
            for msg in conversation_history[-50:]  # last 50 messages only
        )
    kwargs["_history_chars"] = history_chars
    del model
...
def post_llm_call(
    ...
    **kwargs: Any,
    ...
    # AC10: keep conversation_history — it is used for adaptive budget in _recall_context
    # Only del model (not conversation_history)
    del conversation_history  # noqa
    del model
    session = _session_key(session_id)
    ...
    def get_config_schema(self) -> list[_ConfigField]:
        return [
            ...
            _ConfigField(name="capture_turns", type="boolean", default=True, env="MEMORY_CRYSTAL_CAPTURE_TURNS"),
             _ConfigField(name="inject_recall", type="boolean", default=False, env="MEMORY_CRYSTAL_INJECT_RECALL"),
    ...
    result = {
        ...
        "sessions": {
            key: {
                ...
                "lastInjectedChars": value.last_injected_chars,
                "lastInjectedEstTokens": value.last_injected_est_tokens,
                "lastError": value.last_error,
                "circuitOpen": _circuit_open(value),
                "circuitFailures": value.circuit_failures,
                "circuitTimeouts": value.circuit_timeouts,
                "hooks": value.hooks,
            }
