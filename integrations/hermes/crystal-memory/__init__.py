...
class MemoryCrystalClient:
    ...
    def request(self, path: str, payload: dict[str, Any] | None = None, method: str = "POST", timeout: float | None = None) -> dict[str, Any]:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "memory-crystal-hermes-plugin/0.8.12",
            },
        )
        try:
    ...
    def wake(self, session_id: str, channel: str, *, automatic: bool = False) -> dict[str, Any]:
        return self.request(
            "/api/mcp/wake",
    ...
    def recall(self, query: str, session_id: str, channel: str, limit: int = MAX_MEMORIES, *, automatic: bool = False) -> dict[str, Any]:
        return self.request(
            "/api/mcp/recall",
...
def _write_skip_reason(kwargs: dict[str, Any]) -> str:
    ...
    if _is_group_context(kwargs):
        if not _env_bool("MEMORY_CRYSTAL_ALLOW_GROUP_WRITES", True):
...
def _recall_context(session_id: str, query: str, platform: str | None = None, *, is_first_turn: bool = False, **kwargs: Any) -> str:
    ...
    try:
        ...
        if is_first_turn or not state.wake_injected:
            wake = client.wake(session_id, channel, automatic=True)
            _record_backend_result(state, session_id, wake)
            parts.append(_format_wake(wake))
            state.wake_injected = True
        recall = client.recall(text, session_id, channel, automatic=True)
...
class MemoryCrystalProvider:
    ...
    def get_config_schema(self) -> list[_ConfigField]:
        return [
            ...
            _ConfigField(name="allow_group_writes", type="boolean", default=True, env="MEMORY_CRYSTAL_ALLOW_GROUP_WRITES"),
