    ...
    def request(self, path: str, payload: dict[str, Any] | None = None, method: str = "POST", timeout: float | None = None) -> dict[str, Any]:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "memory-crystal-hermes-plugin/0.8.8",
            },
        )
        try:
...
def crystal_doctor(ctx: Any = None, argstr: str = "", **kwargs: Any) -> str:
    ...
    rows.append(_doctor_row("pass" if not status["allowGroupWrites"] else "warn", "group writes", "disabled" if not status["allowGroupWrites"] else "enabled"))
