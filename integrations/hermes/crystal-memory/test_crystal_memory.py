...
class FakeClient:
    ...
    def wake(self, session_id, channel, automatic=False):
        self.calls.append(("wake", session_id, channel))
        return {"briefing": "Welcome back. Current project: Hermes integration."}

    def recall(self, query, session_id, channel, limit=6, automatic=False):
...
class HermesPluginTests(unittest.TestCase):
    ...
    def test_post_llm_call_captures_group_writes_by_default(self):
        plugin.post_llm_call(
            session_id="s1",
    ...
    def test_post_llm_call_skips_group_writes_when_disabled(self):
        os.environ["MEMORY_CRYSTAL_ALLOW_GROUP_WRITES"] = "false"
