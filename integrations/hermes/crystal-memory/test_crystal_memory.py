...
class HermesPluginTests(unittest.TestCase):
    ...
    def test_auto_recall_timeout_default_is_eight_seconds(self):
        class TimeoutClient(plugin.MemoryCrystalClient):
            def __init__(self):
                super().__init__(api_url="https://convex.example", api_key="mc_test")
                self.requests = []
    ...
    def test_recall_defaults_surface_ten_memories_with_750_char_previews(self):
        long_content = "x" * 2000
        ...
        self.assertEqual(recall_call[4], 10)

        context = result["context"]
        listed = [line for line in context.splitlines() if line.startswith("- [semantic/fact]")]
        self.assertEqual(len(listed), 10)
        # Preview cap: title + 750-char content slice per row (not the old 360).
        first_row = listed[0]
        self.assertIn("0:" + "x" * (750 - len("0:")), first_row)
        self.assertLess(len(first_row), 900)
    ...
    def test_default_context_budget_is_eleven_thousand_chars(self):
        self.assertEqual(plugin._context_budget(), 11000)
        long_parts = ["z" * 20000]
        bounded = plugin._bounded_context(long_parts)
        self.assertLessEqual(len(bounded), 11000)
