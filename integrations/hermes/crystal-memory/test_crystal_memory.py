...
class HermesPluginTests(unittest.TestCase):
    def setUp(self):
        ...
        plugin._PREFETCH_WAKE_THREAD = None
        plugin._PREFETCH_WAKE_RESULT = None
        plugin._PREFETCH_WAKE_SESSION = ""
        plugin._PREFETCH_RECALL_THREAD = None
        plugin._PREFETCH_RECALL_SESSION = ""
        plugin._PREFETCH_RECALL_CACHE_KEY = ""
    ...
    def _wait_prefetch(self, timeout=1.0):
        """Wait for the background prefetch thread to complete and state to settle."""
        deadline = time.time() + timeout
        # Wait for daemon thread to finish executing
        while time.time() < deadline:
            thread = plugin._PREFETCH_RECALL_THREAD
            if thread is None:
                time.sleep(0.005)
                continue
            if not thread.is_alive():
                time.sleep(0.05)  # Let daemon's post-call processing settle
                return
            time.sleep(0.005)
        # Last chance: check if thread died during sleep
        thread = plugin._PREFETCH_RECALL_THREAD
        if thread is not None and not thread.is_alive():
            time.sleep(0.05)
            return
        raise TimeoutError("prefetch thread did not finish within 1s")
